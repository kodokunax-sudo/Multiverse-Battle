...
function sfxArenaBombTick() { playArenaSound(1000, 'square', 0.06, 0.02); }
        ...
        case 3:
            {
                ...
                attacks.push({ type: "danger", x: 200, y: 180, size: 70, spd: vx, spdY: vy, color: "#ff3333", damageOnMoving: true, damage: Math.floor(dmg * 1.6), bouncesLeft: 5 });
            }
            break;
        case 4:
            for (let i = 0; i < (isEarly ? 1 : 2); i++) {
...
function renderArena() {
    ...
    if (arenaPhase === "dodge") {
        for (let i = attacks.length - 1; i >= 0; i--) {
            ...
            if (a.type === "bomb") {
                ...
                if (a.timer > 0) {
                    ...
                    if (a.timer < 15 && a.timer % 6 === 0) sfxArenaBombTick();
                }
                else if (a.timer > -20) {
                    // Взрыв бомбы
            ...
            if (b.state === "aiming") {
                ...
                if (!b.hasHit && invulnTimer <= 0) {
                    ...
                    if (dist < b.width / 2 + heart.hitbox) {
                        ...
                        else if (b.color === "#ffdd00") { if (heartWasMoving) { bdmg = Math.max(1, Math.floor(arenaBaseDmg * 3 / 4)); msg = "ЗАЩИТА!"; } else { bdmg = Math.floor(arenaBaseDmg * 3); msg = "СТОИШЬ!"; } }
                        else if (b.color === "#ff3333") { if (!heartWasMoving) { bdmg = Math.max(1, Math.floor(arenaBaseDmg * 3 / 4)); msg = "ЗАМЕР!"; } else { bdmg = Math.floor(arenaBaseDmg * 3); msg = "ДВИЖЕНИЕ!"; } }
        ...
        for (let i = 0; i < arenaClickTargets.length; i++) {
            ...
            ctx.save(); let grad = ctx.createRadialGradient(t.x, t.y, 2, t.x, t.y, r);
            ...
            ctx.beginPath(); ctx.moveTo(t.x - r - 2, t.y); ctx.lineTo(t.x - 5, t.y); ctx.moveTo(t.x + 5, t.y); ctx.lineTo(t.x + r + 2, t.y);
            ctx.moveTo(t.x, t.y - r - 2); ctx.lineTo(t.x, t.y - 5); ctx.moveTo(t.x, t.y + 5); ctx.lineTo(t.x, t.y + r + 2); ctx.stroke();
            ctx.restore();
        }
    }
