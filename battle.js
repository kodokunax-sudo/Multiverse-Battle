// ========== АРЕНА UNDERTALE v10.5 FINAL ==========
// Полный файл со всеми функциями
// Исправлены все отрицательные радиусы в arc()
...
function sfxArenaRainbow() { playArenaSound(50, 'sawtooth', 1.5, 0.15); setTimeout(function() { playArenaSound(80, 'sawtooth', 1.2, 0.12); }, 200); setTimeout(function() { playArenaSound(60, 'sawtooth', 1.0, 0.1); }, 400); }
...
function startArena(bossWave) {
    ...
    if (typeof team !== 'undefined' && typeof mainCardIndex !== 'undefined' && team.length > 0 && mainCardIndex >= 0 && mainCardIndex < team.length) {
        ...
        if (typeof myCards !== 'undefined' && mainCardIdx >= 0 && mainCardIdx < myCards.length) {
            var mainCard = myCards[mainCardIdx];
...
function startDodgePhase() {
    ...
    arenaAttackInterval = setInterval(function() {
    ...
    var dodgeTime = Math.floor(10000 + Math.random() * 6000);
    ...
    arenaPhaseTimeout = setTimeout(function() { 
        if (arenaPhase === "dodge" && arenaActive) {
            if (arenaDodgeTimerInterval) { clearInterval(arenaDodgeTimerInterval); arenaDodgeTimerInterval = null; }
            startAttackPhase(); 
        }
...
function startAttackPhase() {
    ...
    var usedPositions = [];
    for (var i = 0; i < arenaTotalTargets; i++) {
        var x, y, tooClose, attempts = 0;
        do {
            x = 40 + Math.random() * 320; y = 80 + Math.random() * 340;
            tooClose = false;
            for (var p of usedPositions) { if (Math.sqrt((x - p.x) * (x - p.x) + (y - p.y) * (y - p.y)) < 60) { tooClose = true; break; } }
            attempts++;
...
function applyArenaDamage() {
    if (!arenaActive) return;
    var dmgMult = 0;
    var ratio = arenaClicksHit / arenaTotalTargets;
...
function spawnBlaster(w) {
    var bossW = arenaCurrentWave;
    var isRainbow = Math.random() < 0.05 && bossW >= 800;
    var colors = ["#fff", "#ffdd00", "#ff3333"];
    var bColor = isRainbow ? "rainbow" : colors[Math.floor(Math.random() * colors.length)];
...
function spawnAttack() {
    var s = arenaSpeedMult;
    var bw = arenaCurrentWave;
    var isEarly = bw < 100;
    var dmg = arenaBaseDmg;
    ...
    switch (arenaAttackType) {
        case 0:
            ...
            if (isVertical) {
                var offsetDirection = Math.random() > 0.5 ? 1 : -1;
                var gapCenter = heart.y + offsetDirection * (50 + Math.random() * 80);
                var gapSize = 70 + Math.random() * 40;
                var startX = Math.random() > 0.5 ? -30 : 430;
                var dirX = startX < 0 ? 3.5 * s : -3.5 * s;
                ...
                var offsetDirection = Math.random() > 0.5 ? 1 : -1;
                var gapCenter = heart.x + offsetDirection * (50 + Math.random() * 80);
                var gapSize = 70 + Math.random() * 40;
                var startY = Math.random() > 0.5 ? -30 : 530;
                var dirY = startY < 0 ? 2.4 * s : -2.4 * s;
        ...
        case 1:
            ...
            for (var i = 0; i < chaosCount; i++) {
                ...
                var angle = Math.atan2(heart.y - y, heart.x - x);
                var atk = { type: "square", x: x, y: y, size: 20, spd: Math.cos(angle) * 2.0, spdY: Math.sin(angle) * 2.0, color: "#4499ff", damage: Math.floor(dmg / 2), bouncesLeft: 3 };
                if (shouldShrink) shrinkAttack(atk);
                attacks.push(atk);
            }
            break;
        case 2: 
            for (var i = 0; i < (isEarly ? 1 : 2); i++) {
                var side = Math.floor(Math.random() * 4);
                var xPos, yPos;
                ...
                var angle = Math.atan2(heart.y - yPos, heart.x - xPos);
                var atk = { type: "sword", x: xPos, y: yPos, angle: angle, size: 45, width: 15, color: "#ffaa00", spd: Math.cos(angle) * 2.4, spdY: Math.sin(angle) * 2.4, damageOnStanding: true, damage: Math.floor(dmg * 1.2), bouncesLeft: 0 };
                if (shouldShrink) shrinkAttack(atk);
                attacks.push(atk);
            }
            break;
        case 3:
            for (var i = 0; i < (isEarly ? 1 : 2); i++) {
                var vx = (Math.random() > 0.5 ? 1 : -1) * (0.8 + Math.random() * 0.4) * s;
                var vy = (Math.random() > 0.5 ? 1 : -1) * (0.8 + Math.random() * 0.4) * s;
                var atk = { type: "danger", x: 200, y: 180, size: 70, spd: vx, spdY: vy, color: "#ff3333", damageOnMoving: true, damage: Math.floor(dmg * 1.6), bouncesLeft: 1 };
                if (shouldShrink) shrinkAttack(atk);
                attacks.push(atk);
            }
            break;
        case 4:
            for (var i = 0; i < (isEarly ? 2 : 3); i++) {
                var rx = (Math.random() - 0.5) * 2;
                var atk = { type: "square", x: heart.x + (Math.random() - 0.5) * 120, y: -40, size: 25, spd: rx, spdY: 1.8 + Math.random() * 1.0, color: "#ff69b4", knockback: 80, bouncesLeft: Infinity };
                if (shouldShrink) shrinkAttack(atk);
                attacks.push(atk);
            }
            break;
        case 5:
            if (arenaHP < arenaMaxHP) {
                var atk = { type: "circle", x: Math.random() * 400, y: -30, radius: 18, spd: (Math.random() - 0.5) * 1.5, spdY: 2.5, color: "#44ff44", healPercent: 0.15, bouncesLeft: 2 };
                if (shouldShrink) shrinkAttack(atk);
                attacks.push(atk);
            } else {
                var atk = { type: "square", x: Math.random() * 360 + 20, y: -30, size: 22, spd: 0, spdY: 3.0, color: "#fff", damage: dmg, bouncesLeft: 2 };
                if (shouldShrink) shrinkAttack(atk);
                attacks.push(atk);
            }
            break;
        case 6:
            var atk6 = { type: "rainbow", x: 50 + Math.random() * 300, y: -60, radius: 30, spd: (Math.random() - 0.5) * 0.8, spdY: 1.2 + Math.random() * 0.5, color: "rainbow", oneshot: true, bouncesLeft: 0 };
        ...
        case 7:
            for (var i = 0; i < (isEarly ? 1 : 2); i++) {
                ...
                var sa = Math.atan2(heart.y - sy, heart.x - sx);
                var atk1 = { type: "sword", x: sx, y: sy, angle: sa, size: 40, width: 12, color: "#ffaa00", spd: Math.cos(sa) * 2.2, spdY: Math.sin(sa) * 2.2, damageOnStanding: true, damage: Math.floor(dmg * 1.3), bouncesLeft: 0 };
                var atk2 = { type: "danger", x: heart.x + (Math.random() - 0.5) * 100, y: 540, size: 25, spd: (Math.random() - 0.5) * 1.2, spdY: -2.2, color: "#ff3333", damageOnMoving: true, damage: Math.floor(dmg * 1.3), bouncesLeft: 0 };
            ...
            break;
        case 8:
            var atk81 = { type: "danger", x: -40, y: heart.y, size: 40, spd: 2.5 * s, spdY: 0, color: "#ff3333", damageOnMoving: true, damage: Math.floor(dmg * 1.5), bouncesLeft: 0 };
            if (shouldShrink) shrinkAttack(atk81);
            attacks.push(atk81);
            if (!isEarly) {
                var atk82 = { type: "square", x: 440, y: heart.y + (Math.random() > 0.5 ? 60 : -60), size: 30, spd: -2.5 * s, spdY: 0, color: "#fff", damage: Math.floor(dmg * 1.5), bouncesLeft: 2 };
                if (shouldShrink) shrinkAttack(atk82);
                attacks.push(atk82);
            }
            break;
        case 9:
            for (var i = 0; i < (isEarly ? 1 : (bw >= 500 ? 2 : 1)); i++) {
                var bx = 50 + Math.random() * 300; var by = 50 + Math.random() * 400;
                var atk = { type: "bomb", x: bx, y: by, timer: 60, maxRadius: isEarly ? 75 : (80 + Math.random() * 40), hit: false, damage: Math.floor(dmg * 2), bouncesLeft: 0, particlesSpawned: false, shakeTime: 0 };
...
function loseArena() {
    if (typeof _superState !== 'undefined' && _superState.markResurrectCharges > 0) {
        ...
        if (mainCard && mainCard.name === "Император Марк") {
            ...
            _superState.markDmgBonus = 1.5;
            _superState.markSpeedBonus = 2;
            heartSpeed *= 2;
...
function drawActiveBuffs() {
    ...
    if (_superState.borosHeal && _superState.borosHeal.active) {
        var remain = (_superState.borosHeal.totalDuration - _superState.borosHeal.elapsed).toFixed(1);
...
function renderArena() {
    ...
    if (arenaPhase === "dodge") {
        ...
        if (arenaAllowedTypes.includes(10) && !heartWasMoving) {
            var bw = arenaCurrentWave;
            var ct = bw >= 700 ? 70 : (bw >= 350 ? 90 : 110);
            if (heartStandingTime > ct) { spawnBlaster(bw); heartStandingTime = 0; }
        }
    }
    
    if (arenaKarma > 0) {
        var drain = Math.max(0.15, arenaKarma * 0.05);
    ...
    var sx = (arenaShake ? (Math.random() - 0.5) * arenaShake : 0) + superShakeX;
    var sy = (arenaShake ? (Math.random() - 0.5) * arenaShake : 0) + superShakeY;
    ...
    if (arenaVignette > 0) {
        ctx.save();
        var vignetteGrad = ctx.createRadialGradient(200, 250, 150, 200, 250, 350);
    ...
    for (var y = 0; y <= 500; y += step) { ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(400, y); ctx.stroke(); }
    ctx.restore();
    ...
    ctx.save();
    for (var bpi = 0; bpi < arenaBgParticles.length; bpi++) {
        var bp = arenaBgParticles[bpi];
    ...
    if (arenaAttackType === 4 && arenaPhase === "dodge") {
        ctx.save();
        var spikeGlow = Math.sin(now / 200) * 0.3 + 0.7;
        ctx.fillStyle = "#ff3333";
        ctx.shadowColor = "#ff0000";
        ctx.shadowBlur = 10 * spikeGlow;
        for (var x = 0; x < 400; x += 15) { ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x + 7, 18); ctx.lineTo(x + 15, 0); ctx.fill(); }
        for (var x = 0; x < 400; x += 15) { ctx.beginPath(); ctx.moveTo(x, 500); ctx.lineTo(x + 7, 482); ctx.lineTo(x + 15, 500); ctx.fill(); }
        for (var y = 0; y < 500; y += 15) { ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(18, y + 7); ctx.lineTo(0, y + 15); ctx.fill(); }
        for (var y = 0; y < 500; y += 15) { ctx.beginPath(); ctx.moveTo(400, y); ctx.lineTo(382, y + 7); ctx.lineTo(400, y + 15); ctx.fill(); }
        ctx.restore();
    }
    
    ctx.fillStyle = "rgba(10,10,20,0.9)"; ctx.fillRect(8, 6, 384, 22);
    
    var totalWidth = 372 * Math.max(0, arenaHP / arenaMaxHP);
    var yellowWidth = 372 * Math.max(0, arenaKarma / arenaMaxHP);
    if (yellowWidth > totalWidth) yellowWidth = totalWidth;
    var greenWidth = totalWidth - yellowWidth;
    ...
    if (yellowWidth > 0) {
        var karmaGlow = Math.sin(now / 120) * 0.2 + 0.8;
    ...
    var bossGlow = Math.sin(now / 250) * 0.15 + 0.85;
    ...
    if (arenaComboTimer > 0 && arenaComboText) {
        ctx.save();
        var comboAlpha = Math.min(1, arenaComboTimer / 20);
    ...
    if (wallGapIndicator && wallGapIndicator.life > 0) {
        ctx.save();
        var alpha = wallGapIndicator.life / 35;
    ...
    // Трейл с проверкой на положительный радиус
    ctx.save(); ctx.globalCompositeOperation = 'lighter';
    for (var i = arenaTrail.length - 1; i >= 0; i--) {
        var t = arenaTrail[i]; t.life--; var ratio = Math.max(0, t.life / t.maxLife);
        var trailRadius = Math.max(0.1, t.size * ratio);
        ctx.fillStyle = t.color; ctx.globalAlpha = ratio * 0.7; ctx.beginPath(); ctx.arc(t.x, t.y - 2, trailRadius, 0, Math.PI * 2); ctx.fill();
        if (t.life <= 0) arenaTrail.splice(i, 1);
    }
    ctx.restore();

    if (arenaPhase === "dodge") {
        var frozen = (typeof _superState !== 'undefined' && _superState.antispiralFrozen);
        var speedMod = arenaGlobalSpeedMod * (typeof _superState !== 'undefined' && _superState.antispiralSpeedBoost ? 1.5 : 1.0);
        var timeStopped = (typeof _superState !== 'undefined' && _superState.garouTimeStop);
        ...
        for (var i = attacks.length - 1; i >= 0; i--) {
            ...
            if (a.type === "bomb") {
                ...
                if (a.timer > 0) {
                    ctx.save(); var isFlashing = Math.floor(a.timer / 6) % 2 === 0; var bombX = a.x; var bombY = a.y;
                    var bombRadius = Math.max(1, 11 + Math.abs(Math.sin(a.timer * 0.45)) * 4);
                    ctx.shadowColor = "#ff3333"; ctx.shadowBlur = isFlashing ? 25 : 10;
                    var gradient = ctx.createRadialGradient(bombX, bombY, Math.max(0.1, bombRadius * 0.2), bombX, bombY, bombRadius);
                    ...
                    ctx.beginPath(); ctx.arc(bombX + 3, bombY - bombRadius - 20, Math.max(1, isFlashing ? 5 : 4), 0, Math.PI * 2); ctx.fill();
                    ctx.restore();
                } else if (a.timer > -35) {
                    var progress = Math.abs(a.timer) / 35; var cr = Math.max(1, a.maxRadius * progress);
                    ...
                    ctx.save(); ctx.globalCompositeOperation = 'lighter';
                    var grad = ctx.createRadialGradient(a.x, a.y, Math.max(0.1, cr*0.1), a.x, a.y, cr);
                    ...
                    ctx.strokeStyle = "rgba(255,255,255,0.7)"; ctx.lineWidth = Math.max(0.1, 3*(1-progress));
                    for (var r = 0; r < 12; r++) { var angle = (r/12)*Math.PI*2; ctx.beginPath(); ctx.moveTo(a.x, a.y); ctx.lineTo(a.x+Math.cos(angle)*cr*0.8, a.y+Math.sin(angle)*cr*0.8); ctx.stroke(); }
                    ctx.fillStyle = "#ffffff"; for (var ep = 0; ep < 50; ep++) { var eAngle = Math.random()*Math.PI*2; ctx.beginPath(); ctx.arc(a.x+Math.cos(eAngle)*Math.random()*cr, a.y+Math.sin(eAngle)*Math.random()*cr, Math.max(0.1, 1+Math.random()*3), 0, Math.PI*2); ctx.fill(); }
                    ctx.restore();
                    if (!a.hit && invulnTimer <= 0) { var dx = heart.x-a.x, dy = heart.y-a.y; if (Math.sqrt(dx*dx+dy*dy) < (cr+heart.hitbox)) { applyHit(a.damage||30, "ВЗРЫВ!"); a.hit = true; } }
                } else { attacks.splice(i, 1); }
                continue;
            }
            
            if (a.type !== "wall" && a.bouncesLeft !== undefined && a.bouncesLeft > 0 && !frozen && !timeStopped) {
                var sz = a.size || (a.radius ? a.radius*2 : 20);
            ...
            var hit = false;
            if (a.type === "circle" || a.type === "rainbow") { var dx = heart.x-a.x, dy = heart.y-a.y; var radToCheck = Math.max(0.1, a.radius - 2); hit = Math.sqrt(dx*dx+dy*dy) < (heart.hitbox + radToCheck); }
            ...
            if (frozen || timeStopped) {
                ...
                ctx.lineWidth = 2;
                var sz = a.size || a.radius || 20;
                if (a.type === "circle" || a.type === "rainbow") {
                    var circR = Math.max(0.1, (a.radius || 10) + 3);
                    ctx.beginPath(); ctx.arc(a.x, a.y, circR, 0, Math.PI*2); ctx.fill(); ctx.stroke();
                    ...
                    var sz2 = Math.max(1, sz);
                    ctx.fillRect(a.x - 2, a.y - 2, sz2 + 4, sz2 + 4);
                    ctx.strokeRect(a.x - 2, a.y - 2, sz2 + 4, sz2 + 4);
                }
                ctx.restore();
            }
            
            // Отрисовка атак с проверкой радиусов
            ctx.save(); var col = a.color; if (a.type === "rainbow") col = "hsl(" + ((now/2+a.x)%360) + ",100%,60%)"; ctx.fillStyle = col;
            if (a.type === "square" || a.type === "danger") {
                var sz = a.size||20; ctx.translate(a.x+sz/2, a.y+sz/2);
                ...
                var circleR = Math.max(0.1, a.radius || 10);
                ctx.shadowColor = "#44ff44"; ctx.shadowBlur = 12;
                ctx.beginPath(); ctx.arc(a.x, a.y, circleR, 0, Math.PI*2); ctx.fill(); 
                ctx.strokeStyle="rgba(0,0,0,0.2)"; ctx.lineWidth=2; ctx.beginPath(); ctx.arc(a.x, a.y, Math.max(0.1, circleR*0.7), 0, Math.PI*2); ctx.stroke(); 
                ctx.fillStyle="#fff"; ctx.beginPath(); ctx.arc(a.x, a.y, Math.max(0.1, circleR*0.4), 0, Math.PI*2); ctx.fill(); 
            }
            else if (a.type === "rainbow") { 
                var rainbowR = Math.max(0.1, a.radius || 10);
                ctx.beginPath(); ctx.arc(a.x, a.y, rainbowR, 0, Math.PI*2); ctx.fill(); 
                ctx.strokeStyle="rgba(0,0,0,0.2)"; ctx.lineWidth=2; ctx.beginPath(); ctx.arc(a.x, a.y, Math.max(0.1, rainbowR*0.7), 0, Math.PI*2); ctx.stroke(); 
                ctx.fillStyle="#fff"; ctx.beginPath(); ctx.arc(a.x, a.y, Math.max(0.1, rainbowR*0.4), 0, Math.PI*2); ctx.fill(); 
            }
            ctx.restore();
        }
        ...
        // Бластеры
        for (var i = arenaBlasters.length-1; i >= 0; i--) {
            var b = arenaBlasters[i], ac = b.color === "rainbow" ? "hsl(" + ((now/2)%360) + ",100%,60%)" : b.color;
            if (!frozen && !timeStopped) {
            ...
            if (b.state === "aiming") { if (b.timer > 30) b.angle = Math.atan2(heart.y-b.y, heart.x-b.x); ctx.save(); ctx.globalAlpha = 0.2+Math.abs(Math.sin(b.timer*0.25))*0.2; ctx.strokeStyle = ac; ctx.lineWidth = 2; ctx.setLineDash([6,4]); ctx.lineDashOffset = -now / 40; ctx.beginPath(); ctx.moveTo(b.x,b.y); ctx.lineTo(b.x+Math.cos(b.angle)*800, b.y+Math.sin(b.angle)*800); ctx.stroke(); ctx.setLineDash([]); ctx.restore(); }
                ...
                var bWidth = Math.max(1, b.width + 16 + Math.random() * 10);
                ctx.strokeStyle = ac; ctx.lineWidth = bWidth; ctx.globalAlpha = 0.3; ctx.beginPath(); ctx.moveTo(b.x,b.y); ctx.lineTo(b.x+Math.cos(b.angle)*800, b.y+Math.sin(b.angle)*800); ctx.stroke(); 
                ctx.lineWidth = Math.max(1, b.width); ctx.globalAlpha = 0.9; ctx.stroke(); 
                ctx.strokeStyle = "#fff"; ctx.lineWidth = Math.max(0.5, b.width*0.4); ctx.globalAlpha = 1.0; ctx.stroke(); ctx.restore(); 
                if (!b.hasHit && invulnTimer <= 0 && !frozen && !timeStopped) { 
                    var dx = heart.x-b.x, dy = heart.y-b.y, dist = Math.abs(dx*Math.sin(b.angle)-dy*Math.cos(b.angle)); 
                    if (dist < Math.max(1, b.width/2) + heart.hitbox) { 
                        var bdmg=0, msg=""; 
        ...
        if (invulnTimer <= 0 || Math.floor(now/80)%2===0) { 
            ...
            var heartPulse = 1.0+Math.abs(Math.sin(now/140))*0.1; 
            ...
            if (invulnTimer>0) { 
                var shieldGrad = ctx.createRadialGradient(0, 2, Math.max(0.1, heart.size*1.1), 0, 2, Math.max(0.1, heart.size*1.4));
                shieldGrad.addColorStop(0, 'rgba(0,191,255,0.1)');
                shieldGrad.addColorStop(1, 'rgba(0,191,255,0.6)');
                ctx.strokeStyle = shieldGrad; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(0, 2, Math.max(0.1, heart.size*1.3), 0, Math.PI*2); ctx.stroke(); 
            } 
            ctx.restore();
        }
    } else if (arenaPhase === "attack") {
        for (var i = 0; i < arenaClickTargets.length; i++) { 
            var t = arenaClickTargets[i]; if (t.hit) continue; 
            t.pulse += 0.12; var r = Math.max(0.1, t.radius + Math.sin(t.pulse)*3); 
            ctx.save(); 
            var glowPulse = Math.sin(now / 300 + i) * 0.3 + 0.7;
            var grad = ctx.createRadialGradient(t.x, t.y, 2, t.x, t.y, r); 
            ...
            ctx.beginPath(); ctx.arc(t.x, t.y, Math.max(0.1, t.radius), 0, Math.PI*2); ctx.stroke(); ctx.restore(); 
        }
    }
    ...
    for (var i = arenaParticles.length-1; i >= 0; i--) { 
        var p = arenaParticles[i]; 
        p.x+=p.vx; p.y+=p.vy; p.vx*=0.95; p.vy*=0.95; p.life--; 
        var ratio = Math.max(0, p.life / p.maxLife);
        if (!p.isLightning) {
            var pSize = Math.max(0.1, p.size * ratio);
            ctx.fillStyle = p.color; ctx.globalAlpha = Math.max(0, ratio); ctx.beginPath(); ctx.arc(p.x, p.y, pSize, 0, Math.PI*2); ctx.fill(); 
        }
    ...
    // Ударные волны
    ctx.save(); for (var i = arenaShockwaves.length-1; i >= 0; i--) { 
        var sw = arenaShockwaves[i]; sw.r += sw.v; sw.life--; 
        var ratio = Math.max(0, sw.life / sw.maxLife);
        var swR = Math.max(0.1, sw.r);
        ctx.strokeStyle = sw.color; ctx.lineWidth = Math.max(0.1, 2.5*ratio); ctx.globalAlpha = ratio; 
        ctx.beginPath(); ctx.arc(sw.x, sw.y, swR, 0, Math.PI*2); ctx.stroke(); 
        if (sw.life <= 0) arenaShockwaves.splice(i,1); 
    } ctx.restore();
    
    // Тексты
    ctx.save(); for (var i = floatingTexts.length-1; i >= 0; i--) { 
        var ft = floatingTexts[i]; ft.y += ft.vy; ft.x += (ft.vx || 0); ft.life--; 
        ctx.fillStyle = ft.color; ctx.globalAlpha = Math.max(0, ft.life/50); 
        ctx.font = "bold 14px monospace"; ctx.shadowColor = ft.color; ctx.shadowBlur = 4; 
