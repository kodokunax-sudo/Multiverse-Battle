...
function loadGameData(d) { 
    ...
    // ★ ЗАГРУЗКА МАСТЕРСТВА И ОПЫТА КАРТ ★
    ...
    if (d.dailyRewards) {
        ...
        dailyRewards.claimedToday = d.dailyRewards.claimedToday || false;
    ...
    if (d.passData) {
        passData.currentTier = d.passData.currentTier || 1;
...
function normalizeMainCard() { if (team.length === 0) { mainCardIndex = -1; } else { if (mainCardIndex < 0 || mainCardIndex >= team.length) { mainCardIndex = 0; } } }
...
function createCard(r) { 
    ...
    if (templates.length > 0) { 
        let validTemplates = templates.filter(t => (t.minRebirth || minRebirth) <= rebirthCount); 
    ...
    if (!template) { 
        ...
        return { id: Date.now() + Math.random() * 10000, name: "???", rarity: r, damage: s.damage, hp: s.hp, sellPrice: s.sellPrice, speed: s.speed || 0.5, ability: null, universe: "?", unsellable: false, minRebirth: 0, statusAbility: null, extraStatus: null, mastery: 1, masteryExp: 0 }; 
    } 
    let s = cardStats[r] || { damage: 5, hp: 10, sellPrice: 10, speed: 0.5 }; 
...
function increaseFatigue(clickSpeedMultiplier = 1) { if (activeBuffs["fatigueImmune"] && activeBuffs["fatigueImmune"] > Date.now()) return; let resist = upgrades.fatigueResist.level * upgrades.fatigueResist.increment; if (hasSukunaFingers && sukunaTarget && sukunaExpireTime > Date.now()) resist *= 4; team.forEach(idx => { let cd = myCards[idx]; if (cd?.ability?.type === 'fatigueResist' && (!hasMasteryAbility || hasMasteryAbility(cd))) resist += cd.ability.value; }); let baseIncrease = 2 * clickSpeedMultiplier; let increase = Math.max(0.1, baseIncrease - resist); let oldFatigue = fatigue; fatigue = Math.min(100, fatigue + increase); updateFatigue(); updateRestBtn(); checkAutoRest(); if (fatigue <= 0 && oldFatigue > 0) updateChallengeProgress("fatigueZero", 1); }
...
function checkAchievements() { if (totalWins >= 10 && !achievements.win10) { achievements.win10 = true; points += Math.floor(100 * getStarMult()); } if (totalWins >= 50 && !achievements.win50) { achievements.win50 = true; points += Math.floor(500 * getStarMult()); } if (totalWins >= 100 && !achievements.win100) { achievements.win100 = true; points += Math.floor(1000 * getStarMult()); } if (totalWins >= 500 && !achievements.win500) { achievements.win500 = true; points += Math.floor(5000 * getStarMult()); } if (totalWins >= 1000 && !achievements.win1000) { achievements.win1000 = true; points += Math.floor(10000 * getStarMult()); } if (rebirthCount >= 3 && !achievements.rebirth3) { achievements.rebirth3 = true; points += Math.floor(3000 * getStarMult()); } if (rebirthCount >= 5 && !achievements.rebirth5) { achievements.rebirth5 = true; points += Math.floor(5000 * getStarMult()); } if (maxPoints >= 10000 && !achievements.points10k) { achievements.points10k = true; points += Math.floor(1000 * getStarMult()); } if (maxPoints >= 50000 && !achievements.points50k) { achievements.points50k = true; points += Math.floor(3000 * getStarMult()); } if (maxPoints >= 100000 && !achievements.points100k) { achievements.points100k = true; points += Math.floor(5000 * getStarMult()); } if (totalClicks >= 1000 && !achievements.totalClicks1k) { achievements.totalClicks1k = true; points += Math.floor(500 * getStarMult()); } if (totalClicks >= 10000 && !achievements.totalClicks10k) { achievements.totalClicks10k = true; points += Math.floor(3000 * getStarMult()); } let al = team.length === 6 && team.every(i => myCards[i]?.rarity === "Легендарная"); if (al && !achievements.legendaryTeam) { achievements.legendaryTeam = true; points += Math.floor(1000 * getStarMult()); } let as = team.length === 6 && team.every(i => myCards[i]?.rarity === "Секретная"); if (as && !achievements.secretTeam) { achievements.secretTeam = true; points += Math.floor(5000 * getStarMult()); } let allCommonCheck = team.length === 6 && team.every(i => myCards[i]?.rarity === "Обычная"); if (allCommonCheck && !achievements.allCommon) { achievements.allCommon = true; points += Math.floor(100 * getStarMult()); } let allRareCheck = team.length === 6 && team.every(i => myCards[i]?.rarity === "Редкая"); if (allRareCheck && !achievements.allRare) { achievements.allRare = true; points += Math.floor(200 * getStarMult()); } saveAll(); renderAchievements(); }
...
// ========== СТАТУС-ЭФФЕКТЫ (ТОЛЬКО С УР. 3+) ==========
function getStatusEffects() { 
    let bleed = 1.0, freeze = 0, shock = 0, blind = 0, fireDmg = 0, fireDur = 0, poisonDmg = 0; 
    let dupes = countTeamDuplicates(); 
    let dupMult = {}; 
    for (let name in dupes) { if (dupes[name] >= 3) dupMult[name] = Math.floor(dupes[name] / 2); else dupMult[name] = 1; } 
    team.forEach(idx => { 
        let cd = myCards[idx]; 
        if (!cd) return; 
        let mult = dupMult[cd.name] || 1; 
        let statuses = []; 
        if (typeof hasMasteryStatus === 'function' && !hasMasteryStatus(cd)) statuses = [];
        else statuses = [cd?.statusAbility, cd?.ability?.type === 'sevenSpecial' ? { type: 'blind', value: 2 } : null, ...(cd?.extraStatus || [])].filter(Boolean);
        statuses.forEach(sa => { 
            if (sa.type === 'bleed') bleed += sa.value * mult; 
            if (sa.type === 'freezeStacks') freeze += sa.value * mult; 
            if (sa.type === 'shock') shock += sa.chance * mult; 
            if (sa.type === 'blind') blind += sa.value * mult; 
            if (sa.type === 'fire') { fireDmg += sa.damage * mult; fireDur = Math.max(fireDur, sa.duration); } 
            if (sa.type === 'poison') poisonDmg += sa.damage * mult; 
        }); 
    }); 
    return { bleed, freeze, shock, blind, fireDmg, fireDur, poisonDmg }; 
}
...
// ========== ПАССИВНЫЕ МОДИФИКАТОРЫ (СПОСОБНОСТИ С УР. 4+) ==========
function getPassiveModifiers() { 
    let dm = 1.0, tm = 1.0, bb = 0, hm = 1.0, sb = 0; 
    let ab = 1 + abilityUpgradeLevel * upgrades.abilityPower.increment; 
    let dupes = countTeamDuplicates(); 
    let dupMult = {}; 
    for (let name in dupes) { if (dupes[name] >= 3) dupMult[name] = Math.floor(dupes[name] / 2); else dupMult[name] = 1; } 
    team.forEach(idx => { 
        ...
        let mult = dupMult[cd.name] || 1; 
        let a = cd.ability; 
        if (a && typeof hasMasteryAbility === 'function' && !hasMasteryAbility(cd)) a = null;
        if (a) { 
            if (a.type === 'damageAura') dm += a.value * ab * mult; 
            if (a.type === 'bossDamage') bb += a.value * ab * mult; 
            if (a.type === 'damageReduction') tm -= a.value * ab * mult; 
            if (a.type === 'hpBuff') hm += a.value * ab * mult; 
            if (a.type === 'bossSupport' && currentEnemy?.isBoss) { if (!bossSupportUsedThisFight) { fatigue = Math.max(0, fatigue - 50); bossSupportUsedThisFight = true; } bb += 0.15 * ab * mult; } 
            if (a.type === 'bossDoubleSelf' && currentEnemy?.isBoss) dm += 1.0 * ab * mult; 
            if (a.type === 'bossDouble' && currentEnemy?.isBoss) dm += 1.0 * ab * mult; 
            if (a.type === 'sevenSpecial') { tm -= (a.damageReduction || 0.10) * ab * mult; } 
            if (a.type === 'bossDamage' && a.bossReduction && currentEnemy?.isBoss) tm -= a.bossReduction * ab * mult; 
            if (a.type === 'bossDamage' && a.damageReduction && !currentEnemy?.isBoss) tm -= a.damageReduction * ab * mult; 
            if (a.type === 'damageAura' && a.damageTakenMod) tm += a.damageTakenMod * ab * mult; 
...
// ========== ОФЛАЙН-ПРОГРЕСС ==========
function processOfflineProgress() { if (!afkTeam.length) return; let now = Date.now(); let elapsed = Math.floor((now - lastSaveTime) / 1000); if (elapsed < 10) return; let maxElapsed = Math.min(elapsed, 7200); let wavesCompleted = Math.floor(maxElapsed / 2); if (wavesCompleted <= 0) return; let simWave = afkCurrentWave || wave; let simHp = playerHp; let simDmg = (5 + upgrades.damage.level * upgrades.damage.increment + (window.afkTeamDamage || 0)) * 0.8; let simMaxHp = window.playerMaxHp || 100; let earnedPoints = 0; let earnedCards = 0; for (let w = 0; w < wavesCompleted; w++) { let ehp = 50 + simWave * 12; let edmg = 15 + simWave * 6; if (simWave % 10 === 0) { ehp *= 4; edmg *= 3; } while (ehp > 0 && simHp > 0) { ehp -= simDmg; if (ehp <= 0) break; if (Math.random() < 0.33) { simHp -= Math.floor(edmg * 0.9); } } if (simHp <= 0) { simHp = Math.floor(simMaxHp * 0.8); simWave = Math.max(1, simWave - 5); continue; } let rew = simWave % 10 === 0 ? Math.floor(simWave / 2 * getStarMult()) : Math.floor(simWave / 3 * getStarMult()); earnedPoints += rew; simWave++; afkWinsCompleted = (afkWinsCompleted || 0) + 1; simHp = Math.min(simMaxHp, simHp + Math.floor(simMaxHp * 0.2)); if (simWave % 10 === 0 && Math.random() < 0.3) { let rarity = getBossRewardRarity(simWave); if (rarity !== "Босс") { let c = createCard(rarity); if (c) { myCards.push(c); earnedCards++; } } } } if (earnedPoints > 0) { points += earnedPoints; if (points > maxPoints) maxPoints = points; afkWavesCompleted += wavesCompleted; afkCurrentWave = simWave; let mins = Math.floor(elapsed / 60); showFloatingText("💤 АФК: +" + earnedPoints + "⭐ за " + mins + "мин!", "#2ecc71"); updateChallengeProgress("earnPoints", earnedPoints); updateChallengeProgress("wins", wavesCompleted); saveAll(); } lastSaveTime = now; }

// ========== ПАЛЬЦЫ СУКУНЫ ==========
function showSukunaModal() { if (rebirthCount < 4 || (mode !== "moder" && points < 15000)) return; if (!hasSukunaFingers && mode !== "moder") return; let html = '<h2>🗿 Выберите героя для Пальцев Сукуны</h2><div style="max-height:300px;overflow-y:auto;">'; if (!team.length) { html += '<p style="color:#888;">Нет героев в команде.</p>'; } else { team.forEach((idx, s) => { let cd = myCards[idx]; if (!cd) return; let isTarget = (sukunaTarget === cd.name && sukunaExpireTime > Date.now()); html += '<div class="team-select-item ' + (isTarget ? 'disabled' : '') + '" onclick="' + (isTarget ? '' : 'applySukuna(\'' + cd.name.replace(/'/g, "\\'") + '\')') + '"><span>' + escapeHtml(cd.name) + '</span><span>' + (isTarget ? '✅ Активен' : '▶ Выбрать') + '</span></div>'; }); } html += '</div><button class="btn" style="width:100%;margin-top:10px;background:#e74c3c;" onclick="closeModal()">Отмена</button>'; let el = document.getElementById("modalContent"); if (el) el.innerHTML = html; el = document.getElementById("modalOverlay"); if (el) el.style.display = "flex"; }
...
// ========== ГАЧА ==========
const gachaPrices = { common: 200, rare: 400, superRare: 800, epic: 1600, mythic: 3200, legendary: 8000, secret: 15000 };
...
function rollGachaRarity(type) { let luckBonus = upgrades.luck.level * upgrades.luck.increment; let roll = Math.random() * 100; switch(type) { case "common": if (roll < 1 + luckBonus) return "Мифическая"; if (roll < 3 + luckBonus) return "Эпик"; if (roll < 11 + luckBonus) return "Сверх редкая"; if (roll < 31 + luckBonus) return "Редкая"; return "Обычная"; case "rare": if (roll < 3 + luckBonus) return "Мифическая"; if (roll < 10 + luckBonus) return "Эпик"; if (roll < 30 + luckBonus) return "Сверх редкая"; if (roll < 90) return "Редкая"; return "Обычная"; case "superRare": if (roll < 2 + luckBonus) return "Легендарная"; if (roll < 7 + luckBonus) return "Мифическая"; if (roll < 25 + luckBonus) return "Эпик"; if (roll < 80) return "Сверх редкая"; return "Редкая"; case "epic": if (roll < 0.2 + luckBonus * 0.1) return "Секретная"; if (roll < 8 + luckBonus) return "Легендарная"; if (roll < 30 + luckBonus) return "Мифическая"; if (roll < 80) return "Эпик"; return "Сверх редкая"; case "mythic": if (roll < 0.8 + luckBonus * 0.2) return "Секретная"; if (roll < 30 + luckBonus) return "Легендарная"; if (roll < 80) return "Мифическая"; return "Эпик"; case "legendary": if (roll < 2 + luckBonus) return "Секретная"; if (roll < 80) return "Легендарная"; return "Мифическая"; case "secret": if (roll < 20 + luckBonus * 2) return "Секретная"; return "Легендарная"; default: return "Обычная"; } }
...
function generateEnemy() { firstAttackThisFight = true; bossSupportUsedThisFight = false; if (hpDecayInterval) { clearInterval(hpDecayInterval); hpDecayInterval = null; } if (fireInterval) { clearInterval(fireInterval); fireInterval = null; } let el = document.getElementById("spareBtn"); if (el) el.style.display = "none"; el = document.getElementById("dialogBox"); if (el) el.style.display = "none"; currentDialog = null; let world = getCurrentWorld(); let isBoss = wave % 10 === 0; let isUniqueBoss = (typeof bossTemplates !== 'undefined' && bossTemplates[wave] !== undefined); let hp, dmg, name, dialogue = "", enemyStat = null; if (isUniqueBoss) { let bt = bossTemplates[wave]; hp = Math.floor((50 + wave * 12) * bt.hpMult); dmg = Math.floor((15 + wave * 6) * bt.dmgMult / 2); name = bt.name; dialogue = bt.dialogue || ""; if (bt.enemyStatus) enemyStat = bt.enemyStatus; showBossDialogue(dialogue); sfxBossAppear(); if (wave === 10000) currentDialog = finalBossResponses; } else if (isBoss) { hp = Math.floor((50 + wave * 12) * 2); dmg = Math.floor((15 + wave * 6) * 1); name = "👑 БОСС"; hideBossDialogue(); } else { hp = 50 + wave * 12; dmg = 15 + wave * 6; name = enemyNames[Math.floor(Math.random() * enemyNames.length)]; let randomStat = enemyStatusPool[Math.floor(Math.random() * enemyStatusPool.length)]; if (randomStat) enemyStat = randomStat; hideBossDialogue(); } enemyStatuses = { fireTicks:0, fireDamage:0, poisonDamage:0, bleedMult:1.0, freezeStacks:0, shockChance:0, blindStacks:0 }; if (enemyStat) { if (enemyStat.type === "freezeStacks") enemyStatuses.freezeStacks = enemyStat.value; if (enemyStat.type === "bleed") enemyStatuses.bleedMult = 1 + enemyStat.value; if (enemyStat.type === "shock") enemyStatuses.shockChance = enemyStat.chance; } applyStatusEffects(); currentEnemy = { name, hp, maxHp:hp, damage:dmg, isBoss:isBoss||isUniqueBoss }; let arenaWaves = [50, 100, 150, 200, 250, 300, 350, 400, 450, 500, 550, 600, 650, 700, 750, 800, 850, 900, 950, 1000, 1100, 1200, 1300, 1400, 1500, 1600, 1700, 1800, 1900, 2000, 2100, 2200, 2300, 2400, 2500, 2600, 2700, 2800, 2900, 3000, 3200, 3400, 3600, 3800, 4000, 4200, 4400, 4600, 4800, 5000, 10000]; let showArenaBtn = (isBoss || isUniqueBoss) && wave >= 50 && arenaWaves.includes(wave);
    ...
    if (wave === 200 && isUniqueBoss) {
        ...
        if (livingBtn) livingBtn.style.display = "block";
    } else {
...
function spareBoss() { if (!currentEnemy || !currentEnemy.isBoss || currentEnemy.hp <= 0) return; let isUniqueBoss = (typeof bossTemplates !== 'undefined' && bossTemplates[wave] !== undefined); if (!isUniqueBoss) { alert("⚠️ Обычного босса нельзя пощадить!"); return; } if (currentEnemy.hp > currentEnemy.maxHp * 0.3) { alert("⚠️ Здоровье босса должно быть ниже 30% чтобы пощадить!"); return; } let spareChance = 0.30 + spareBonusFromTeam; if (Math.random() < spareChance) { let bt = bossTemplates[wave]; if (bt && bt.spareReward) { let template = customCardTemplates["Босс"].find(t => t.name === bt.spareReward); if (template) { let rewardCard = createCardFromTemplate(template, "Босс"); myCards.push(rewardCard); sfxCardObtain(); alert("🎉 Босс присоединился! Получена карта: " + bt.spareReward + " (шанс был " + Math.floor(spareChance*100) + "%)"); saveAll(); } } currentEnemy.hp = 0; victory(); } else { let extraDmg = Math.floor(currentEnemy.damage * 3); playerHp -= extraDmg; alert("💢 Босс отказался! Он наносит тройной урон: " + extraDmg + "!"); let el = document.getElementById("spareBtn"); if (el) el.style.display = "none"; if (playerHp <= 0) defeat(); else { renderEnemy(); updatePlayerStats(); } } }
function skipArenaFight() { if (!currentEnemy || !currentEnemy.isBoss) return; let isUniqueBoss = (typeof bossTemplates !== 'undefined' && bossTemplates[wave] !== undefined); if (!isUniqueBoss) return; let alreadyDefeated = typeof defeatedBosses !== 'undefined' && Array.isArray(defeatedBosses) && defeatedBosses.includes(wave); if (!alreadyDefeated) return; currentEnemy.isBoss = false; currentEnemy.damage = Math.floor((15 + wave * 6) * 1); if (typeof defeatedBosses !== 'undefined' && Array.isArray(defeatedBosses)) { if (!defeatedBosses.includes(wave)) { defeatedBosses.push(wave); } } document.getElementById("startArenaBtn").style.display = "none"; let skipBtn = document.getElementById("skipArenaBtn"); if (skipBtn) skipBtn.style.display = "none"; let spareBtn = document.getElementById("spareBtn"); if (spareBtn) spareBtn.style.display = "none"; showFloatingText("⏭️ Арена пропущена!", "#ffaa00"); }
...
function handleClick() { initAudio(); if (typeof arenaActive !== 'undefined' && arenaActive) return; if (typeof livingStoneActive !== 'undefined' && livingStoneActive) return; let arenaBtn = document.getElementById("startArenaBtn"); if (arenaBtn && arenaBtn.style.display !== "none" && arenaBtn.style.display !== "") return; let livingBtn = document.getElementById("startLivingStoneBtn"); if (livingBtn && livingBtn.style.display !== "none" && livingBtn.style.display !== "") return; if (playerHp <= 0) { resetGame(); return; } if (!currentEnemy || currentEnemy.hp <= 0) return; if (deathNoteTarget && wave === deathNoteTarget && !skipUsed) { currentEnemy.hp = 0; skipUsed = true; deathNoteTarget = null; victory(); return; } totalClicks++; addPassExp(1); updateChallengeProgress("totalClicksGoal", 1); let now = Date.now(); let clickInterval = lastClickTime ? (now - lastClickTime) / 1000 : 999; lastClickTime = now; let fatigueMultiplier = 1; if (clickInterval < 0.1) { fatigueMultiplier = 3; } else if (clickInterval < 0.5) { comboCount++; } else { comboCount = 0; comboMultiplier = 1; } if (comboCount >= 50) comboMultiplier = 5; else if (comboCount >= 25) comboMultiplier = 3; else if (comboCount >= 10) comboMultiplier = 2; if (comboCount === 10) showFloatingText("⚡ КОМБО x2!", "#ffaa00"); if (comboCount === 25) showFloatingText("⚡ КОМБО x3!", "#ff8800"); if (comboCount === 50) showFloatingText("⚡ КОМБО x5!", "#ff4400"); let dmg = window.playerFinalDamage || 1; let m = getPassiveModifiers(); if (currentEnemy.isBoss) dmg = Math.floor(dmg * (1 + m.bossBonus)); let cc = upgrades.crit.level * upgrades.crit.increment; let oneShotChance = 0; team.forEach(idx => { let cd = myCards[idx]; if (cd?.ability?.type === 'oneShot' && (typeof hasMasteryAbility === 'function' ? hasMasteryAbility(cd) : true)) { oneShotChance += cd.ability.chance * (1 + abilityUpgradeLevel * 0.1); } }); team.forEach(idx => { let cd = myCards[idx]; if (cd?.ability?.type === 'critChance' && (typeof hasMasteryAbility === 'function' ? hasMasteryAbility(cd) : true)) cc += cd.ability.value * (1 + abilityUpgradeLevel * 0.1); if (cd?.ability?.type === 'damageMultChance' && Math.random() < cd.ability.chance && (typeof hasMasteryAbility === 'function' ? hasMasteryAbility(cd) : true)) dmg = Math.floor(dmg * cd.ability.mult); }); if (oneShotChance > 0 && Math.random() < oneShotChance) { dmg = currentEnemy.hp; if (team.some(idx => myCards[idx]?.name === "Сайтама") && team.some(idx => myCards[idx]?.name === "Космический Гароу")) { evoProgress.oneShotCount++; } sfxCrit(); showFloatingText("💀 ВАНШОТ!", "#ff0000"); } else { dmg = Math.floor(dmg * comboMultiplier); if (Math.random() < cc) { dmg = Math.floor(dmg * 2); sfxCrit(); showFloatingText("💥 КРИТ! x2", "#feca57"); } else { sfxClick(); showFloatingText("-" + dmg, "#fff"); } dmg = Math.floor(dmg * enemyStatuses.bleedMult); } if (enemyStatuses.fireTicks > 0 && enemyStatuses.fireDamage > 0) { startFireEffectPassive(enemyStatuses.fireDamage, enemyStatuses.fireTicks * 1000); enemyStatuses.fireTicks = 0; } currentEnemy.hp -= dmg; 
    // ★ ОПЫТ МАСТЕРСТВА ЗА УРОН ★
    if (typeof grantMasteryExpFromFight === 'function') grantMasteryExpFromFight(dmg, 0);
    updateChallengeProgress("bigDamage", dmg); team.forEach(idx => { let cd = myCards[idx]; if (cd?.ability?.type === 'clickDmgSelf' && (typeof hasMasteryAbility === 'function' ? hasMasteryAbility(cd) : true)) { let hpLoss = Math.max(1, Math.floor(playerHp * cd.ability.value)); playerHp -= hpLoss; showFloatingText("💔 -" + hpLoss + " HP", "#ff4444"); } if (cd?.statusAbility?.type === 'clickDmgSelf' && (typeof hasMasteryStatus === 'function' ? hasMasteryStatus(cd) : true)) { let hpLoss = Math.max(1, Math.floor(playerHp * cd.statusAbility.value)); playerHp -= hpLoss; showFloatingText("💔 -" + hpLoss + " HP", "#ff4444"); } }); if (playerHp <= 0) { defeat(); return; } if (currentEnemy.hp <= 0) { victory(); return; } clicksSinceLastCounter++; let maxClicks = Math.max(1, 3 - enemyStatuses.freezeStacks + enemyStatuses.blindStacks); if (clicksSinceLastCounter >= maxClicks) { 
        let takenDmg = Math.floor(currentEnemy.damage * m.takenMult);
        playerHp -= takenDmg; 
        // ★ ОПЫТ МАСТЕРСТВА ЗА ПОЛУЧЕННЫЙ УРОН ★
        if (typeof grantMasteryExpFromFight === 'function') grantMasteryExpFromFight(0, takenDmg);
...
function victory() { 
    ...
    if (isBoss) { let rarity = getBossRewardRarity(wave); if (rarity !== "Босс") { let c = createCard(rarity); if (c) myCards.push(c); } renderMyCards(); let hasZeno = team.some(idx => myCards[idx]?.ability?.type === 'zenoCheckpoint' && (typeof hasMasteryAbility === 'function' ? hasMasteryAbility(myCards[idx]) : true)); if (hasZeno && Math.random() < 0.10) { let nextCp = Math.floor(wave / 50) * 50 + 50; if (nextCp > highestCheckpoint) { highestCheckpoint = nextCp; saveAll(); } showFloatingText("🌀 ЗЕНО: чекпоинт " + nextCp + "!", "#9b59b6"); } sfxVictory(); } else { sfxVictory(); } 
    ...
    increaseFatigue(); 
    clicksSinceLastCounter = 0; 
    team.forEach(idx => { let cd = myCards[idx]; if (cd?.ability?.type === 'healOnWin' && (typeof hasMasteryAbility === 'function' ? hasMasteryAbility(cd) : true)) playerHp = Math.min(window.playerMaxHp, playerHp + window.playerMaxHp * cd.ability.percent); }); 
    checkAutoSell(); 
    generateEnemy(); 
    renderPoints(); 
    updatePlayerStats(); 
...
function defeat() { if (hpDecayInterval) { clearInterval(hpDecayInterval); hpDecayInterval = null; } if (fireInterval) { clearInterval(fireInterval); fireInterval = null; } if (!resurrectedThisFight) { for (let idx of team) { let cd = myCards[idx]; if (cd?.ability?.type === 'resurrect' && Math.random() < cd.ability.chance * (1 + abilityUpgradeLevel * 0.1) && (typeof hasMasteryAbility === 'function' ? hasMasteryAbility(cd) : true)) { playerHp = window.playerMaxHp || 100; resurrectedThisFight = true; sfxAbility(); showFloatingText("✨ Воскрешение!", "#2ecc71"); renderEnemy(); updatePlayerStats(); return; } } } let bonus = 0; team.forEach(idx => { let cd = myCards[idx]; if (cd?.ability?.type === 'deathBonus' && (typeof hasMasteryAbility === 'function' ? hasMasteryAbility(cd) : true)) bonus += cd.ability.value; }); if (bonus > 0) points += Math.floor(points * bonus); if (points > maxPoints) maxPoints = points; defeatHistory.unshift({ wave, hp: Math.floor(playerHp) }); if (defeatHistory.length > 10) defeatHistory.pop(); sfxDefeat(); let nearestCheckpoint = Math.floor(wave / 50) * 50; if (nearestCheckpoint > highestCheckpoint) { highestCheckpoint = nearestCheckpoint; saveAll(); } playerHp = window.playerMaxHp || 100; totalWins = 0; if (activeCheckpoint > 0 && activeCheckpoint <= highestCheckpoint) { wave = activeCheckpoint; clicksSinceLastCounter = 0; fatigue = Math.max(0, fatigue - 20); updateFatigue(); updateRestBtn(); resurrectedThisFight = false; generateEnemy(); saveAll(); renderEnemy(); renderDefeatHistory(); updatePlayerStats(); renderCheckpoints(); return; } if (highestCheckpoint > 1) { let useCp = confirm("💀 Вы погибли на волне " + wave + "!\n\nУ вас есть чекпоинт на волне " + highestCheckpoint + ".\n\nНачать с чекпоинта? (OK = Да, Отмена = с 1 волны)"); if (useCp) { activeCheckpoint = highestCheckpoint; totalWins = 0; wave = highestCheckpoint; clicksSinceLastCounter = 0; fatigue = Math.max(0, fatigue - 20); updateFatigue(); updateRestBtn(); resurrectedThisFight = false; generateEnemy(); saveAll(); renderEnemy(); renderDefeatHistory(); updatePlayerStats(); renderCheckpoints(); return; } } wave = 1; clicksSinceLastCounter = 0; generateEnemy(); fatigue = Math.max(0, fatigue - 20); updateFatigue(); updateRestBtn(); resurrectedThisFight = false; saveAll(); renderEnemy(); renderDefeatHistory(); updatePlayerStats(); }
...
function runAfkTick() { if (!afkActive) return; let dmg = (5 + upgrades.damage.level * upgrades.damage.increment + (window.afkTeamDamage || 0)) * 0.8; let ehp = 50 + afkCurrentWave * 12; let edmg = 15 + afkCurrentWave * 6; if (afkCurrentWave % 10 === 0) { ehp *= 4; edmg *= 3; } ehp -= dmg; 
    // ★ ОПЫТ МАСТЕРСТВА ЗА УРОН В АФК ★
    if (typeof grantMasteryExpFromFight === 'function') grantMasteryExpFromFight(dmg, 0);
    if (ehp <= 0) { let rew = afkCurrentWave % 10 === 0 ? Math.floor(afkCurrentWave / 2 * getStarMult()) : Math.floor(afkCurrentWave / 3 * getStarMult()); points += rew; if (points > maxPoints) maxPoints = points; totalWins++; addExp(afkCurrentWave % 10 === 0 ? 25 : 5); addPassExp(afkCurrentWave % 10 === 0 ? 25 : 5); updateChallengeProgress("earnPoints", rew); updateChallengeProgress("wins", 1); if (afkCurrentWave % 10 === 0) { updateChallengeProgress("bossKills", 1); let rarity = getBossRewardRarity(afkCurrentWave); if (rarity !== "Босс") { let c = createCard(rarity); if (c) myCards.push(c); } renderMyCards(); } afkCurrentWave++; afkWavesCompleted++; afkWinsCompleted = (afkWinsCompleted || 0) + 1; let el = document.getElementById("afkWave"); if (el) el.innerText = afkCurrentWave; el = document.getElementById("afkWavesCompleted"); if (el) el.innerText = afkWavesCompleted; playerHp = Math.min(window.playerMaxHp, playerHp + Math.floor((50 + upgrades.hp.level * upgrades.hp.increment + (window.afkTeamHpBonus || 0)) * 0.8 * 0.2)); increaseFatigue(); renderPoints(); updatePlayerStats(); checkAchievements(); checkAutoSell(); } else { if (Math.random() < 0.33) { let takenDmg = Math.floor(edmg * 0.9);
            playerHp -= takenDmg; 
            // ★ ОПЫТ МАСТЕРСТВА ЗА ПОЛУЧЕННЫЙ УРОН В АФК ★
            if (typeof grantMasteryExpFromFight === 'function') grantMasteryExpFromFight(0, takenDmg);
            if (playerHp <= 0) { afkCurrentWave = 1; playerHp = (50 + upgrades.hp.level * upgrades.hp.increment + (window.afkTeamHpBonus || 0)) * 0.8; fatigue = Math.max(0, fatigue - 20); updateFatigue(); updateRestBtn(); let el = document.getElementById("afkWave"); if (el) el.innerText = afkCurrentWave; } } } let el = document.getElementById("playerHp"); if (el) el.innerText = Math.floor(playerHp); if (afkActive) afkTimer = setTimeout(runAfkTick, 2000); saveAll(); }

// ========== ЧЕКПОИНТЫ ==========
function toggleCheckpoint(cp) { if (activeCheckpoint === cp) { activeCheckpoint = 0; saveAll(); renderCheckpoints(); return; } let action = confirm("🚩 Чекпоинт " + cp + " волна\n\nOK — перейти СЕЙЧАС\nОтмена — использовать при смерти"); if (action) { activeCheckpoint = cp; wave = cp; playerHp = window.playerMaxHp || 100; clicksSinceLastCounter = 0; fatigue = Math.max(0, fatigue - 30); updateFatigue(); updateRestBtn(); resurrectedThisFight = false; generateEnemy(); saveAll(); renderAll(); updatePlayerStats(); renderCheckpoints(); showFloatingText("🚩 Телепорт на волну " + cp + "!", "#ffdd00"); } else { activeCheckpoint = cp; saveAll(); renderCheckpoints(); } }
...
function renderPoints() { let displayPoints = (mode === "moder" && moderUnlocked) ? "∞" : points; ['pointsAmount', 'pointsAmount2', 'pointsAmount3', 'pointsAmountBulk', 'pointsAmountRest', 'pointsAmountGacha'].forEach(id => { let e = document.getElementById(id); if (e) e.innerText = displayPoints; }); }
