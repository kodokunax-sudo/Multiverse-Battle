...
function performGacha(type) {
    ...
    if (mode !== "moder") {
        ...
        if (gachaDailyLimits[type] >= gachaDailyMax[type]) {
            alert("Дневной лимит круток этого типа исчерпан!");
            return;
        }
        if (points < gachaPrices[type]) {
...
function victory() {
    ...
    if (isBoss && wave % 50 === 0) { 
        ...
        saveAll(); 
        renderCheckpoints(); 
        // Проверяем, не побеждали ли этого босса раньше в этой жизни
