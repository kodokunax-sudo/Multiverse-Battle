...
function drawLightningBolt(x1, y1, x2, y2, color, alpha) {
    if (!ctx) return;
    ctx.save(); ctx.strokeStyle = color; ctx.lineWidth = 1.5 * alpha; ctx.shadowColor = color; ctx.shadowBlur = 8 * alpha; ctx.globalAlpha = alpha;
    ctx.beginPath(); ctx.moveTo(x1, y1);
    let segments = 3;
    for (let i = 1; i < segments; i++) {
        let t = i / segments;
        ctx.lineTo(x1 + (x2 - x1) * t + (Math.random() - 0.5) * 18, y1 + (y2 - y1) * t + (Math.random() - 0.5) * 18);
    }
...
function drawFist(f) {
    ...
    let s = f.size;
    ctx.shadowColor = f.owner === "Гарп" ? "#4444ff" : "#ff0000"; ctx.shadowBlur = 30;
    ctx.fillStyle = f.owner === "Гарп" ? "#2222ff" : "#ff2222"; ctx.fillRect(-s, -s * 1.2, s * 2, s * 2.4);
    ctx.fillStyle = f.owner === "Гарп" ? "#5555ff" : "#ff5555"; ctx.fillRect(-s * 0.7, -s * 1.2, s * 0.4, s * 2.4); ctx.fillRect(s * 0.3, -s * 1.2, s * 0.4, s * 2.4);
    ctx.fillStyle = f.owner === "Гарп" ? "#0000aa" : "#aa0000"; ctx.fillRect(-s, s * 0.8, s * 2, s * 0.4);
    ctx.strokeStyle = "#ffffff"; ctx.lineWidth = 3; ctx.shadowBlur = 0; ctx.strokeRect(-s, -s * 1.2, s * 2, s * 2.4);
    ctx.fillStyle = "#ffffff"; ctx.font = "bold 14px monospace"; ctx.textAlign = "center"; ctx.textBaseline = "middle";
    ctx.shadowColor = "#000"; ctx.shadowBlur = 4; ctx.fillText(f.owner === "Гарп" ? "ГАРП" : "УДАР", 0, 0);
    ctx.restore();
}

function drawCircleMarker(x, y, color, alpha) {
    if (!ctx) return;
    ctx.save(); ctx.globalAlpha = alpha; ctx.strokeStyle = color; ctx.lineWidth = 3; ctx.shadowColor = color; ctx.shadowBlur = 15;
    ctx.setLineDash([5, 5]); ctx.beginPath(); ctx.arc(x, y, 20, 0, Math.PI * 2); ctx.stroke(); ctx.setLineDash([]);
    ctx.beginPath(); ctx.arc(x, y, 5, 0, Math.PI * 2); ctx.fillStyle = color; ctx.fill(); ctx.restore();
}
...
function drawGarouTrail() {
    ...
    ctx.save(); ctx.globalAlpha = 0.3; ctx.strokeStyle = "#ff8800"; ctx.lineWidth = 2; ctx.shadowColor = "#ff8800"; ctx.shadowBlur = 10;
    ctx.setLineDash([4, 8]); ctx.beginPath(); ctx.moveTo(_superState.positionHistory[0].x, _superState.positionHistory[0].y);
    for (let i = 1; i < _superState.positionHistory.length; i++) ctx.lineTo(_superState.positionHistory[i].x, _superState.positionHistory[i].y);
    ctx.stroke(); ctx.setLineDash([]); ctx.restore();
    ctx.save(); ctx.fillStyle = "#ff8800"; ctx.shadowColor = "#ff8800"; ctx.shadowBlur = 8;
    for (let i = 0; i < _superState.positionHistory.length; i += 5) {
        ...
        let alpha = 1 - age;
        if (alpha > 0) { ctx.globalAlpha = alpha * 0.6; ctx.beginPath(); ctx.arc(p.x, p.y, 3, 0, Math.PI * 2); ctx.fill(); }
    }
    ctx.restore();
}
...
function drawBeerBottle(x, y, alpha) {
    if (!ctx) return;
    ctx.save(); ctx.globalAlpha = alpha; ctx.translate(x, y - 25);
    ctx.fillStyle = "#8B4513"; ctx.fillRect(-8, -15, 16, 25);
    ctx.fillStyle = "#D2691E"; ctx.fillRect(-6, -20, 12, 8);
    ctx.fillStyle = "#FFD700"; ctx.fillRect(-4, -22, 8, 4);
    ctx.strokeStyle = "#fff"; ctx.lineWidth = 2; ctx.strokeRect(-8, -15, 16, 25);
    ctx.restore();
}
...
const superAbilities = {
    "Деку (100%)": {
        ...
        onActivate() { _superState.dekusActive = true; _superState.dekusOriginalSpeed = heartSpeed; _superState.dekusDmgMult = 2; _superState.dekusParticles = true; heartSpeed *= 3; },
    ...
    "Сайтама": {
        name: "ОБЫЧНЫЙ УДАР", cooldown: 20000, toggleable: false, duration: 0,
        onActivate() { let willOneshot = Math.random() < 0.01; _superState.fists.push({ x: heart.x, y: heart.y-20, vx: 0, vy: -7, size: 45, life: 80, color: "#ff2222", willOneshot, oneshotChecked: false, pathWidth: 90, owner: "Сайтама" }); if (typeof sfxWhoosh === 'function') sfxWhoosh(); },
        onTick() {}
    },
    "Борос": {
        name: "РЕГЕНЕРАЦИЯ", cooldown: 20000, toggleable: false, duration: 5000,
        onActivate() { _superState.borosHeal = { active: true, healPerSec: arenaMaxHP * 0.06, elapsed: 0, totalDuration: 5 }; _superState.borosParticles = true; heartSpeed *= 0.7; },
        onDeactivate() { if (_superState.borosHeal) { heartSpeed /= 0.7; _superState.borosHeal = null; _superState.borosParticles = false; } },
    ...
    "Космический Гароу": {
        ...
        onActivate() { let now = performance.now(); let target = null; for (let i = _superState.positionHistory.length-1; i >= 0; i--) { let p = _superState.positionHistory[i]; if (now - p.time >= 2000) { target = p; break; } } if (!target && _superState.positionHistory.length > 0) target = _superState.positionHistory[0]; if (target) { _superState.garouMarker = { x: target.x, y: target.y, alpha: 1.0, time: now }; heart.x = target.x; heart.y = target.y; _superState.garouInvulnTimer = 1.0; } _superState.positionHistory = []; },
        onTick(dt) {}
    },
    "Зено": {
        name: "СТИРАНИЕ", cooldown: 45000, toggleable: false, duration: 0,
        onActivate() { attacks = []; arenaBlasters = []; arenaBossMaxHP = Math.floor(arenaBossMaxHP * 0.9); if (typeof sfxArenaVictory === 'function') sfxArenaVictory(); spawnFloatingText(heart.x, heart.y-20, "СТЁРТО!", "#ff00ff"); },
        onTick() {}
    },
    "Анти-спираль": { name: "ПАССИВНАЯ", cooldown: 0, toggleable: false, duration: 0, onActivate() {}, onTick() {} },
    "Молодой Гарп": {
        name: "ГАЛАКТИЧЕСКИЙ КУЛАК", cooldown: 30000, toggleable: false, duration: 0,
        onActivate() { heartSpeed *= 0.5; _superState.garouStunTimer = 1.5; _superState.garouExplosionPending = true; spawnFloatingText(heart.x, heart.y-20, "ЗАРЯД...", "#4444ff"); },
        onTick(dt) {}
    },
    "Им (Правитель)": {
        name: "ТЕНЕВОЕ ПРАВЛЕНИЕ", cooldown: 30000, toggleable: true, duration: Infinity,
        onActivate() { _superState.imAuraActive = true; _superState.imSpeedPenalty = true; heartSpeed *= 0.7; },
    ...
    "Космический Дэнди": {
        ...
        onActivate() { if (Math.random() < 0.8) { let eff = DANDY_GOOD[Math.floor(Math.random() * DANDY_GOOD.length)]; eff.apply(); spawnFloatingText(heart.x, heart.y-20, eff.name + "!", "#44ff44"); } else { let eff = DANDY_BAD[Math.floor(Math.random() * DANDY_BAD.length)]; eff.apply(); spawnFloatingText(heart.x, heart.y-20, eff.name + "!", "#ff4444"); } },
        onTick() {}
    },
    "Кайдо": {
        name: "ДЫХАНИЕ РАЗРУШЕНИЯ", cooldown: 25000, toggleable: false, duration: 0,
        onActivate() { _superState.kaidoDrinking = true; heartSpeed *= 0.5; spawnFloatingText(heart.x, heart.y-20, "ГЛОТОК...", "#D2691E"); setTimeout(() => { _superState.kaidoDrinking = false; heartSpeed /= 0.5; _superState.kaidoBuffActive = true; _superState.kaidoDmgReduction = true; _superState.invertControls = true; spawnFloatingText(heart.x, heart.y-20, "ЯРОСТЬ!", "#ff4444"); setTimeout(() => { _superState.kaidoBuffActive = false; _superState.kaidoDmgReduction = false; _superState.invertControls = false; }, 7000); }, 2000); },
        onTick() {}
    },
...
function updateSuperLogic(dt) {
    ...
    if (_superState.garouStunTimer > 0) { _superState.garouStunTimer -= dt; if (_superState.garouStunTimer <= 0) { _superState.garouStunTimer = 0; if (_superState.garouExplosionPending) { _superState.garouExplosionPending = false; heartSpeed *= 2; attacks = []; arenaBlasters = []; arenaBossMaxHP = Math.floor(arenaBossMaxHP * 0.95); spawnFloatingText(heart.x, heart.y-20, "ГАЛАКТИКА!", "#4444ff"); if (typeof sfxArenaVictory === 'function') sfxArenaVictory(); } } }
    ...
    if (_superState.dekusParticles && arenaActive) { for (let i = 0; i < 2; i++) { let angle = Math.random() * Math.PI * 2; let dist = 15 + Math.random() * 20; arenaParticles.push({ x: heart.x + Math.cos(angle)*8, y: heart.y + Math.sin(angle)*8, endX: heart.x + Math.cos(angle)*dist, endY: heart.y + Math.sin(angle)*dist, vx: 0, vy: 0, life: 12, maxLife: 12, color: "#44ff44", isLightning: true }); } }
    if (_superState.borosParticles && arenaActive) { for (let i = 0; i < 2; i++) arenaParticles.push({ x: heart.x+(Math.random()-0.5)*40, y: heart.y+(Math.random()-0.5)*40, vx: (Math.random()-0.5)*1, vy: -1-Math.random(), life: 30, maxLife: 30, color: "#66ff66", size: 3+Math.random()*4 }); }
    if (_superState.dandyLightnings && arenaActive) { for (let i = 0; i < 3; i++) { let angle = Math.random() * Math.PI * 2; let dist = 20 + Math.random() * 30; arenaParticles.push({ x: heart.x + Math.cos(angle)*10, y: heart.y + Math.sin(angle)*10, endX: heart.x + Math.cos(angle)*dist, endY: heart.y + Math.sin(angle)*dist, vx: 0, vy: 0, life: 15, maxLife: 15, color: "#ffff00", isLightning: true }); } }
    for (let i = _superState.fists.length-1; i >= 0; i--) { let f = _superState.fists[i]; f.x += f.vx; f.y += f.vy; f.life--; let pathWidth = f.pathWidth || 90; for (let j = attacks.length-1; j >= 0; j--) { let a = attacks[j]; let ax = a.x + (a.size || a.radius || 20)/2; let ay = a.y + (a.size || a.radius || 20)/2; if (Math.abs(ax-f.x) < pathWidth/2 && Math.abs(ay-f.y) < f.size+20) { for (let p = 0; p < 12; p++) arenaParticles.push({ x: ax, y: ay, vx: (Math.random()-0.5)*10, vy: (Math.random()-0.5)*10, life: 20, maxLife: 20, color: "#ffaa00", size: 2+Math.random()*4 }); attacks.splice(j, 1); if (typeof sfxBounce === 'function') sfxBounce(); } } if (f.owner === "Сайтама" && f.willOneshot && !f.oneshotChecked && arenaBossMaxHP > 0) { f.oneshotChecked = true; arenaBossMaxHP = 0; if (typeof sfxArenaVictory === 'function') sfxArenaVictory(); if (typeof winArena === 'function') winArena(); _superState.fists.splice(i, 1); break; } if (f.life <= 0 || f.y < -100 || f.y > 600 || f.x < -50 || f.x > 450) _superState.fists.splice(i, 1); }
}

function renderSuperVisuals() {
    if (!ctx) return;
    drawGarouTrail();
    for (let i = arenaParticles.length-1; i >= 0; i--) { let p = arenaParticles[i]; if (p.isLightning && p.life > 0) drawLightningBolt(p.x, p.y, p.endX, p.endY, p.color, p.life/p.maxLife); }
    ...
    if (_superState.usoppStunTimer > 0 && arenaActive) { ctx.save(); ctx.globalAlpha = 0.3; ctx.fillStyle = "#ffff00"; ctx.shadowColor = "#ffff00"; ctx.shadowBlur = 15; ctx.beginPath(); ctx.arc(heart.x, heart.y, heart.size*2, 0, Math.PI*2); ctx.fill(); ctx.restore(); }
    if (_superState.nikaActive && arenaActive) { ctx.save(); ctx.globalAlpha = 0.2+Math.sin(performance.now()/300)*0.1; ctx.fillStyle = "#ffffff"; ctx.shadowColor = "#ffffff"; ctx.shadowBlur = 25; ctx.beginPath(); ctx.arc(heart.x, heart.y, heart.size*2.5, 0, Math.PI*2); ctx.fill(); ctx.restore(); }
    if (_superState.usoppInvuln && arenaActive) { for (let i = 0; i < 2; i++) arenaParticles.push({ x: heart.x+(Math.random()-0.5)*30, y: heart.y+(Math.random()-0.5)*30, vx: (Math.random()-0.5)*0.5, vy: -1-Math.random(), life: 20, maxLife: 20, color: "#ffff88", size: 2+Math.random()*3 }); }
    if (_superState.imAuraActive && arenaActive) { ctx.save(); ctx.strokeStyle = "rgba(128,0,128,0.8)"; ctx.lineWidth = 4; ctx.shadowColor = "#800080"; ctx.shadowBlur = 15; ctx.beginPath(); ctx.arc(heart.x, heart.y, 80, 0, Math.PI*2); ctx.stroke(); ctx.restore(); }
    if (_superState.kaidoDrinking && arenaActive) { drawBeerBottle(heart.x, heart.y, 1); }
    if (_superState.allmightPermaSlow && arenaActive) { ctx.save(); ctx.globalAlpha = 0.2; ctx.fillStyle = "#ff0000"; ctx.shadowColor = "#ff0000"; ctx.shadowBlur = 15; ctx.beginPath(); ctx.arc(heart.x, heart.y, heart.size*1.8, 0, Math.PI*2); ctx.fill(); ctx.restore(); }
    if (_superState.garouInvulnTimer > 0 && arenaActive) { ctx.save(); ctx.globalAlpha = 0.3; ctx.strokeStyle = "#ff8800"; ctx.lineWidth = 3; ctx.shadowColor = "#ff8800"; ctx.shadowBlur = 20; ctx.beginPath(); ctx.arc(heart.x, heart.y, heart.size*1.5, 0, Math.PI*2); ctx.stroke(); ctx.restore(); }
}
