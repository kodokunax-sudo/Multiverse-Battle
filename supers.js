...
// Добавлены визуальные эффекты для скиллов и оптимизированы молнии Деку
...
// ========== ВИЗУАЛЬНЫЕ ЭФФЕКТЫ СУПЕРСПОСОБНОСТЕЙ ==========
function renderSuperVisuals() {
    if (!arenaActive || !ctx || typeof _superState === 'undefined') return;
    let now = Date.now();
    
    // ⚡ 1. ДЕКУ: 100% (Молнии сильно уменьшены по частоте, как просили)
    if (_superState.dekusActive && Math.random() < 0.04) {
        arenaParticles.push({
            x: heart.x + (Math.random() - 0.5) * 40,
            y: heart.y + (Math.random() - 0.5) * 40,
            vx: 0, vy: 0, life: 12, maxLife: 12,
            color: "#44ff44", size: 2,
            isLightning: true, angle: Math.random() * Math.PI * 2
        });
    }

    // ☀️ 2. ЛУФФИ (НИКА): Мультяшные искры и аура солнца
    if (_superState.nikaActive) {
        ...
        for(let i = 0; i < 8; i++){
            ...
            ctx.moveTo(0, 0); ctx.lineTo(8, 25); ctx.lineTo(-8, 25);
        }
        ctx.fill();
        ctx.restore();
        
        if (Math.random() < 0.15) {
            arenaParticles.push({ x: heart.x, y: heart.y, vx: (Math.random()-0.5)*3, vy: -1-Math.random()*4, life: 25, maxLife: 25, color: "#ffffff", size: 3 + Math.random()*2, isBouncy: true });
        }
    }

    // 🌌 3. ГАРОУ: Космическое искажение / Остановка времени
    if (_superState.garouInvulnTimer > 0 || _superState.garouTimeStop) {
        ctx.save();
        let gPulse = Math.sin(now / 150) * 5;
        ctx.strokeStyle = _superState.garouTimeStop ? "rgba(255, 136, 0, 0.6)" : "rgba(255, 68, 0, 0.4)";
        ctx.lineWidth = 2; ctx.setLineDash([5, 5]);
        ctx.lineDashOffset = now / 20;
        ctx.beginPath(); ctx.arc(heart.x, heart.y, 20 + gPulse, 0, Math.PI * 2); ctx.stroke();
        ctx.restore();
    }

    // 🔴 4. ГАРП: Королевская воля (Черно-красные молнии)
    if (_superState.garpHakiActive && Math.random() < 0.1) {
        arenaParticles.push({
            x: heart.x + (Math.random() - 0.5) * 50, y: heart.y + (Math.random() - 0.5) * 50,
            vx: 0, vy: 0, life: 10, maxLife: 10,
            color: Math.random() > 0.5 ? "#ff0000" : "#000000", 
            size: 3, isLightning: true, angle: Math.random() * Math.PI * 2
        });
    }
    ...
    if (_superState.imAuraActive) {
        ...
        ctx.beginPath(); ctx.arc(heart.x, heart.y, 55, 0, Math.PI * 2); ctx.stroke();
        ctx.restore();
    }

    // 💚 6. БОРОС: Регенерация (Медицинские крестики)
    if (_superState.borosHeal && _superState.borosHeal.active && Math.random() < 0.1) {
        arenaParticles.push({
            x: heart.x + (Math.random() - 0.5) * 20, y: heart.y + 10,
            vx: 0, vy: -1.5, life: 30, maxLife: 30, color: "#00ff88", size: 2.5, isCross: true
        });
    }

    // 🌀 7. АНТИ-СПИРАЛЬ: Глитчи и цифровые помехи
    if (_superState.antispiralActive && Math.random() < 0.2) {
        ctx.save();
        ctx.fillStyle = Math.random() > 0.5 ? "rgba(0, 255, 255, 0.6)" : "rgba(255, 0, 255, 0.6)";
        ctx.fillRect(heart.x - 15 + Math.random()*30, heart.y - 15 + Math.random()*30, 15, 3);
        ctx.restore();
    }
    
    // 🌟 8. ВСЕМОГУЩИЙ: Героическое свечение
    if (_superState.allmightBuffTimer > 0) {
        ctx.save();
        ctx.shadowColor = "#ffd700"; ctx.shadowBlur = 15;
        ctx.strokeStyle = "rgba(255, 215, 0, 0.7)";
        ctx.lineWidth = 2;
        ctx.beginPath(); ctx.arc(heart.x, heart.y, 16 + Math.sin(now/100)*3, 0, Math.PI*2); ctx.stroke();
        ctx.restore();
    }

    // 🔥 9. КАЙДО: Огненное дыхание / Ярость
    if (_superState.kaidoBuffActive && Math.random() < 0.25) {
        arenaParticles.push({
            x: heart.x + (Math.random()-0.5)*15, y: heart.y + 10,
            vx: (Math.random()-0.5)*1, vy: -1.5 - Math.random()*2,
            life: 20, maxLife: 20, color: "#ff3300", size: 4 + Math.random()*3, isFire: true
        });
    }
    
    // 💛 10. МАРК: Эхо воскрешения
    if (_superState.markBuffActive) {
         ctx.save();
         ctx.strokeStyle = "rgba(255, 255, 255, 0.9)";
         let markRadius = 15 + (now % 600) / 600 * 25;
         ctx.globalAlpha = 1 - ((now % 600) / 600);
         ctx.beginPath(); ctx.arc(heart.x, heart.y, markRadius, 0, Math.PI*2); ctx.stroke();
         ctx.restore();
    }

    // 🛡️ 11. ДЭНДИ: Щиты и уязвимость
    if (_superState.dandyShield && _superState.dandyShield.timer > 0) {
        ctx.save();
        ctx.strokeStyle = "rgba(0, 255, 255, 0.8)"; ctx.lineWidth = 2;
        ctx.beginPath(); ctx.moveTo(heart.x - 10, heart.y - 12); ctx.lineTo(heart.x + 10, heart.y - 12); ctx.lineTo(heart.x, heart.y + 12); ctx.closePath(); ctx.stroke();
        ctx.restore();
    }
}
...
function renderArena() {
    ...
    // ======== ОБНОВЛЕННАЯ ФИЗИКА И РЕНДЕР ЧАСТИЦ ========
    ctx.save(); ctx.globalCompositeOperation='lighter';
    for (let i = arenaParticles.length-1; i >= 0; i--) { 
        let p = arenaParticles[i]; 
        if (p.isBouncy) p.vy += 0.2; 
        p.x += p.vx; p.y += p.vy; 
        if (!p.isBouncy) { p.vx *= 0.95; p.vy *= 0.95; }
        p.life--; 
        ...
        if (p.isLightning) {
            ...
            let lx = p.x, ly = p.y;
            for(let j = 0; j < 3; j++) {
                lx += Math.cos(p.angle) * 8 + (Math.random()-0.5)*12;
                ly += Math.sin(p.angle) * 8 + (Math.random()-0.5)*12;
                ctx.lineTo(lx, ly);
            }
            ctx.stroke();
        } else if (p.isCross) {
            ctx.fillStyle = p.color;
            ctx.fillRect(p.x - p.size, p.y - p.size/3, p.size*2, p.size*0.6);
            ctx.fillRect(p.x - p.size/3, p.y - p.size, p.size*0.6, p.size*2);
        } else if (p.isFire) {
            ctx.fillStyle = p.color;
            ctx.beginPath(); ctx.arc(p.x, p.y, p.size * ratio, 0, Math.PI * 2); ctx.fill();
            ctx.fillStyle = "#ffaa00";
            ctx.beginPath(); ctx.arc(p.x, p.y, (p.size * ratio) / 2, 0, Math.PI * 2); ctx.fill();
        } else {
            ctx.fillStyle = p.color; 
            ctx.beginPath(); ctx.arc(p.x, p.y, p.size * ratio, 0, Math.PI * 2); ctx.fill(); 
        }
