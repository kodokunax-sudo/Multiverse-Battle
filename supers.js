// ========== СУПЕР-СПОСОБНОСТИ СЕКРЕТНЫХ КАРТ (АРЕНА) v14.5 FIXED ==========
// Исправлено: screenShakeAmount затухает так же быстро, как arenaShake (x0.88)
    ...
    if (typeof arenaActive !== 'undefined' && arenaActive) {
        arenaDodgeTimerInterval = setInterval(function() {
            if (typeof arenaPhase !== 'undefined' && arenaPhase === "dodge" && typeof arenaActive !== 'undefined' && arenaActive) {
                if (typeof arenaDodgeTimer !== 'undefined') {
                    arenaDodgeTimer--;
...
function updateSuperLogic(dt) { 
    ...
    // ★★★ ГЛАВНОЕ ИСПРАВЛЕНИЕ: screenShakeAmount теперь затухает так же быстро как arenaShake ★★★
    if (_superState.screenShakeAmount > 0) {
        _superState.screenShakeAmount *= 0.88;
        if (_superState.screenShakeAmount < 0.15) _superState.screenShakeAmount = 0;
    }
