...
// Исправлено: зависание после Разлома Деку (принудительный сброс таймеров)
    ...
    "Деку (100%)": {
        ...
        onDeactivate() {
            ...
            if (_superState.dekuSmashBlackoutTimer > 0 || _superState.dekuSmashSequenceTimer > 0) {
                ...
                // Принудительный переход если таймер истёк
                if (typeof arenaDodgeTimer !== 'undefined' && arenaDodgeTimer <= 0 && typeof arenaPhase !== 'undefined' && arenaPhase === "dodge") {
                    if (typeof startAttackPhase === 'function') startAttackPhase();
                }
            }
        },
...
// ====== РАЗЛОМ ДЕКУ (ЭПИЧНАЯ ВЕРСИЯ С ФИКСОМ) ======
function triggerDekuSmash() {
    ...
    _superState.originalArenaSpeedMod = arenaGlobalSpeedMod;
        ...
        if (_superState.dekuSmashSequenceTimer > 0) {
            ...
            arenaDodgeTimerInterval = setInterval(function() {
                if (typeof arenaPhase !== 'undefined' && arenaPhase === "dodge" && typeof arenaActive !== 'undefined' && arenaActive) {
                    ...
                    if (typeof updateDodgeTimerDisplay === 'function') updateDodgeTimerDisplay();
                    // Принудительный переход если таймер истёк
                    if (typeof arenaDodgeTimer !== 'undefined' && arenaDodgeTimer <= 0 && typeof startAttackPhase === 'function') {
                        clearInterval(arenaDodgeTimerInterval);
                        startAttackPhase();
                    }
                }
