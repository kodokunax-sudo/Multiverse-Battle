...
let dekuSmashActive = false;
let dekuSmashBlackoutTimer = 0;
let dekuSmashSequenceTimer = 0;
let dekuFists = [];
let originalHeartSpeed = 1.2;
...
function moveHeart() {
    ...
    if (joystickActive) { mx = (joystickX - heart.x) / 15; my = (joystickY - heart.y) / 15; var len = Math.sqrt(mx*mx+my*my); if (len > 1) { mx /= len; my /= len; } }
    ...
    if (isMoving) { heartStandingTime = 0; heartRotation += 0.15; var len = Math.sqrt(mx*mx+my*my); if (len > 1) { mx /= len; my /= len; } heart.x += mx * heartSpeed; heart.y += my * heartSpeed; }
    ...
    clampHeart();
    if (Math.random() > 0.3) { arenaTrail.push({ x: heart.x, y: heart.y, life: 12, maxLife: 12, size: Math.max(1, heart.size*0.75), color: "rgba(255, 30, 30, 0.35)" }); }
...
function triggerDekuSmash() {
    if (!arenaActive) return;
    dekuSmashActive = true;
    dekuSmashBlackoutTimer = 60; // Ровно 1 секунда (при 60 FPS) на кромешную тьму
    dekuSmashSequenceTimer = 150; // 1.5 секунды на эпичное появление кулаков
    
    // Замедляем всё в 3 раза (игрока и атаки)
    originalHeartSpeed = heartSpeed;
    heartSpeed = heartSpeed / 3;
    arenaGlobalSpeedMod = 0.33; 
    
    // Замедляем таймер арены в 3 раза
    if (arenaDodgeTimerInterval) clearInterval(arenaDodgeTimerInterval);
    ...
    }, 3000); // 3000 мс вместо 1000 мс

    // Генерируем массив кулаков с задержкой появления
    dekuFists = [];
    for (let i = 0; i < 8; i++) {
        dekuFists.push({
            ...
            delay: 60 + (i * 10), // Кулаки появляются по очереди после секунды тьмы
            life: 25, 
            active: false
        });
    }
    
    playArenaSound(80, 'sawtooth', 2.0, 0.3); // Жесткий звук активации
}
...
function renderArena() {
    ...
    // --- ДЕКУ: РАЗЛОМ (Логика и визуализация) ---
    if (dekuSmashActive) {
        if (dekuSmashBlackoutTimer > 0) {
            dekuSmashBlackoutTimer--;
        ...
        if (dekuSmashSequenceTimer > 0) {
            dekuSmashSequenceTimer--;
            
            // Отрисовка кулаков
            dekuFists.forEach(fist => {
                if (fist.delay > 0) {
                    fist.delay--;
                } else if (fist.life > 0) {
                    fist.life--;
                    ...
                    // Ломаем блоки на увеличенной площади
                    for (let i = attacks.length - 1; i >= 0; i--) {
                        let a = attacks[i];
                        let dx = a.x - fist.x;
                        let dy = a.y - fist.y;
                        if (Math.sqrt(dx * dx + dy * dy) < fist.radius + 35) { // Площадь жестко увеличена
                            attacks.splice(i, 1);
                            // Изумрудные искры от уничтожения
                            for(let k=0; k<6; k++) {
            ...
            // Возвращаем время и таймеры в нормальное состояние
            dekuSmashActive = false;
            heartSpeed = originalHeartSpeed;
    ...
    if (arenaPhase === "dodge") {
        ...
        if (invulnTimer<=0||Math.floor(now/80)%2===0) { 
            ...
            if (typeof dekuSmashActive !== 'undefined' && dekuSmashBlackoutTimer > 0) {
                ctx.fillStyle = "#000000";
                ctx.shadowBlur = 0;
            } else {
                ...
                ctx.shadowColor=(typeof _superState!=='undefined'&&_superState.dekusParticles)?"#50c878":(typeof _superState!=='undefined'&&_superState.allmightBuffTimer>0)?"#ffd700":(typeof _superState!=='undefined'&&_superState.usoppInvuln)?"#ffff00":"#ff0000"; 
            ...
            if (invulnTimer>0) { 
                ...
                ctx.lineWidth=3; 
                ctx.beginPath(); 
                ctx.arc(0,2,Math.max(0.1,heart.size*1.3),0,Math.PI*2); 
                ctx.stroke(); 
            } 
            ctx.restore(); 
        }
