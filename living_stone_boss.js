// ============================================================
// ЖИВОЙ КАМЕНЬ - БОСС 200 ВОЛНЫ v4.6
// + скорость обратно к 1.0
// + фаза 2: длинный камень + 20 кубиков + злые брови
// + аватарка меняется при QTE-ударах
// + QTE: нужно 100 кликов чтобы выжить
// ============================================================
...
// ★ Урон аватарки при кликах QTE ★
let qteBossDamageLevel = 0; // 0..1 - как сильно побит босс
let qteBossShake = 0;

// Мега-эффекты
let qteShockwaves = [];
let qteLightningBolts = [];
let qteSlashMarks = [];
let qteSparks = [];
...
function handleLSTouchStart(ev) {
    ...
    if (livingStoneState === "phase1" || livingStoneState === "phase2") {
        ...
        if (ev.touches.length > 0 && !lsTouchActive) {
            lsTouchActive = true;
            lsTouchId = ev.touches[0].identifier;
...
function spawnMegaImpact(x, y) {
    ...
    for (var i = 0; i < 8; i++) {
        ...
        qteLightningBolts.push({
            points: points, life: 25, maxLife: 25,
    ...
    for (var i = 0; i < 100; i++) {
        ...
        qteSparks.push({
            x: x, y: y,
            vx: Math.cos(angle3) * spd, vy: Math.sin(angle3) * spd,
...
function livingStoneSpawnAttack() {
    ...
    if (type === 0) {
        ...
        for (var i = 0; i < count3; i++) {
            ...
            livingStoneAttacks.push({
                ...
                vx: Math.cos(angle) * 1.2 * lsSpeedMult,
                vy: Math.sin(angle) * 1.2 * lsSpeedMult,
        ...
        livingStoneAttacks.push({
            type: "laser_warning", x: laserX, y: 0, width: 40,
            timer: Math.floor(60 / lsSpeedMult),
        ...
        for (var i = 0; i < countR; i++) {
            livingStoneAttacks.push({
                ...
                size: 10 + Math.random() * 6, rotation: 0,
        ...
        for (var i = 0; i < spiralCount; i++) {
            ...
            (function(ang, dly) {
                setTimeout(function() {
                    ...
                    livingStoneAttacks.push({
                        type: "orb",
                        x: livingStoneBoss.x, y: livingStoneBoss.y,
                        vx: Math.cos(ang) * 2.0 * speedMult,
                        vy: Math.sin(ang) * 2.0 * speedMult,
                        size: 7, damage: Math.floor(5 * dmgMult),
    ...
    if (type === 8) {
        ...
        livingStoneAttacks.push({
            ...
            radius: 45, maxRadius: 55, life: wellDuration, maxLife: wellDuration,
        ...
        for (var i = 0; i < stoneCount; i++) {
            ...
            livingStoneAttacks.push({
                type: "gravity_stone", x: sx, y: sy, vx: 0, vy: 0,
                ...
                if (livingStoneAttacks[k].type === "gravity_well") {
                    ...
                    for (var j = 0; j < 12; j++) {
                        ...
                        livingStoneAttacks.push({
                            ...
                            size: 9, damage: Math.floor(9 * dmgMult),
    ...
    if (type === 10 && isPhase2) {
        ...
        if (isVertical) {
            ...
            for (var i = 10; i < 490; i += 30) {
                ...
                livingStoneAttacks.push({
                    type: "rock", x: startX, y: i, vx: dirX, vy: 0,
                    size: 22, rotation: 0, rotSpeed: 0.05,
            ...
            for (var i = 10; i < 390; i += 30) {
                ...
                livingStoneAttacks.push({
                    type: "rock", x: i, y: startY, vx: 0, vy: dirY,
                    size: 22, rotation: 0, rotSpeed: 0.05,
                    damage: Math.floor(9 * dmgMult)
                });
            }
        ...
        livingStoneAttacks.push({
            type: "laser_warning", x: laserX2, y: 0, width: 70,
            timer: Math.floor(50 / lsSpeedMult),
        ...
        for (var i = 0; i < 4; i++) {
            livingStoneAttacks.push({
                ...
                size: 12, rotation: 0, rotSpeed: 0.08,
                damage: Math.floor(7 * dmgMult)
            });
        }
    }
...
function updateLivingStonePlayer() {
    ...
    if (lsTouchActive) {
        var tx = lsTouchX - livingStonePlayer.x;
        var ty = lsTouchY - livingStonePlayer.y;
...
function triggerMusicScene() {
    ...
    setTimeout(function() {
        ...
        livingStoneScreenFlashColor = "#ffffff";
    }, Math.floor(4000 / lsSpeedMult));
    
    setTimeout(function() {
        if (!livingStoneActive) return;
        triggerCinematic();
    }, Math.floor(8000 / lsSpeedMult));
}
...
function triggerCinematic() {
    ...
    qteBossArm = {
        startX: livingStoneBoss.x, startY: livingStoneBoss.y,
        endX: livingStonePlayer.x, endY: livingStonePlayer.y,
        progress: 0, duration: 1.2 / lsSpeedMult
    };
    ...
    setTimeout(function() {
        ...
        qtePlayerVel = { x: 12 * lsSpeedMult, y: 8 * lsSpeedMult };
...
function triggerQTEStart() {
    ...
    qteBarrageInterval = setInterval(function() {
        ...
        spawnQTEBarrage();
    }, Math.floor(400 / lsSpeedMult));
    
    // 30 секунд на 100 кликов
    qteEndTimer = setTimeout(function() {
        if (qteBarrageInterval) { clearInterval(qteBarrageInterval); qteBarrageInterval = null; }
        if (!livingStoneActive) return;
        // Если не сделал 100 кликов — умирает
...
function spawnQTEBarrage() {
    ...
    for (var i = 0; i < 6; i++) {
        ...
        var speed = (12 + Math.random() * 8) * lsSpeedMult;
    ...
    for (var j = 0; j < 3; j++) {
        qteBullets.push({
            ...
            vx: (Math.random() - 0.5) * 3 * lsSpeedMult,
            vy: (-15 - Math.random() * 5) * lsSpeedMult,
...
function handleQTEClick(ev) {
    ...
    // ★ Аватарка босса «повреждается» при каждом клике ★
    ...
    for (var i = 0; i < 4; i++) {
        ...
        qteBullets.push({
            x: sx, y: sy,
            vx: (dx/len) * 15 * lsSpeedMult, vy: (dy/len) * 15 * lsSpeedMult,
    ...
    // ★ Автоматическое завершение при достижении 100 кликов ★
    if (qteClicks >= qteClickTarget) {
        if (qteEndTimer) { clearTimeout(qteEndTimer); qteEndTimer = null; }
        if (qteBarrageInterval) { clearInterval(qteBarrageInterval); qteBarrageInterval = null; }
        triggerQTEFinish();
...
function triggerQTEFinish() {
    ...
    setTimeout(function() {
        ...
        setTimeout(function() {
            if (!livingStoneActive) return;
            triggerPhase2();
        }, Math.floor(2500 / lsSpeedMult));
    }, Math.floor(500 / lsSpeedMult));
}
    ...
    for (var i = livingStoneAttacks.length - 1; i >= 0; i--) {
        ...
        if (a.type === "rock") {
            ...
            if (a.timer <= 0) {
                livingStoneAttacks.splice(i, 1);
                livingStoneAttacks.push({
                    type: "laser", x: a.x, y: 0, width: a.width,
                    timer: Math.floor(25 / lsSpeedMult),
                    damage: a.damage, hit: false
                });
                livingStoneShake = 12;
            }
...
function updateCinematic() {
    if (qteCinematicPhase === "grabbed") {
        if (qteBossArm) {
            qteBossArm.progress += 0.02 * lsSpeedMult;
        ...
        var len = Math.sqrt(dx*dx + dy*dy) || 1;
        if (len > 3) {
            livingStonePlayer.x += (dx/len) * 2.5 * lsSpeedMult;
            livingStonePlayer.y += (dy/len) * 2.5 * lsSpeedMult;
        }
        ...
        if (len2 > 3) {
            livingStonePlayer.x += (dx2/len2) * 7 * lsSpeedMult;
            livingStonePlayer.y += (dy2/len2) * 7 * lsSpeedMult;
        }
        qtePlayerTrail.push({ x: livingStonePlayer.x, y: livingStonePlayer.y, life: 20, maxLife: 20 });
...
function livingStoneRenderLoop() {
    ...
    if (livingStoneState === "phase1" || livingStoneState === "phase2") {
        ...
        if (livingStoneTypeTimer <= 0) {
            if (livingStoneState === "phase2") {
                ...
                livingStoneTypeTimer = Math.floor((200 + Math.floor(Math.random() * 200)) / lsSpeedMult);
            } else {
                livingStoneAttackType = Math.floor(Math.random() * 10);
                livingStoneTypeTimer = Math.floor((250 + Math.floor(Math.random() * 250)) / lsSpeedMult);
            }
            var typeNames = [
    ...
    if (lsTouchActive && (livingStoneState === "phase1" || livingStoneState === "phase2")) {
        ctx.save();
        ctx.globalAlpha = 0.3;
        ctx.strokeStyle = "#ffffff";
        ctx.lineWidth = 2;
...
// ★ ПОЛНОСТЬЮ ПЕРЕРАБОТАННАЯ ФУНКЦИЯ ОТРИСОВКИ БОССА ★
function drawLivingStoneBoss() {
    ...
    var isVictory = (livingStoneState === "victory");
    
    // Тряска при клике в QTE
    ...
    ctx.beginPath();
    if (isPhase2) {
        ctx.ellipse(0, 0, size * 2.6, size * 1.3, 0, 0, Math.PI * 2);
    } else {
    ...
    if (isPhase2) {
        // ★ ДЛИННАЯ ФОРМА ★
        ctx.ellipse(0, 0, size * 2.4, size * 0.95, 0, 0, Math.PI * 2);
    } else {
    ...
    if (isPhase2) {
        ...
        for (var i = 0; i < 20; i++) {
            ...
            var cubeSize = 6 + Math.sin(now2 / 300 + i) * 2.5;
    ...
    for (var i = 0; i < crackCount; i++) {
        ...
        var ang2 = ang1 + 1.2;
        var scaleX = isPhase2 ? 1.8 : 1;
        ctx.moveTo(Math.cos(ang1) * size * 0.3 * scaleX, Math.sin(ang1) * size * 0.3);
        ctx.lineTo(Math.cos(ang1) * size * 0.8 * scaleX, Math.sin(ang1) * size * 0.8);
        ctx.lineTo(Math.cos(ang2) * size * 0.9 * scaleX, Math.sin(ang2) * size * 0.9);
        ctx.stroke();
    }
    
    // ★ ТРЕЩИНЫ ОТ УРОНА В QTE ★
    if (isQTEPunch && qteBossDamageLevel > 0) {
        var damageCracks = Math.floor(qteBossDamageLevel * 12);
        ctx.strokeStyle = "rgba(255, 100, 0, " + (0.5 + qteBossDamageLevel * 0.5) + ")";
        ctx.lineWidth = 3;
        ctx.shadowColor = "#ff4400";
        ctx.shadowBlur = 10;
        for (var i = 0; i < damageCracks; i++) {
            ...
            var radEnd = size * 0.9 * (0.5 + qteBossDamageLevel * 0.5);
            ctx.beginPath();
    ...
    if (isQTEPunch) {
        // Глаза становятся налитыми кровью при QTE
        eyeColor = "#ff0000";
    }
    ...
    var eyeOffsetX = isPhase2 ? size * 0.6 : size * 0.3;
    var eyeSize = isPhase2 ? size * 0.15 : size * 0.12;
    
    ctx.beginPath();
    ctx.arc(-eyeOffsetX, -size * 0.15, eyeSize, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.arc(eyeOffsetX, -size * 0.15, eyeSize, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.shadowBlur = 0;
    ctx.beginPath();
    ctx.arc(-eyeOffsetX + size * 0.02, -size * 0.13, eyeSize * 0.4, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.arc(eyeOffsetX + size * 0.02, -size * 0.13, eyeSize * 0.4, 0, Math.PI * 2);
    ctx.fill();
    ...
    if (isPhase2) {
        ...
        // Левая бровь (наклонена вниз вправо — злая)
        ctx.beginPath();
        ctx.moveTo(-size * 1.1, -size * 0.55);
        ctx.lineTo(-size * 0.35, -size * 0.28);
        ctx.stroke();
        
        // Правая бровь (наклонена вниз влево — злая)
        ctx.beginPath();
        ctx.moveTo(size * 1.1, -size * 0.55);
        ctx.lineTo(size * 0.35, -size * 0.28);
        ctx.stroke();
    ...
    if (isPhase2) {
        ...
        ctx.lineWidth = 4;
        ctx.beginPath();
        var mouthY = size * 0.35;
        var mouthW = size * 1.2;
        // Волнообразный злой рот
        ctx.moveTo(-mouthW/2, mouthY);
        for (var i = 0; i <= 6; i++) {
            var t = i / 6;
            var mx = -mouthW/2 + mouthW * t;
            var my = mouthY + (i % 2 === 0 ? 0 : size * 0.12);
            ctx.lineTo(mx, my);
        }
        ctx.stroke();
...
function drawQTEOverlay() {
    ...
    if (livingStoneState === "qte_intro") {
        ...
        if (blink) ctx.fillText("▶ ПРИГОТОВЬСЯ...", 200, 460);
        ctx.restore();
    } else if (livingStoneState === "qte_punch") {
        // ★ ПРОГРЕСС-БАР 100 КЛИКОВ ★
        var barW = 300;
        var barH = 20;
        var barX = 50;
        var barY = 430;
        var progress = Math.min(1, qteClicks / qteClickTarget);
        ...
        ctx.shadowColor = "#000";
        ctx.shadowBlur = 3;
        ctx.fillText("👊 " + qteClicks + " / " + qteClickTarget, barX + barW/2, barY + barH/2 + 5);
        ctx.restore();
        ...
        if (blink2) ctx.fillText("👊 НАЖИМАЙ! 👊", 200, 470);
        ctx.restore();
...
console.log("[LIVING STONE] Модуль загружен v4.6");
