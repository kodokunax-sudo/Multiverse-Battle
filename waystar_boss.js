// ============================================================
// ПУТЕВОДНАЯ ЗВЕЗДА — БОСС 500 ВОЛНЫ v10.0 (BEAUTY EDITION)
// Эффекты свечения, переработанное Созвездие, новые частицы!
// ============================================================

if (window._waystarBossLoaded === true) {
    console.warn("[WAYSTAR] Скрипт уже загружен! Второй вызов игнорируется.");
} else {
    window._waystarBossLoaded = true;

// ★ ВСЕ ПЕРЕМЕННЫЕ ЧЕРЕЗ var ★
...
var waystarFinalDeathAnim = null;
...
// Улучшенный спавн частиц с типами
function spawnWaystarParticles(x, y, count, color, speed, shape) {
    var sType = shape || "circle";
    for (var i = 0; i < count; i++) {
        var a = Math.random() * Math.PI * 2, s = speed * (0.3 + Math.random());
        var rShape = sType === "mixed" ? (Math.random() > 0.6 ? "star" : (Math.random() > 0.5 ? "spark" : "circle")) : sType;
...
function waystarSpawnAttack() {
    ...
    if (type === 0) {
        ...
        if (Math.random() < 0.33) {
            ...
            waystarAttacks.push({
                type: "laser", state: "warning", x: targetX, width: 55,
                warningTimer: 45, activeTimer: 0, maxActive: 70, damage: 18, hit: false, chargeParticles: 0
            });
        }
        wsPlaySound(300, 'square', 0.3, 0.15);
    } else if (type === 3) {
        // Улучшенное СОЗВЕЗДИЕ
        var points = [];
        var count2 = isSecond ? 6 : 4;
        for (var i = 0; i < count2; i++) points.push({ x: 40 + Math.random() * 320, y: 80 + Math.random() * 340 });
        waystarAttacks.push({ type: "constellation", points: points, timer: 180, warning: 80, damage: 20, hit: false, animTime: 0 });
...
function waystarTriggerSplit() {
    ...
    for (var i = 0; i < 6; i++) {
        setTimeout(function(idx) { addWaystarShockwave(waystarBoss.x, waystarBoss.y, ["#ffffff", "#ffd700", "#ff00ff", "#ff4400"][idx%4], 350, 30, 6); }, i * 100);
    }
    for (var i = 0; i < 20; i++) {
    ...
    for (var i = 0; i < 200; i++) spawnWaystarParticles(waystarBoss.x, waystarBoss.y, 1, ["#ffd700", "#ffffff", "#ff8800", "#ff00ff"][Math.floor(Math.random() * 4)], 18, "mixed");
...
function waystarStartSplit2Animation() {
    ...
    setTimeout(function() {
        ...
        for (var i = 0; i < 8; i++) {
            setTimeout(function(idx) { addWaystarShockwave(waystarBoss.x, waystarBoss.y, ["#ffffff", "#ffd700", "#ff00ff", "#ff4400"][idx%4], 400, 35, 7); }, i * 80);
        }
        for (var i = 0; i < 60; i++) {
            ...
            waystarParticles.push({ x: waystarBoss.x, y: waystarBoss.y,
                ...
                color: ["#ffd700", "#ff00ff", "#ffffff"][i % 3], size: 4 + Math.random() * 3, shape: "star" });
        }
        for (var i = 0; i < 300; i++) spawnWaystarParticles(waystarBoss.x, waystarBoss.y, 1, ["#ffd700", "#ffffff", "#ff8800", "#ff00ff"][Math.floor(Math.random() * 4)], 20, "mixed");
    ...
    for (var i = waystarPlayerBullets.length - 1; i >= 0; i--) {
        ...
        for (var j = 0; j < waystarPieces.length; j++) {
            ...
            if (Math.abs(b.x - p.x) < p.size + 5 && Math.abs(b.y - p.y) < p.size + 5) {
                ...
                spawnWaystarParticles(p.x, p.y, 8, "#ffd700", 4, "spark");
                addWaystarFlash(p.x, p.y, 12);
                if (p.hp <= 0) {
                    p.alive = false; waystarPiecesAlive--;
                    addWaystarShockwave(p.x, p.y, "#ff00ff", 60, 12, 3);
                    addWaystarFlash(p.x, p.y, 25);
                    spawnWaystarParticles(p.x, p.y, 15, "#ff00ff", 6, "star");
...
function waystarStartReturning() {
    ...
    for (var i = 0; i < 150; i++) spawnWaystarParticles(200, 100, 1, ["#ff00ff", "#ffffff", "#ffaa00"][Math.floor(Math.random() * 3)], 15, "mixed");
    waystarSmallBoss = { x: 200, y: 100, size: 28, rotation: 0, pulse: 0, time: 0, alpha: 0, trail: [] };
...
function waystarStartPhase3() {
    ...
    waystarAttackTimer = 0; waystarAttackType = 0; waystarTypeTimer = 120; waystarAmbientTimer = 0; waystarShootCooldown = 0;
    waystarRageMode = false; waystarPhase3Embers = []; waystarPhase3Rings = []; waystarEscalationLevel = 0; waystarEscalationTimer = 0;
    waystarPhase3AttacksStarted = false;
    ...
    for (var i = 0; i < 6; i++) {
        setTimeout(function(idx) { addWaystarShockwave(200, 100, ["#ff00ff", "#ffd700", "#ffffff", "#ff00ff"][idx%4], 350, 30, 6); }, i * 100);
    }
    for (var i = 0; i < 40; i++) {
        var ang = (i / 40) * Math.PI * 2;
        addWaystarLightning(200, 100, 200 + Math.cos(ang) * 250, 100 + Math.sin(ang) * 250, i % 2 === 0 ? "#ff00ff" : "#ffffff", 1.0, 3);
    }
    for (var i = 0; i < 300; i++) spawnWaystarParticles(200, 100, 1, ["#ff00ff", "#ffffff", "#ffd700", "#ff4400"][Math.floor(Math.random() * 4)], 20, "mixed");

    if (typeof showFloatingText === 'function') showFloatingText("💥 ФИНАЛЬНАЯ ФАЗА! 💥", "#ff00ff");
    waystarAttackTimer = 9999; 
}
    ...
    if (type === 0) {
        ...
        for (var i = 0; i < count; i++) {
            ...
            waystarAttacks.push({ type: "shotgun_bullet", x: waystarSmallBoss.x, y: waystarSmallBoss.y,
                vx: Math.cos(ang) * 3.8 * s, vy: Math.sin(ang) * 3.8 * s, size: 6, damage: 9, life: 250, trail: [] });
        }
        ...
        for (var k = 0; k < bombCount; k++) {
            waystarBombs.push({ x: 60 + Math.random() * 280, y: 80 + Math.random() * 300, timer: 150, maxTimer: 150, radius: 70, damage: 25, exploded: false, explosionTimer: 0 });
        }
        wsPlaySound(500, 'sine', 0.3, 0.15);
    } else if (type === 2) {
        if (!waystarDash) {
            var dx = waystarPlayer.x - waystarSmallBoss.x, dy = waystarPlayer.y - waystarSmallBoss.y;
            var len = Math.sqrt(dx * dx + dy * dy) || 1;
            waystarDash = { x: waystarSmallBoss.x, y: waystarSmallBoss.y, startX: waystarSmallBoss.x, startY: waystarSmallBoss.y,
                targetX: waystarPlayer.x, targetY: waystarPlayer.y, dirX: dx / len, dirY: dy / len, progress: 0, duration: 55, damage: 20, hit: false };
        }
        wsPlaySound(150, 'sawtooth', 0.5, 0.25);
    } else if (type === 3) {
        waystarAttacks.push({ type: "giant_laser", state: "warning", x: waystarSmallBoss.x, y: waystarSmallBoss.y,
            angle: Math.atan2(waystarPlayer.y - waystarSmallBoss.y, waystarPlayer.x - waystarSmallBoss.x),
            width: 55, warningTimer: 70, activeTimer: 0, maxActive: 55, damage: 25, hit: false, trackTimer: 45 });
        ...
        for (var i = 0; i < count; i++) {
            ...
            (function(a, d) {
                setTimeout(function() {
                    ...
                    waystarAttacks.push({ type: "spiral", x: waystarSmallBoss.x, y: waystarSmallBoss.y, vx: Math.cos(a) * 2.2 * s, vy: Math.sin(a) * 2.2 * s, size: 6, damage: 9, life: 250, trail: [] });
                }, d);
            })(baseAng, delay);
            (function(a, d) {
                setTimeout(function() {
        ...
        for (var i = 0; i < mineCount; i++) {
            waystarBombs.push({ x: 40 + Math.random() * 320, y: 80 + Math.random() * 320, timer: 110, maxTimer: 110, radius: 40, damage: 12, exploded: false, explosionTimer: 0, small: true });
        }
...
function handleWaystarClick(ev) {
    ...
    if (waystarDialogActive) {
        ...
        } else { waystarProgressDialog(); }
    }
}
function handleWaystarTouchStart(ev) {
    if (!waystarActive) return; ev.preventDefault();
    if (waystarDialogActive) { var t = ev.touches[0]; handleWaystarClick({ clientX: t.clientX, clientY: t.clientY }); return; }
    if (ev.touches.length > 0) {
        var rect = canvas.getBoundingClientRect();
        waystarTouchActive = true; waystarTouchId = ev.touches[0].identifier;
        waystarTouchX = ev.touches[0].clientX - rect.left; waystarTouchY = ev.touches[0].clientY - rect.top;
    }
}
function handleWaystarTouchMove(ev) {
    if (!waystarActive || !waystarTouchActive) return; ev.preventDefault();
    var rect = canvas.getBoundingClientRect();
...
function handleWaystarTouchEnd(ev) {
    if (!waystarTouchActive) return; var still = false;
...
function updateWaystarBoss() {
    ...
    if (waystarState === "phase1") {
        ...
        waystarBoss.rotation += 0.015 * waystarSpeedMult; waystarBoss.pulse += 0.08; waystarBoss.time += 0.02;
    } else if (waystarState === "phase3") {
        waystarSmallBoss.alpha = Math.min(1, waystarSmallBoss.alpha + 0.02);
        var mSpeed = waystarRageMode ? 2.2 : 1.6; mSpeed += waystarEscalationLevel * 0.15;
        ...
        waystarSmallBoss.pulse += (waystarRageMode ? 0.20 : 0.13); waystarSmallBoss.time += (waystarRageMode ? 0.06 : 0.04);
    }
    if (waystarBoss2.active === true) {
        ...
        waystarBoss2.rotation -= 0.02 * waystarSpeedMult; waystarBoss2.pulse += 0.1;
    } else if (waystarBoss2.active === "leaving") {
        waystarBoss2.y -= 4; waystarBoss2.alpha -= 0.02; if (waystarBoss2.alpha <= 0) waystarBoss2.active = false;
    }
}
    ...
    for (var i = waystarAttacks.length - 1; i >= 0; i--) {
        ...
        if (a.type === "meteor" || a.type === "spiral" || a.type === "star_rain" || a.type === "shotgun_bullet") {
            ...
            if (a.y > 520 || a.x < -30 || a.x > 430 || a.life <= 0) {
                if (a.y > 500 && a.type === "meteor") { addWaystarShockwave(a.x, 500, "#ff8800", 50, 12, 3); spawnWaystarParticles(a.x, 500, 12, "#ff8800", 5, "spark"); }
            ...
            if (waystarInvulnTimer <= 0 && Math.sqrt(Math.pow(waystarPlayer.x - a.x, 2) + Math.pow(waystarPlayer.y - a.y, 2)) < a.size + 8) {
                ...
                applyWaystarHit(a.damage, msg); waystarAttacks.splice(i, 1);
            }
        }
        else if (a.type === "laser") {
            if (a.state === "warning") {
                a.warningTimer--; a.chargeParticles++;
                if (a.chargeParticles % 3 === 0) spawnWaystarParticles(a.x + (Math.random() - 0.5) * a.width, 490 - Math.random() * 40, 2, "#ff3333", 4, "spark");
                if (a.warningTimer <= 0) {
                    a.state = "active"; a.activeTimer = a.maxActive;
                    addWaystarFlash(a.x, 250, 140); addWaystarShockwave(a.x, 250, "#ff3333", 160, 22, 5); addWaystarShockwave(a.x, 250, "#ffffff", 100, 16, 3);
                    wsPlaySound(150, 'sawtooth', 0.7, 0.35); waystarShake = 20; waystarScreenFlash = 10; waystarScreenFlashColor = "#ff3333";
                    for (var l = 0; l < 4; l++) addWaystarLightning(a.x, 0, a.x + (Math.random() - 0.5) * 30, 500, "#ffffff", 0.9, 2);
                }
            } else if (a.state === "active") {
                a.activeTimer--;
                if (!a.hit && waystarInvulnTimer <= 0 && Math.abs(waystarPlayer.x - a.x) < a.width / 2 + 8) { a.hit = true; applyWaystarHit(a.damage, "ЛАЗЕР!"); }
                if (a.activeTimer % 2 === 0) spawnWaystarParticles(a.x + (Math.random() - 0.5) * a.width, Math.random() * 500, 3, Math.random() > 0.5 ? "#ffffff" : "#ff3333", 5, "spark");
                if (a.activeTimer % 8 === 0) addWaystarSlash(a.x, Math.random() * 500, Math.PI / 2, 60, 3);
            ...
            if (a.state === "warning") {
                a.warningTimer--;
                if (a.trackTimer > 0) { a.trackTimer--; a.x = waystarSmallBoss.x; a.y = waystarSmallBoss.y; a.angle = Math.atan2(waystarPlayer.y - a.y, waystarPlayer.x - a.x); }
                if (a.warningTimer <= 0) {
                    a.state = "active"; a.activeTimer = a.maxActive; a.x = waystarSmallBoss.x; a.y = waystarSmallBoss.y;
                    addWaystarFlash(a.x, a.y, 180); addWaystarShockwave(a.x, a.y, "#ff00ff", 220, 28, 7); addWaystarShockwave(a.x, a.y, "#ffffff", 160, 22, 4);
                    wsPlaySound(150, 'sawtooth', 0.9, 0.4); waystarShake = 30; waystarScreenFlash = 20; waystarScreenFlashColor = "#ff00ff";
                    for (var sIdx = 0; sIdx < 8; sIdx++) addWaystarSlash(a.x, a.y, a.angle + (Math.random()-0.5)*0.6, 80, 5);
                }
            } else if (a.state === "active") {
                a.activeTimer--;
                if (!a.hit && waystarInvulnTimer <= 0) {
                    var dx = waystarPlayer.x - a.x, dy = waystarPlayer.y - a.y, dist = Math.sqrt(dx*dx + dy*dy);
                    var playerAngle = Math.atan2(dy, dx), angleDiff = Math.abs(playerAngle - a.angle);
                ...
                if (a.activeTimer % 2 === 0) {
                    ...
                    spawnWaystarParticles(a.x + Math.cos(a.angle) * dist2, a.y + Math.sin(a.angle) * dist2, 3, ["#ff00ff", "#ffffff"][Math.floor(Math.random()*2)], 6, "spark");
                }
                if (a.activeTimer <= 0) a.state = "done";
            } else { waystarAttacks.splice(i, 1); continue; }
        }
            ...
            if (a.warning === 0 && !a.hit) {
                // Момент удара созвездия
                waystarShake = 15;
                wsPlaySound(250, 'sawtooth', 0.3, 0.4);
                addWaystarShockwave(a.points[1].x, a.points[1].y, "#00ffff", 200, 20, 5);
            }
            if (a.warning <= 0 && waystarInvulnTimer <= 0 && !a.hit) {
                for (var p = 0; p < a.points.length - 1; p++) {
                    ...
                    if (d < 15) { a.hit = true; applyWaystarHit(a.damage, "СОЗВЕЗДИЕ!"); break; }
                }
            }
        }
    }
    ...
    for (var i = waystarBombs.length - 1; i >= 0; i--) {
        ...
        if (!bomb.exploded) {
            ...
            if (bomb.timer <= 0) {
                ...
                spawnWaystarParticles(bomb.x, bomb.y, 40, "#ff4400", 8, "star");
        ...
        if (t >= 1) waystarDash = null;
            ...
            for (var k = waystarAttacks.length - 1; k >= 0; k--) {
                if (waystarAttacks[k].type === "shotgun_bullet" || waystarAttacks[k].type === "spiral" || waystarAttacks[k].type === "star_rain") {
                    ...
                    if (d < 80) { spawnWaystarParticles(waystarAttacks[k].x, waystarAttacks[k].y, 8, "#ff00ff", 5, "spark"); waystarAttacks.splice(k, 1); }
                }
            }
            if (!waystarDash.hit && waystarInvulnTimer <= 0) {
                var d = Math.sqrt(Math.pow(waystarPlayer.x - newX, 2) + Math.pow(waystarPlayer.y - newY, 2));
                if (d < 25) {
                    waystarDash.hit = true; applyWaystarHit(waystarDash.damage, "РЫВОК!");
                    ...
                    waystarPlayer.x = Math.max(16, Math.min(384, waystarPlayer.x)); waystarPlayer.y = Math.max(80, Math.min(484, waystarPlayer.y));
                    waystarShake = 40; addWaystarShockwave(waystarPlayer.x, waystarPlayer.y, "#ff00ff", 200, 20, 6);
                }
            }
            waystarSmallBoss.x = newX; waystarSmallBoss.y = newY;
            for (var i = 0; i < 4; i++) spawnWaystarParticles(newX + (Math.random()-0.5)*25, newY + (Math.random()-0.5)*25, 1, ["#ff00ff", "#ffffff"][Math.floor(Math.random()*2)], 5, "star");
...
function applyWaystarHit(dmg, textMsg) {
    ...
    waystarVignette = 20;
    spawnWaystarParticles(waystarPlayer.x, waystarPlayer.y, 35, "#ff3333", 8, "mixed");
    ...
    if (waystarState === "phase3" && !waystarRageMode && waystarPlayerHp < waystarPlayerMaxHp * 0.5) {
        ...
        for (var i = 0; i < 60; i++) spawnWaystarParticles(waystarSmallBoss.x, waystarSmallBoss.y, 1, ["#ff0000", "#ff6600", "#ffff00"][Math.floor(Math.random()*3)], 15, "star");
...
function waystarVictory() {
    ...
    waystarAttacks = []; waystarPlayerBullets = []; waystarEnemyBullets = []; waystarBombs = []; waystarDash = null;
...
function grantWaystarReward() {
    if (typeof defeatedBosses !== 'undefined' && Array.isArray(defeatedBosses) && !defeatedBosses.includes(500)) {
        defeatedBosses.push(500); if (typeof saveAll === 'function') saveAll();
    }
    ...
    for (var i = 0; i < 5; i++) { setTimeout(function(idx) { addWaystarShockwave(200, 250, ["#ffffff", "#ffd700", "#ffaa00"][idx % 3], 350, 30, 5); }, i * 100); }
    for (var i = 0; i < 150; i++) spawnWaystarParticles(200 + (Math.random() - 0.5) * 400, 250 + (Math.random() - 0.5) * 400, 1, ["#ffd700", "#ffffff", "#ffaa00"][Math.floor(Math.random() * 3)], 12, "star");
    ...
    setTimeout(function() { stopWaystarFight(); if (typeof currentEnemy !== 'undefined' && currentEnemy) currentEnemy.hp = 0; if (typeof victory === 'function') victory(); }, 3500);
}
...
function waystarDefeat() {
    ...
    for (var i = 0; i < 80; i++) spawnWaystarParticles(waystarPlayer.x, waystarPlayer.y, 1, "#ff3333", 10, "mixed");
...
function waystarRenderLoop() {
    ...
    if (waystarSplitAnim) {
        waystarSplitAnim.timer++; var t = Math.min(1, waystarSplitAnim.timer / waystarSplitAnim.duration);
        ...
        waystarBoss2.alpha = Math.min(1, t * 2); waystarBoss.rotation += 0.03; waystarBoss2.rotation -= 0.03;
    }

    if (waystarEscapeAnim) {
        waystarEscapeAnim.timer++; var et = Math.min(1, waystarEscapeAnim.timer / waystarEscapeAnim.duration);
        waystarBoss2.y = waystarEscapeAnim.startY - et * 300; waystarBoss2.x = waystarEscapeAnim.startX + Math.sin(et * Math.PI * 2) * 30;
        waystarBoss2.size = 40 * (1 - et * 0.6); waystarBoss2.alpha = 1 - et * 0.5; waystarBoss2.rotation += 0.1;
        if (et < 0.9) spawnWaystarParticles(waystarBoss2.x, waystarBoss2.y, 2, "#ff00ff", 3, "star");
    }
    ...
    if (!waystarDialogActive) {
        if (waystarState === "phase1") {
            if (waystarBossHp <= waystarBossMaxHp * 0.5 && waystarBossHp > 0) {
                ...
                for (var i = waystarPlayerBullets.length - 1; i >= 0; i--) {
                    ...
                    if (Math.sqrt(dx * dx + dy * dy) < waystarBoss.size + b.size) {
                        ...
                        spawnWaystarParticles(b.x, b.y, 12, "#ffd700", 6, "spark");
            ...
            for (var i = waystarPlayerBullets.length - 1; i >= 0; i--) {
                ...
                if (Math.sqrt(dx * dx + dy * dy) < waystarSmallBoss.size + b.size) {
                    waystarBossHp -= b.damage; waystarBossFlash = 6;
                    spawnWaystarParticles(b.x, b.y, 15, "#ff00ff", 6, "mixed");
                    addWaystarFlash(b.x, b.y, 20); addWaystarShockwave(b.x, b.y, "#ff00ff", 40, 10, 2);
                    ...
                    wsPlaySound(1400, 'square', 0.08, 0.08);
                    waystarPlayerBullets.splice(i, 1);
                    if (waystarBossHp <= 0) { waystarVictory(); return; } continue;
                }
                if (b.y < -20 || b.life <= 0) waystarPlayerBullets.splice(i, 1);
            }
            updateWaystarBoss(); updateWaystarAttacks();
            ...
            if (waystarEscalationTimer >= 90) {
                waystarEscalationTimer = 0; var hpRatio = waystarBossHp / waystarBossMaxHp; var newLevel = Math.floor((1 - hpRatio) * 4);
                if (newLevel > waystarEscalationLevel) {
                    ...
                    for (var ei = 0; ei < 40; ei++) spawnWaystarParticles(waystarSmallBoss.x, waystarSmallBoss.y, 1, ["#ff00ff", "#ff0000", "#ffffff"][Math.floor(Math.random()*3)], 12, "star");
                }
            }
            var escalationMult = 1 - waystarEscalationLevel * 0.08; if (escalationMult < 0.55) escalationMult = 0.55;
            var aRate = Math.floor((75 / waystarSpeedMult) * escalationMult); if (waystarRageMode) aRate = Math.floor(aRate * 0.75);
            ...
            if (waystarTypeTimer <= 0) {
                ...
                var typeBase = waystarRageMode ? 180 : 260; var typeEsc = typeBase - waystarEscalationLevel * 10;
                if (typeEsc < 130) typeEsc = 130; waystarTypeTimer = Math.floor(typeEsc + Math.random() * 120);
        ...
        if (!waystarChoiceActive) {
            waystarDialogAutoTimer++; if (current && waystarDialogAutoTimer > current.time * 60) waystarProgressDialog();
        }
    }
    ...
    for (var i = waystarParticles.length - 1; i >= 0; i--) {
        ...
        p.x += p.vx; p.y += p.vy; p.vx *= 0.94; p.vy *= 0.94; p.rot += p.rotSpeed; p.life--;
    ...
    ctx.globalCompositeOperation = "lighter"; // Красивое наложение эффектов

    if (waystarState === "phase3") {
        for (var i = 0; i < waystarPhase3Embers.length; i++) {
            var e = waystarPhase3Embers[i];
            ctx.save(); ctx.globalAlpha = Math.max(0, e.life / 100); ctx.fillStyle = e.color; ctx.shadowColor = e.color; ctx.shadowBlur = 10;
            ctx.beginPath(); ctx.arc(e.x, e.y, e.size, 0, Math.PI * 2); ctx.fill(); ctx.restore();
        }
        for (var i = 0; i < waystarPhase3Rings.length; i++) {
            var r = waystarPhase3Rings[i];
            ctx.save(); ctx.globalAlpha = Math.max(0, r.life / r.maxLife) * 0.6; ctx.translate(r.x, r.y); ctx.rotate(r.angle);
            ctx.strokeStyle = r.color; ctx.lineWidth = 2.5; ctx.shadowColor = r.color; ctx.shadowBlur = 15;
            ctx.beginPath(); ctx.arc(0, 0, r.radius, 0, Math.PI * 1.7); ctx.stroke(); ctx.restore();
        }
    }

    if (waystarScreenFlash > 0) { ctx.globalCompositeOperation = "source-over"; ctx.globalAlpha = waystarScreenFlash / 40; ctx.fillStyle = waystarScreenFlashColor; ctx.fillRect(0, 0, 400, 500); ctx.globalAlpha = 1; ctx.globalCompositeOperation = "lighter"; }
    ...
    ctx.globalCompositeOperation = "source-over"; // Возвращаем для рамок
    ...
    ctx.globalCompositeOperation = "source-over"; // Сбрасываем для игрока и UI
    if (waystarState === "phase1" || waystarState === "phase2" || waystarState === "phase3") drawWaystarPlayer();

    // Отрисовка красивых частиц с типами
    for (var i = 0; i < waystarParticles.length; i++) {
        var p = waystarParticles[i];
        ctx.save(); ctx.globalAlpha = Math.max(0, p.life / p.maxLife) * 0.8; ctx.fillStyle = p.color;
        ctx.translate(p.x, p.y); ctx.rotate(p.rot);
        if (p.isSplitPiece) { ctx.fillRect(-p.size, -p.size, p.size * 2, p.size * 2); }
        else if (p.shape === "star") {
            ctx.beginPath();
            for(var j=0; j<5; j++) {
                ctx.lineTo(Math.cos((18 + j * 72) * Math.PI / 180) * p.size * 1.5, -Math.sin((18 + j * 72) * Math.PI / 180) * p.size * 1.5);
                ctx.lineTo(Math.cos((54 + j * 72) * Math.PI / 180) * p.size * 0.6, -Math.sin((54 + j * 72) * Math.PI / 180) * p.size * 0.6);
            }
    ...
    for (var i = 0; i < waystarTexts.length; i++) { var t = waystarTexts[i]; ctx.globalAlpha = Math.min(1, t.life / 30); ctx.font = "bold 18px monospace"; ctx.textAlign = "center"; ctx.fillStyle = t.color; ctx.shadowColor = t.color; ctx.shadowBlur = 8; ctx.fillText(t.text, t.x, t.y); ctx.shadowBlur = 0; }
    ctx.globalAlpha = 1;
    ...
    if (waystarRageMode && waystarState === "phase3") {
        ctx.save(); ctx.font = "bold 16px monospace"; ctx.textAlign = "center"; ctx.fillStyle = "#ff0000"; ctx.shadowColor = "#ff0000"; ctx.shadowBlur = 15;
        var rageGlow = 0.5 + Math.abs(Math.sin(performance.now() / 100)) * 0.5; ctx.globalAlpha = rageGlow; ctx.fillText("🔥 RAGE MODE 🔥", 200, 470); ctx.restore();
    }
    ...
    if (waystarState === "phase3" && waystarEscalationLevel > 0) {
        ...
        ctx.fillStyle = escColors[Math.min(waystarEscalationLevel, 4)]; ctx.shadowColor = ctx.fillStyle; ctx.shadowBlur = 12;
        ctx.globalAlpha = 0.7 + Math.abs(Math.sin(performance.now() / 200)) * 0.3; ctx.fillText(escName, 200, 455); ctx.restore();
    }
...
function drawWaystarBoss2() {
    ...
    ctx.save(); ctx.translate(b.x, b.y); ctx.globalAlpha = b.alpha;
    var glow = ctx.createRadialGradient(0, 0, 5, 0, 0, size * 4.0);
    glow.addColorStop(0, "rgba(255, 0, 255, 0.8)"); glow.addColorStop(0.5, "rgba(200, 0, 255, 0.4)"); glow.addColorStop(1, "transparent");
    ctx.fillStyle = glow; ctx.beginPath(); ctx.arc(0, 0, size * 4.0, 0, Math.PI * 2); ctx.fill();
    ctx.rotate(-b.rotation); ctx.fillStyle = "#ff00ff"; ctx.shadowColor = "#ff55ff"; ctx.shadowBlur = 15;
    ctx.beginPath(); for (var i = 0; i < 16; i++) { var ang = (i / 16) * Math.PI * 2; var r = (i % 2 === 0) ? size : size * 0.4; ctx.lineTo(Math.cos(ang)*r, Math.sin(ang)*r); } ctx.closePath(); ctx.fill();
    ctx.strokeStyle = "#ffffff"; ctx.lineWidth = 2; ctx.stroke();
    ctx.fillStyle = "#ffffff"; ctx.beginPath(); ctx.arc(0, 0, size * 0.3, 0, Math.PI * 2); ctx.fill();
    ctx.restore();
}
...
function drawWaystarSmallBoss() {
    var b = waystarSmallBoss; var pulse = 1 + Math.sin(b.pulse) * 0.15; var size = b.size * pulse;
    ctx.save(); ctx.globalAlpha = b.alpha;
    for (var i = 0; i < b.trail.length; i++) {
        var tr = b.trail[i]; ctx.globalAlpha = (tr.life / 18) * 0.3; ctx.fillStyle = waystarRageMode ? "#ff0000" : "#ff00ff";
        ctx.beginPath(); ctx.arc(tr.x, tr.y, size * (tr.life/18), 0, Math.PI * 2); ctx.fill();
    }
    ctx.globalAlpha = b.alpha; ctx.translate(b.x, b.y);
    var g = ctx.createRadialGradient(0,0,0,0,0,size*3); g.addColorStop(0, waystarRageMode ? "#ff0000" : "#ff00ff"); g.addColorStop(1, "transparent");
    ctx.fillStyle = g; ctx.beginPath(); ctx.arc(0,0,size*3,0,Math.PI*2); ctx.fill();
    ctx.rotate(b.rotation); ctx.fillStyle = waystarRageMode ? "#ff5555" : "#ff55ff"; ctx.shadowColor = ctx.fillStyle; ctx.shadowBlur = 20;
    ctx.beginPath(); for(var i=0; i<8; i++) { var a = (i/8)*Math.PI*2; var r = (i%2===0)?size:size*0.5; ctx.lineTo(Math.cos(a)*r, Math.sin(a)*r); } ctx.closePath(); ctx.fill();
...
function drawWaystarPhase1Attacks() {
    for (var i = 0; i < waystarAttacks.length; i++) {
        ...
        if (a.type === "constellation") {
            ...
            for (var p = 1; p < a.points.length; p++) ctx.lineTo(a.points[p].x, a.points[p].y);
            if (a.warning > 0) {
                ctx.strokeStyle = "rgba(0, 255, 255, " + (0.3 + Math.sin(a.animTime * 0.3) * 0.3) + ")";
                ctx.lineWidth = 2; ctx.setLineDash([10, 10]); ctx.stroke();
            } else {
                ctx.strokeStyle = "#00ffff"; ctx.lineWidth = 6 + Math.random() * 4; ctx.shadowColor = "#00ffff"; ctx.shadowBlur = 15;
                ctx.stroke();
            }
            ctx.fillStyle = "#ffffff"; ctx.shadowColor = "#ffffff"; ctx.shadowBlur = 10;
            for(var p=0; p<a.points.length; p++) { ctx.beginPath(); ctx.arc(a.points[p].x, a.points[p].y, 5, 0, Math.PI*2); ctx.fill(); }
            ctx.restore();
        } else if (a.type === "laser" || a.type === "giant_laser") {
            ctx.save();
            if (a.state === "warning") {
                ctx.fillStyle = a.type === "laser" ? "rgba(255,0,0,0.3)" : "rgba(255,0,255,0.3)";
                if (a.type === "giant_laser") { ctx.translate(a.x, a.y); ctx.rotate(a.angle); ctx.fillRect(0, -a.width/2, 600, a.width); }
                else ctx.fillRect(a.x - a.width/2, 0, a.width, 500);
            } else if (a.state === "active") {
                var c1 = a.type === "laser" ? "#ff0000" : "#ff00ff", c2 = "#ffffff";
                if (a.type === "giant_laser") {
                    ctx.translate(a.x, a.y); ctx.rotate(a.angle);
                    var grad = ctx.createLinearGradient(0, -a.width/2, 0, a.width/2); grad.addColorStop(0, "transparent"); grad.addColorStop(0.2, c1); grad.addColorStop(0.5, c2); grad.addColorStop(0.8, c1); grad.addColorStop(1, "transparent");
                    ctx.fillStyle = grad; ctx.shadowColor = c1; ctx.shadowBlur = 20; ctx.fillRect(0, -a.width/2, 600, a.width);
                } else {
                    var grad = ctx.createLinearGradient(a.x - a.width/2, 0, a.x + a.width/2, 0); grad.addColorStop(0, "transparent"); grad.addColorStop(0.2, c1); grad.addColorStop(0.5, c2); grad.addColorStop(0.8, c1); grad.addColorStop(1, "transparent");
                    ctx.fillStyle = grad; ctx.shadowColor = c1; ctx.shadowBlur = 20; ctx.fillRect(a.x - a.width/2, 0, a.width, 500);
                }
            }
            ctx.restore();
        } else {
            ...
            if (a.type === "meteor") {
                var g = ctx.createRadialGradient(0,0,0,0,0,a.size*2); g.addColorStop(0, "#ffaa00"); g.addColorStop(1, "transparent");
                ctx.fillStyle = g; ctx.beginPath(); ctx.arc(0,0,a.size*2,0,Math.PI*2); ctx.fill();
                ctx.fillStyle = "#ff4400"; ctx.beginPath(); ctx.arc(0,0,a.size,0,Math.PI*2); ctx.fill();
                ctx.fillStyle = "#ffff00"; ctx.beginPath(); ctx.arc(0,0,a.size*0.6,0,Math.PI*2); ctx.fill();
            } else if (a.type === "spiral" || a.type === "shotgun_bullet") {
                ctx.fillStyle = waystarState === "phase3" ? "#ff00ff" : "#ff00ff"; ctx.shadowColor = ctx.fillStyle; ctx.shadowBlur = 10;
                ctx.beginPath(); ctx.arc(0,0,a.size,0,Math.PI*2); ctx.fill();
                ctx.fillStyle = "#ffffff"; ctx.beginPath(); ctx.arc(0,0,a.size*0.4,0,Math.PI*2); ctx.fill();
            } else if (a.type === "star_rain") {
                ctx.fillStyle = "#ffffff"; ctx.shadowColor = "#ffffff"; ctx.shadowBlur = 10;
                ctx.beginPath(); for(var j=0; j<5; j++) { ctx.lineTo(Math.cos((18+j*72)*Math.PI/180)*a.size, -Math.sin((18+j*72)*Math.PI/180)*a.size); ctx.lineTo(Math.cos((54+j*72)*Math.PI/180)*a.size*0.4, -Math.sin((54+j*72)*Math.PI/180)*a.size*0.4); } ctx.closePath(); ctx.fill();
            }
            ctx.restore();
        }
    }
...
function drawWaystarPlayerBullets() {
    for (var i = 0; i < waystarPlayerBullets.length; i++) {
        ...
        ctx.fillStyle = "#ffff00"; ctx.shadowColor = "#ffaa00"; ctx.shadowBlur = 8;
        ctx.beginPath(); ctx.arc(0, 0, b.size, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = "#ffffff"; ctx.beginPath(); ctx.arc(0, 0, b.size*0.5, 0, Math.PI * 2); ctx.fill();
        ctx.restore();
    }
}
...
function drawWaystarEnemyBullets() {
    for (var i = 0; i < waystarEnemyBullets.length; i++) {
        ...
        ctx.beginPath(); ctx.arc(0, 0, b.size, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = "#ffffff"; ctx.beginPath(); ctx.arc(0, 0, b.size*0.4, 0, Math.PI * 2); ctx.fill();
        ctx.restore();
    }
}
...
function drawWaystarPieces() {
    for (var i = 0; i < waystarPieces.length; i++) {
        ...
        var s = p.size * (1 + Math.sin(p.pulse) * 0.2);
        ctx.fillStyle = (p.hp === 2) ? "#ffd700" : "#ff00ff";
        ctx.shadowColor = ctx.fillStyle; ctx.shadowBlur = 10;
        ctx.beginPath(); for(var j=0; j<5; j++) { ctx.lineTo(Math.cos((18+j*72)*Math.PI/180)*s, -Math.sin((18+j*72)*Math.PI/180)*s); ctx.lineTo(Math.cos((54+j*72)*Math.PI/180)*s*0.4, -Math.sin((54+j*72)*Math.PI/180)*s*0.4); } ctx.closePath(); ctx.fill();
        ctx.restore();
    }
}
...
function drawWaystarBombs() {
    for (var i = 0; i < waystarBombs.length; i++) {
        ...
        if (!b.exploded) {
            ctx.save(); ctx.translate(b.x, b.y);
            ctx.fillStyle = "rgba(255, 0, 0, 0.2)"; ctx.beginPath(); ctx.arc(0, 0, b.radius, 0, Math.PI * 2); ctx.fill();
            ctx.strokeStyle = "rgba(255, 0, 0, 0.8)"; ctx.lineWidth = 2;
            var fillRadius = b.radius * (1 - b.timer / b.maxTimer);
            ctx.beginPath(); ctx.arc(0, 0, fillRadius, 0, Math.PI * 2); ctx.stroke();
            ctx.restore();
        }
    }
}
...
function drawWaystarDash() {
    if (waystarDash) {
        ctx.save();
        ctx.strokeStyle = "rgba(255, 0, 255, 0.3)"; ctx.lineWidth = 40;
        ctx.beginPath(); ctx.moveTo(waystarDash.startX, waystarDash.startY); ctx.lineTo(waystarDash.targetX, waystarDash.targetY); ctx.stroke();
        ctx.strokeStyle = "rgba(255, 255, 255, 0.8)"; ctx.lineWidth = 2; ctx.setLineDash([10, 10]);
        ctx.beginPath(); ctx.moveTo(waystarDash.startX, waystarDash.startY); ctx.lineTo(waystarDash.targetX, waystarDash.targetY); ctx.stroke();
        ctx.restore();
    }
}
...
function drawWaystarPlayer() {
    ...
    if (waystarInvulnTimer > 0 && Math.floor(Date.now() / 100) % 2 === 0) ctx.globalAlpha = 0.5;
    ctx.fillStyle = "#00ffff"; ctx.shadowColor = "#00ffff"; ctx.shadowBlur = 10;
    ctx.beginPath(); ctx.moveTo(0, -12); ctx.lineTo(10, 10); ctx.lineTo(0, 5); ctx.lineTo(-10, 10); ctx.closePath(); ctx.fill();
    ctx.fillStyle = "#ffffff"; ctx.beginPath(); ctx.moveTo(0, -5); ctx.lineTo(4, 6); ctx.lineTo(-4, 6); ctx.closePath(); ctx.fill();
    if (waystarKeys.w || waystarKeys.arrowup || waystarTouchActive) {
        ctx.fillStyle = "#ffaa00"; ctx.beginPath(); ctx.moveTo(0, 15 + Math.random()*5); ctx.lineTo(4, 6); ctx.lineTo(-4, 6); ctx.closePath(); ctx.fill();
    }
    ctx.restore();
}
...
function drawWaystarHpBars() {
    ctx.save();
    ctx.fillStyle = "rgba(0,0,0,0.5)"; ctx.fillRect(10, 10, 380, 14);
    ctx.fillStyle = "rgba(0,0,0,0.5)"; ctx.fillRect(10, 30, 380, 14);
    var bossP = Math.max(0, waystarBossHp / waystarBossMaxHp);
    var plP = Math.max(0, waystarPlayerHp / waystarPlayerMaxHp);
    ctx.fillStyle = waystarState === "phase3" ? "#ff00ff" : "#ffd700"; ctx.fillRect(12, 12, 376 * bossP, 10);
    ctx.fillStyle = "#00ff00"; if (plP < 0.3) ctx.fillStyle = "#ff0000"; ctx.fillRect(12, 32, 376 * plP, 10);
    ctx.font = "10px monospace"; ctx.fillStyle = "#ffffff"; ctx.textAlign = "center";
    ctx.fillText(waystarState === "phase3" ? "ПУТЕВОДНАЯ ЗВЕЗДА (ИСТИННАЯ)" : "ПУТЕВОДНАЯ ЗВЕЗДА", 200, 21);
    ctx.fillText("ИГРОК", 200, 41);
    ctx.restore();
}

function drawWaystarDialog() {
    ctx.save();
    ctx.fillStyle = "rgba(0, 0, 0, 0.85)"; ctx.strokeStyle = "#ffd700"; ctx.lineWidth = 3;
    ctx.fillRect(10, 380, 380, 110); ctx.strokeRect(10, 380, 380, 110);
    var current = waystarDialogQueue[waystarDialogStep];
    if (current) {
        ctx.font = "bold 16px monospace"; ctx.fillStyle = "#ffd700"; ctx.textAlign = "left";
        ctx.fillText(current.speaker, 20, 405);
        ctx.font = "14px monospace"; ctx.fillStyle = "#ffffff";
        var words = current.text.split(" "), line = "", y = 430;
        for (var n = 0; n < words.length; n++) {
            var testLine = line + words[n] + " ";
            if (ctx.measureText(testLine).width > 340 && n > 0) { ctx.fillText(line, 20, y); line = words[n] + " "; y += 20; }
            else { line = testLine; }
        }
        ...
        if (current.choice) {
            ...
            for (var i = 0; i < 3; i++) {
                var by = startY + i * (btnH + gap);
                ctx.fillStyle = "rgba(0, 0, 0, 0.9)"; ctx.fillRect(btnX, by, btnW, btnH);
                ctx.strokeStyle = "#ff00ff"; ctx.strokeRect(btnX, by, btnW, btnH);
                ctx.font = "12px monospace"; ctx.fillStyle = "#ffffff"; ctx.textAlign = "center";
                
                var cWords = choices[i].text.split(" "), cLine = "", cy = by + 22;
                for (var j = 0; j < cWords.length; j++) {
                    var cTest = cLine + cWords[j] + " ";
                    if (ctx.measureText(cTest).width > btnW - 20 && j > 0) { ctx.fillText(cLine, btnX + btnW/2, cy); cLine = cWords[j] + " "; cy += 16; }
                    else { cLine = cTest; }
                }
                ctx.fillText(cLine, btnX + btnW/2, cy);
            }
        }
    }
    ctx.restore();
