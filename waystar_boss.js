// ============================================================
// ПУТЕВОДНАЯ ЗВЕЗДА — БОСС 500 ВОЛНЫ v10.5
// ФИКС: выбор теперь работает сразу при клике + защита от ошибок
// ============================================================
...
function addWaystarLightning(x1, y1, x2, y2, color, alpha, width) {
    ...
    for (var i = 1; i < steps; i++) { var t = i / steps; pts.push({ x: x1 + (x2 - x1) * t + (Math.random() - 0.5) * 30, y: y1 + (y2 - y1) * t + (Math.random() - 0.5) * 30 }); }
    waystarLightningBolts.push({ x1: x1, y1: y1, x2: x2, y2: y2, pts: pts, color: color, alpha: alpha, life: 15, maxLife: 15, width: width });
}
function addWaystarSlash(x, y, angle, length, width) { waystarSlashMarks.push({ x: x, y: y, angle: angle, length: length, width: width || 4, life: 25, maxLife: 25 }); }
function addWaystarFlash(x, y, size) { waystarFlashBursts.push({ x: x, y: y, size: size || 40, life: 20, maxLife: 20 }); }
function spawnWaystarParticles(x, y, count, color, speed) {
    for (var i = 0; i < count; i++) { var a = Math.random() * Math.PI * 2, s = speed * (0.3 + Math.random()); waystarParticles.push({ x: x, y: y, vx: Math.cos(a) * s, vy: Math.sin(a) * s, life: 25 + Math.random() * 20, maxLife: 45, color: color, size: 1.5 + Math.random() * 3 }); }
}
...
function updateWaystarAmbient() {
    ...
    waystarAmbientTimer++; waystarPhase3AuraPulse += 0.15;
    var escMult = 1 + waystarEscalationLevel * 0.2;
    if (waystarAmbientTimer % 4 === 0) { waystarPhase3Rings.push({ x: waystarSmallBoss.x, y: waystarSmallBoss.y, radius: 30, maxRadius: 90 + waystarEscalationLevel * 12, angle: Math.random() * Math.PI * 2, rotSpeed: (Math.random() - 0.5) * 0.25, life: 30, maxLife: 30, color: Math.random() > 0.5 ? "#ff00ff" : "#ffffff" }); }
    if (waystarAmbientTimer % Math.max(2, Math.floor(3 / escMult)) === 0) { waystarPhase3Embers.push({ x: Math.random() * 400, y: 500 + Math.random() * 40, vx: (Math.random() - 0.5) * 1.2, vy: -(1.5 + Math.random() * 2) * escMult, size: 1 + Math.random() * 2.5, life: 70 + Math.random() * 40, color: Math.random() > 0.5 ? "#ff00ff" : "#ffaa00" }); }
    if (waystarAmbientTimer % Math.max(8, Math.floor(24 / escMult)) === 0) { var ang = Math.random() * Math.PI * 2; addWaystarLightning(waystarSmallBoss.x, waystarSmallBoss.y, waystarSmallBoss.x + Math.cos(ang) * (100 + Math.random() * 80), waystarSmallBoss.y + Math.sin(ang) * (100 + Math.random() * 80), Math.random() > 0.5 ? "#ff00ff" : "#ffffff", 0.9, 2.5); }
    if (waystarAmbientTimer % Math.max(5, Math.floor(14 / escMult)) === 0) { addWaystarSlash(30 + Math.random() * 340, 60 + Math.random() * 380, Math.random() * Math.PI * 2, 30 + Math.random() * 60, 3); }
    if (waystarAmbientTimer % Math.max(6, Math.floor(18 / escMult)) === 0) { addWaystarFlash(waystarSmallBoss.x + (Math.random() - 0.5) * 80, waystarSmallBoss.y + (Math.random() - 0.5) * 80, 25 + Math.random() * 40); }
    if (waystarAmbientTimer % Math.max(20, Math.floor(70 / escMult)) === 0) { waystarScreenFlash = 8; waystarScreenFlashColor = waystarEscalationLevel >= 3 ? "#ff0000" : "#ff00ff"; }
    if (waystarAmbientTimer % 2 === 0) { spawnWaystarParticles(waystarSmallBoss.x + (Math.random() - 0.5) * 40, waystarSmallBoss.y + (Math.random() - 0.5) * 40, 1, ["#ff00ff", "#ffaa00", "#ffffff"][Math.floor(Math.random() * 3)], 2.5); }
...
function startWaystarFight() {
    ...
    console.log("[WAYSTAR] Старт боя. Модер:", isWaystarModerActive());
    waystarActive = true;
...
function initWaystarBgStars() {
    ...
    for (var i = 0; i < 120; i++) { waystarBgStars.push({ x: Math.random() * 400, y: Math.random() * 500, size: 0.5 + Math.random() * 2.2, speed: 0.1 + Math.random() * 0.4, alpha: 0.2 + Math.random() * 0.8, twinkle: Math.random() * Math.PI * 2 }); }
    var colors = ["#1a0a2e", "#0a1a3a", "#2a0a2a", "#0a1a2a"];
    for(var j=0; j<4; j++) { waystarNebulas.push({ x: Math.random() * 400, y: Math.random() * 500, radius: 100 + Math.random() * 150, color: colors[Math.floor(Math.random() * colors.length)], speed: 0.2 + Math.random() * 0.3, angle: Math.random() * Math.PI * 2, rotSpeed: (Math.random() - 0.5) * 0.01 }); }
}
...
function startWaystarDialog() {
    ...
    for (var i = 0; i < waystarDialogQueue.length; i++) { waystarDialogQueue[i].time = 2 + waystarDialogQueue[i].text.length * 0.03; }
}
...
function selectWaystarChoice(choiceId) {
    ...
    for (var i = 0; i < responses.length; i++) { responses[i].time = 2 + responses[i].text.length * 0.04; waystarDialogQueue.push(responses[i]); }
    waystarDialogStep++;
    waystarDialogAutoTimer = 0;
...
function waystarSpawnAttack() {
    ...
    if (type === 0) {
        ...
        for (var i = 0; i < count; i++) { waystarAttacks.push({ type: "meteor", x: 20 + Math.random() * 360, y: -40 - Math.random()*50, vx: (Math.random() - 0.5) * 2 * s, vy: (3.5 + Math.random() * 2.5) * s, size: 12 + Math.random() * 6, rotation: Math.random() * Math.PI * 2, rotSpeed: (Math.random() - 0.5) * 0.3, damage: 12, life: 300, trail: [] }); }
        addWaystarShockwave(waystarBoss.x, waystarBoss.y, "#ff8800", 100, 15, 3);
        wsPlaySound(200, 'square', 0.3, 0.15);
    } else if (type === 1) {
        var count = isSecond ? 20 : 14; var baseAngle = Math.random() * Math.PI * 2;
        for (var i = 0; i < count; i++) { var ang = baseAngle + (i / count) * Math.PI * 4; var delay = i * 4; (function(a, d) { setTimeout(function() { if (!waystarActive || waystarState !== "phase1") return; waystarAttacks.push({ type: "spiral", x: waystarBoss.x, y: waystarBoss.y, vx: Math.cos(a) * 4 * s, vy: Math.sin(a) * 4 * s, size: 7, damage: 10, life: 250, trail: [] }); }, d); })(ang, delay); }
        addWaystarShockwave(waystarBoss.x, waystarBoss.y, "#ff00ff", 80, 12, 3);
        wsPlaySound(400, 'sine', 0.5, 0.12);
    } else if (type === 2) {
        if (Math.random() < 0.33) { var targetX = 60 + Math.random() * 280; waystarAttacks.push({ type: "laser", state: "warning", x: targetX, width: 55, warningTimer: 45, activeTimer: 0, maxActive: 70, damage: 18, hit: false, chargeParticles: 0 }); }
        wsPlaySound(300, 'square', 0.3, 0.15);
    } else if (type === 3) {
        var points = []; var count2 = isSecond ? 6 : 5;
        for (var i = 0; i < count2; i++) { points.push({ x: 40 + Math.random() * 320, y: 80 + Math.random() * 340, pulse: Math.random() * Math.PI * 2, sparkTimer: 0 }); }
        waystarAttacks.push({ type: "constellation", points: points, timer: 180, warning: 60, damage: 22, hit: false, activePulse: 0, hitFlash: 0, lastHitTime: 0 });
        addWaystarShockwave(200, 250, "#00ffff", 140, 18, 3);
        wsPlaySound(500, 'sine', 0.5, 0.18);
    } else if (type === 4) {
        var count3 = isSecond ? 14 : 10;
        for (var i = 0; i < count3; i++) { var side = Math.random() > 0.5 ? 1 : -1; waystarAttacks.push({ type: "star_rain", x: side > 0 ? -30 : 430, y: Math.random() * 200 - 100, vx: side > 0 ? 5 * s : -5 * s, vy: 3 * s, size: 10, rotation: Math.random() * Math.PI * 2, rotSpeed: (Math.random() - 0.5) * 0.3, damage: 11, life: 250, trail: [] }); }
...
function waystarTriggerSplit() {
    ...
    for (var i = 0; i < 6; i++) { setTimeout(function(idx) { addWaystarShockwave(waystarBoss.x, waystarBoss.y, ["#ffffff", "#ffd700", "#ff00ff", "#ff4400"][idx%4], 350, 30, 6); }, i * 100); }
    for (var i = 0; i < 20; i++) { var ang = (i / 20) * Math.PI * 2 + Math.random() * 0.3; addWaystarLightning(waystarBoss.x, waystarBoss.y, waystarBoss.x + Math.cos(ang) * 200, waystarBoss.y + Math.sin(ang) * 200, i % 2 === 0 ? "#ffffff" : "#ffd700", 1.0, 3); }
...
function waystarStartSplit2Animation() {
    ...
    setTimeout(function() {
        ...
        wsPlaySound(200, 'sawtooth', 1.5, 0.4); setTimeout(function() { wsPlaySound(100, 'sawtooth', 1.2, 0.35); }, 200);
        for (var i = 0; i < 60; i++) { var ang = (i / 60) * Math.PI * 2 + Math.random() * 0.2; var len = 150 + Math.random() * 150; addWaystarLightning(waystarBoss.x, waystarBoss.y, waystarBoss.x + Math.cos(ang) * len, waystarBoss.y + Math.sin(ang) * len, i % 3 === 0 ? "#ffffff" : (i % 3 === 1 ? "#ffd700" : "#ff00ff"), 1.0, 3); }
        for (var i = 0; i < 8; i++) { setTimeout(function(idx) { addWaystarShockwave(waystarBoss.x, waystarBoss.y, ["#ffffff", "#ffd700", "#ff00ff", "#ff4400"][idx%4], 400, 35, 7); }, i * 80); }
        for (var i = 0; i < 60; i++) { var ang = (i / 60) * Math.PI * 2; var dist = 100 + Math.random() * 60; waystarParticles.push({ x: waystarBoss.x, y: waystarBoss.y, vx: Math.cos(ang) * 3, vy: Math.sin(ang) * 3, targetX: waystarBoss.x + Math.cos(ang) * dist, targetY: waystarBoss.y + Math.sin(ang) * dist, isSplitPiece: true, life: 90, maxLife: 90, color: ["#ffd700", "#ff00ff", "#ffffff"][i % 3], size: 4 + Math.random() * 3 }); }
...
function updateWaystarSpaceInvaders() {
    ...
    if (waystarInvaderShootTimer >= shootRate) { waystarInvaderShootTimer = 0; var shooter = alivePieces[Math.floor(Math.random() * alivePieces.length)]; waystarEnemyBullets.push({ x: shooter.x, y: shooter.y + 15, vy: 5 + Math.random() * 3, size: 8, life: 200, trail: [] }); wsPlaySound(400, 'sawtooth', 0.1, 0.1); }
    for (var i = waystarPlayerBullets.length - 1; i >= 0; i--) {
        ...
        for (var j = 0; j < waystarPieces.length; j++) {
            ...
            if (Math.abs(b.x - p.x) < p.size + 5 && Math.abs(b.y - p.y) < p.size + 5) {
                ...
                if (p.hp <= 0) { p.alive = false; waystarPiecesAlive--; addWaystarShockwave(p.x, p.y, "#ff00ff", 60, 12, 3); addWaystarFlash(p.x, p.y, 25); spawnWaystarParticles(p.x, p.y, 25, "#ff00ff", 6); wsPlaySound(200, 'sawtooth', 0.3, 0.2); }
...
function waystarStartReturning() {
    ...
    for (var i = 0; i < 30; i++) { var ang = (i / 30) * Math.PI * 2; addWaystarLightning(200, 100, 200 + Math.cos(ang) * 250, 100 + Math.sin(ang) * 250, "#ff00ff", 0.8, 3); }
...
function waystarStartPhase3() {
    ...
    wsPlaySound(200, 'sawtooth', 1.0, 0.3); setTimeout(function() { wsPlaySound(400, 'square', 0.5, 0.25); }, 200); setTimeout(function() { wsPlaySound(800, 'sawtooth', 0.8, 0.3); }, 400);
    waystarShake = 40; waystarScreenFlash = 30; waystarScreenFlashColor = "#ff00ff"; waystarScreenDistort = 35;
    for (var i = 0; i < 6; i++) { setTimeout(function(idx) { addWaystarShockwave(200, 100, ["#ff00ff", "#ffd700", "#ffffff", "#ff00ff"][idx%4], 350, 30, 6); }, i * 100); }
    for (var i = 0; i < 40; i++) { var ang = (i / 40) * Math.PI * 2; addWaystarLightning(200, 100, 200 + Math.cos(ang) * 250, 100 + Math.sin(ang) * 250, i % 2 === 0 ? "#ff00ff" : "#ffffff", 1.0, 3); }
...
function waystarSpawnPhase3Attack() {
    var type = waystarAttackType; var s = waystarSpeedMult * (waystarRageMode ? 1.05 : 0.85);
    if (type === 0) {
        var count = 6 + waystarEscalationLevel; if (waystarRageMode) count += 2;
        var baseAngle = Math.atan2(waystarPlayer.y - waystarSmallBoss.y, waystarPlayer.x - waystarSmallBoss.x);
        for (var i = 0; i < count; i++) { var offset = (i - (count-1)/2) * 0.12; var ang = baseAngle + offset; waystarAttacks.push({ type: "shotgun_bullet", x: waystarSmallBoss.x, y: waystarSmallBoss.y, vx: Math.cos(ang) * 3.8 * s, vy: Math.sin(ang) * 3.8 * s, size: 6, damage: 9, life: 250, trail: [] }); }
        ...
        var bombCount = 2 + Math.floor(waystarEscalationLevel / 2); if (waystarRageMode) bombCount += 1;
        for (var k = 0; k < bombCount; k++) { var bx = 60 + Math.random() * 280; var by = 80 + Math.random() * 300; waystarBombs.push({ x: bx, y: by, timer: 150, maxTimer: 150, radius: 70, damage: 25, exploded: false, explosionTimer: 0, sunPhase: "big", sunScale: 1.0, sunColor: "#ffdd00", phaseTimer: 0 }); }
        wsPlaySound(500, 'sine', 0.3, 0.15);
    } else if (type === 2) {
        if (!waystarDash) { var dx = waystarPlayer.x - waystarSmallBoss.x; var dy = waystarPlayer.y - waystarSmallBoss.y; var len = Math.sqrt(dx * dx + dy * dy) || 1; waystarDash = { x: waystarSmallBoss.x, y: waystarSmallBoss.y, startX: waystarSmallBoss.x, startY: waystarSmallBoss.y, targetX: waystarPlayer.x, targetY: waystarPlayer.y, dirX: dx / len, dirY: dy / len, progress: 0, duration: 55, damage: 20, hit: false }; }
        wsPlaySound(150, 'sawtooth', 0.5, 0.25);
    } else if (type === 3) {
        waystarAttacks.push({ type: "giant_laser", state: "warning", x: waystarSmallBoss.x, y: waystarSmallBoss.y, angle: Math.atan2(waystarPlayer.y - waystarSmallBoss.y, waystarPlayer.x - waystarSmallBoss.x), width: 55, warningTimer: 70, activeTimer: 0, maxActive: 55, damage: 25, hit: false, trackTimer: 45 });
        wsPlaySound(400, 'square', 0.4, 0.2);
    } else if (type === 4) {
        var count = 20 + waystarEscalationLevel * 2; if (waystarRageMode) count += 4;
        for (var i = 0; i < count; i++) {
            var baseAng = (i / count) * Math.PI * 4; var delay = i * 6;
            (function(a, d) { setTimeout(function() { if (!waystarActive || waystarState !== "phase3") return; waystarAttacks.push({ type: "spiral", x: waystarSmallBoss.x, y: waystarSmallBoss.y, vx: Math.cos(a) * 2.2 * s, vy: Math.sin(a) * 2.2 * s, size: 6, damage: 9, life: 250, trail: [] }); }, d); })(baseAng, delay);
            (function(a, d) { setTimeout(function() { if (!waystarActive || waystarState !== "phase3") return; waystarAttacks.push({ type: "spiral", x: waystarSmallBoss.x, y: waystarSmallBoss.y, vx: Math.cos(-a) * 2.2 * s, vy: Math.sin(-a) * 2.2 * s, size: 6, damage: 9, life: 250, trail: [] }); }, d); })(baseAng, delay);
        }
        wsPlaySound(600, 'sine', 0.5, 0.15);
    } else if (type === 5) {
        var mineCount = 5 + waystarEscalationLevel; if (waystarRageMode) mineCount += 2;
        for (var i = 0; i < mineCount; i++) { var bx = 40 + Math.random() * 320; var by = 80 + Math.random() * 320; waystarBombs.push({ x: bx, y: by, timer: 110, maxTimer: 110, radius: 40, damage: 12, exploded: false, explosionTimer: 0, small: true, sunPhase: "big", sunScale: 0.7, sunColor: "#ffdd00", phaseTimer: 0 }); }
        wsPlaySound(450, 'sine', 0.2, 0.12);
    }
}

// ★★★ КЛИК — ЗДЕСЬ ГЛАВНЫЙ ФИКС: СРАЗУ ПЕРЕКЛЮЧАЕМ ФАЗУ ★★★
function handleWaystarClick(ev) {
    if (!waystarActive) return;

    // ★ ФИНАЛЬНАЯ СЦЕНА: выбор kill/spare — ПЕРЕКЛЮЧАЕМ ФАЗУ СРАЗУ ★
    if (waystarFinalActive && waystarFinalPhase === "choice" && waystarChoiceSelection === null) {
        ...
        if (mx > btnX && mx < btnX + btnW) {
            if (my > killY && my < killY + btnH) {
                ...
                waystarFinalPhase = "kill_flash";   // ★ СРАЗУ ПЕРЕКЛЮЧАЕМ ★
                waystarFinalTimer = 0;
                wsPlaySound(300, 'sawtooth', 0.8, 0.4);
                waystarScreenFlash = 40; waystarScreenFlashColor = "#ffffff";
                waystarShake = 30;
                console.log("[WAYSTAR] Выбор: УБИТЬ → фаза kill_flash");
                return;
            }
            if (my > spareY && my < spareY + btnH) {
                waystarChoiceSelection = "spare";
                waystarFinalPhase = "spare_thanks";   // ★ СРАЗУ ПЕРЕКЛЮЧАЕМ ★
                waystarFinalTimer = 0;
                wsPlaySound(500, 'sine', 0.6, 0.25);
                console.log("[WAYSTAR] Выбор: ПОЩАДИТЬ → фаза spare_thanks");
                return;
            }
        }
    }
...
function handleWaystarTouchStart(ev) {
    if (!waystarActive) return;
    ev.preventDefault();
    if (waystarDialogActive || (waystarFinalActive && waystarFinalPhase === "choice")) { var t = ev.touches[0]; handleWaystarClick({ clientX: t.clientX, clientY: t.clientY }); return; }
...
function updateWaystarPlayer() {
    if (waystarState === "phase2") {
        ...
        waystarPlayer.x += mx * 6.5; waystarPlayer.x = Math.max(20, Math.min(380, waystarPlayer.x));
...
function updateWaystarBoss() {
    ...
    if (waystarState === "phase1") { waystarBoss.x += waystarBoss.vx * waystarSpeedMult; if (waystarBoss.x < 60 || waystarBoss.x > 340) waystarBoss.vx *= -1; waystarBoss.rotation += 0.015 * waystarSpeedMult; waystarBoss.pulse += 0.08; waystarBoss.time += 0.02; }
    else if (waystarState === "phase3") { waystarSmallBoss.alpha = Math.min(1, waystarSmallBoss.alpha + 0.02); var mSpeed = waystarRageMode ? 1.6 : 1.1; mSpeed += waystarEscalationLevel * 0.1; waystarSmallBoss.x += Math.sin(waystarSmallBoss.time) * mSpeed * waystarSpeedMult; waystarSmallBoss.x = Math.max(40, Math.min(360, waystarSmallBoss.x)); waystarSmallBoss.y = 100 + Math.sin(waystarSmallBoss.time * 0.7) * (waystarRageMode ? 18 : 12); waystarSmallBoss.rotation += (waystarRageMode ? 0.06 : 0.035) * waystarSpeedMult; waystarSmallBoss.pulse += (waystarRageMode ? 0.20 : 0.13); waystarSmallBoss.time += (waystarRageMode ? 0.06 : 0.04); }
    if (waystarBoss2.active === true) { waystarBoss2.x += waystarBoss2.vx * waystarSpeedMult; if (waystarBoss2.x < 60 || waystarBoss2.x > 340) waystarBoss2.vx *= -1; waystarBoss2.rotation -= 0.02 * waystarSpeedMult; waystarBoss2.pulse += 0.1; }
...
function updateWaystarAttacks() {
    for (var i = waystarAttacks.length - 1; i >= 0; i--) {
        ...
        if (a.type === "meteor" || a.type === "spiral" || a.type === "star_rain" || a.type === "shotgun_bullet") {
            a.trail.push({ x: a.x, y: a.y, life: 12 }); if (a.trail.length > 8) a.trail.shift();
            a.x += a.vx; a.y += a.vy; a.rotation += (a.rotSpeed||0); a.life--;
            if (a.y > 520 || a.x < -30 || a.x > 430 || a.life <= 0) { if (a.y > 500 && a.type === "meteor") { addWaystarShockwave(a.x, 500, "#ff8800", 50, 12, 3); spawnWaystarParticles(a.x, 500, 12, "#ff8800", 5); } waystarAttacks.splice(i, 1); continue; }
            ...
            if (a.state === "warning") { a.warningTimer--; a.chargeParticles++; if (a.chargeParticles % 3 === 0) { spawnWaystarParticles(a.x + (Math.random() - 0.5) * a.width, 490 - Math.random() * 40, 2, "#ff3333", 4); } if (a.warningTimer <= 0) { a.state = "active"; a.activeTimer = a.maxActive; addWaystarFlash(a.x, 250, 140); addWaystarShockwave(a.x, 250, "#ff3333", 160, 22, 5); addWaystarShockwave(a.x, 250, "#ffffff", 100, 16, 3); wsPlaySound(150, 'sawtooth', 0.7, 0.35); waystarShake = 20; waystarScreenFlash = 10; waystarScreenFlashColor = "#ff3333"; for (var l = 0; l < 4; l++) addWaystarLightning(a.x, 0, a.x + (Math.random() - 0.5) * 30, 500, "#ffffff", 0.9, 2); } }
            else if (a.state === "active") { a.activeTimer--; if (!a.hit && waystarInvulnTimer <= 0 && Math.abs(waystarPlayer.x - a.x) < a.width / 2 + 8) { a.hit = true; applyWaystarHit(a.damage, "ЛАЗЕР!"); } if (a.activeTimer % 2 === 0) spawnWaystarParticles(a.x + (Math.random() - 0.5) * a.width, Math.random() * 500, 3, Math.random() > 0.5 ? "#ffffff" : "#ff3333", 5); if (a.activeTimer % 8 === 0) addWaystarSlash(a.x, Math.random() * 500, Math.PI / 2, 60, 3); if (a.activeTimer <= 0) a.state = "done"; }
            else { waystarAttacks.splice(i, 1); continue; }
        }
        else if (a.type === "giant_laser") {
            if (a.state === "warning") { a.warningTimer--; if (a.trackTimer > 0) { a.trackTimer--; a.x = waystarSmallBoss.x; a.y = waystarSmallBoss.y; a.angle = Math.atan2(waystarPlayer.y - a.y, waystarPlayer.x - a.x); } if (a.warningTimer <= 0) { a.state = "active"; a.activeTimer = a.maxActive; a.x = waystarSmallBoss.x; a.y = waystarSmallBoss.y; addWaystarFlash(a.x, a.y, 180); addWaystarShockwave(a.x, a.y, "#ff00ff", 220, 28, 7); addWaystarShockwave(a.x, a.y, "#ffffff", 160, 22, 4); wsPlaySound(150, 'sawtooth', 0.9, 0.4); waystarShake = 30; waystarScreenFlash = 20; waystarScreenFlashColor = "#ff00ff"; for (var sIdx = 0; sIdx < 8; sIdx++) addWaystarSlash(a.x, a.y, a.angle + (Math.random()-0.5)*0.6, 80, 5); } }
            else if (a.state === "active") { a.activeTimer--; if (!a.hit && waystarInvulnTimer <= 0) { var dx = waystarPlayer.x - a.x, dy = waystarPlayer.y - a.y; var dist = Math.sqrt(dx*dx + dy*dy); var playerAngle = Math.atan2(dy, dx); var angleDiff = Math.abs(playerAngle - a.angle); while (angleDiff > Math.PI) angleDiff = Math.abs(angleDiff - Math.PI * 2); var perpDist = Math.abs(Math.sin(angleDiff)) * dist; if (perpDist < a.width / 2 + 10) { a.hit = true; applyWaystarHit(a.damage, "ГИГА-ЛАЗЕР!"); } } if (a.activeTimer % 2 === 0) { var dist2 = Math.random() * 400; spawnWaystarParticles(a.x + Math.cos(a.angle) * dist2, a.y + Math.sin(a.angle) * dist2, 3, ["#ff00ff", "#ffffff"][Math.floor(Math.random()*2)], 6); } if (a.activeTimer <= 0) a.state = "done"; }
            else { waystarAttacks.splice(i, 1); continue; }
        }
        else if (a.type === "constellation") {
            a.timer--; if (a.warning > 0) a.warning--; if (a.timer <= 0) { waystarAttacks.splice(i, 1); continue; } if (a.hitFlash > 0) a.hitFlash--;
            if (a.warning <= 0 && waystarInvulnTimer <= 0 && !a.hit) {
                for (var p1 = 0; p1 < a.points.length; p1++) {
                    for (var p2 = p1 + 1; p2 < a.points.length; p2++) {
                        var startPt = a.points[p1]; var endPt = a.points[p2];
                        var dist = Math.sqrt(Math.pow(endPt.x - startPt.x, 2) + Math.pow(endPt.y - startPt.y, 2)); if (dist > 220) continue;
                        var d = distToSegment(waystarPlayer.x, waystarPlayer.y, startPt.x, startPt.y, endPt.x, endPt.y);
                        if (d < 14) { a.hit = true; a.hitFlash = 20; addWaystarFlash(waystarPlayer.x, waystarPlayer.y, 60); addWaystarShockwave(waystarPlayer.x, waystarPlayer.y, "#00ffff", 120, 20, 5); addWaystarShockwave(waystarPlayer.x, waystarPlayer.y, "#ffffff", 80, 14, 3); for (var lx = 0; lx < 6; lx++) { var lang = Math.random() * Math.PI * 2; addWaystarLightning(waystarPlayer.x, waystarPlayer.y, waystarPlayer.x + Math.cos(lang) * (60 + Math.random() * 60), waystarPlayer.y + Math.sin(lang) * (60 + Math.random() * 60), Math.random() > 0.5 ? "#00ffff" : "#ffffff", 0.9, 2.5); } spawnWaystarParticles(waystarPlayer.x, waystarPlayer.y, 30, "#00ffff", 8); spawnWaystarParticles(waystarPlayer.x, waystarPlayer.y, 20, "#ffffff", 6); waystarScreenFlash = 12; waystarScreenFlashColor = "#00ffff"; applyWaystarHit(a.damage, "СОЗВЕЗДИЕ!"); break; }
                    }
                    if (a.hit) break;
                }
            }
    ...
    for (var i = waystarBombs.length - 1; i >= 0; i--) {
        ...
        if (!bomb.exploded) {
            ...
            if (progress < 0.4) { bomb.sunPhase = "big"; bomb.sunColor = "#ffdd00"; bomb.sunScale = 1.0 + Math.sin(performance.now() / 100) * 0.05; }
            ...
            else { bomb.sunPhase = "flash"; bomb.sunScale = 0.25 + Math.abs(Math.sin(performance.now() / 40)) * 0.15; bomb.sunColor = "#ffffff"; }
            if (bomb.phaseTimer % 5 === 0) { spawnWaystarParticles(bomb.x + (Math.random() - 0.5) * 60 * bomb.sunScale, bomb.y + (Math.random() - 0.5) * 60 * bomb.sunScale, 1, bomb.sunColor, 3); }
            if (bomb.timer <= 0) {
                bomb.exploded = true; bomb.explosionTimer = 25;
                ...
                addWaystarFlash(bomb.x, bomb.y, bomb.radius * 2);
                spawnWaystarParticles(bomb.x, bomb.y, 60, "#0088ff", 12); spawnWaystarParticles(bomb.x, bomb.y, 40, "#ffffff", 9); spawnWaystarParticles(bomb.x, bomb.y, 30, "#ff4400", 7);
                for (var lb = 0; lb < 8; lb++) { var lang = Math.random() * Math.PI * 2; addWaystarLightning(bomb.x, bomb.y, bomb.x + Math.cos(lang) * (bomb.radius * 1.5), bomb.y + Math.sin(lang) * (bomb.radius * 1.5), Math.random() > 0.5 ? "#00aaff" : "#ffffff", 0.9, 3); }
                wsPlaySound(60, 'sawtooth', 1.2, 0.5); setTimeout(function() { wsPlaySound(40, 'sawtooth', 1.0, 0.4); }, 100);
                waystarShake = 35; waystarScreenFlash = 15; waystarScreenFlashColor = "#00aaff";
...
function applyWaystarHit(dmg, textMsg) {
    ...
    waystarPlayerHp -= dmg; waystarInvulnTimer = 40; waystarShake = 18;
    waystarScreenFlash = 10; waystarScreenFlashColor = "#ff0000"; waystarVignette = 20;
    ...
    if (waystarState === "phase3" && !waystarRageMode && waystarPlayerHp < waystarPlayerMaxHp * 0.5) {
        ...
        waystarScreenFlash = 25; waystarScreenFlashColor = "#ff0000"; waystarShake = 35;
...
// ★★★ ПОБЕДА — ФИКС: перезапуск RAF и жёсткий сброс ★★★
function waystarVictory() {
    ...
    console.log("[WAYSTAR] Финальная сцена, фаза: choice");

    // ★ ФИКС: перезапускаем RAF на всякий случай ★
...
function updateWaystarFinalScene() {
    ...
    try {
        ...
        if (waystarFinalWatchdog > 2400 && waystarFinalPhase !== "done") {
            console.warn("[WAYSTAR] WATCHDOG: принудительное завершение (фаза: " + waystarFinalPhase + ")");
        ...
        if (waystarFinalPhase === "choice") {
            // Ждём выбора — фаза переключается в handleWaystarClick
        }
        else if (waystarFinalPhase === "kill_flash") {
            if (waystarFinalTimer > 30) {
                ...
                for (var i = 0; i < 200; i++) spawnWaystarParticles(200 + (Math.random()-0.5)*300, 250 + (Math.random()-0.5)*300, 1, ["#ffd700", "#ffffff", "#ffaa00"][Math.floor(Math.random()*3)], 12);
                console.log("[WAYSTAR] Финал: награда");
            }
        }
        else if (waystarFinalPhase === "kill_reward") {
            if (waystarFinalTimer > 60) { waystarFinalPhase = "blackout"; waystarFinalTimer = 0; waystarWakeText = "Вы очнулись, но чувствуете себя плохо.\nПродолжайте путь..."; console.log("[WAYSTAR] Финал: blackout"); }
        }
        else if (waystarFinalPhase === "spare_thanks") {
            if (waystarFinalTimer > 90) {
                waystarFinalPhase = "stone_appear"; waystarFinalTimer = 0;
                waystarStone.alpha = 0; waystarStone.x = 200; waystarStone.y = -80; waystarStone.size = 40; waystarStone.rotation = 0;
                wsPlaySound(100, 'sawtooth', 1.5, 0.4);
                console.log("[WAYSTAR] Финал: камень появляется");
            }
        }
        else if (waystarFinalPhase === "stone_appear") {
            waystarStone.alpha = Math.min(1, waystarStone.alpha + 0.03);
            if (waystarStone.y < 250) waystarStone.y += 3;
            waystarStone.rotation += 0.03;
            if (waystarFinalTimer > 60) { waystarFinalPhase = "stone_kill"; waystarFinalTimer = 0; }
            if (waystarFinalTimer % 3 === 0) spawnWaystarParticles(waystarStone.x + (Math.random()-0.5)*80, waystarStone.y + (Math.random()-0.5)*80, 1, "#8B7355", 3);
        }
        else if (waystarFinalPhase === "stone_kill") {
            var targetX = 200, targetY = 100;
            var dx = targetX - waystarStone.x; var dy = targetY - waystarStone.y;
            var dist = Math.sqrt(dx*dx + dy*dy) || 1;
            if (dist > 5) { waystarStone.x += (dx / dist) * 15; waystarStone.y += (dy / dist) * 15; waystarStone.rotation += 0.15; }
            else {
                waystarShake = 50; waystarScreenFlash = 40; waystarScreenFlashColor = "#ffffff";
                addWaystarShockwave(200, 100, "#ff0000", 300, 30, 8);
                addWaystarShockwave(200, 100, "#ffffff", 250, 25, 5);
                for (var i = 0; i < 200; i++) spawnWaystarParticles(200, 100, 1, ["#8B7355", "#ff0000", "#ffffff"][Math.floor(Math.random()*3)], 15);
                wsPlaySound(40, 'sawtooth', 2.0, 0.5);
                waystarFinalPhase = "stone_absorb"; waystarFinalTimer = 0;
                console.log("[WAYSTAR] Финал: удар");
            }
        }
        else if (waystarFinalPhase === "stone_absorb") {
            if (waystarFinalTimer % 5 === 0) { spawnWaystarParticles(200, 100, 3, "#ff00ff", 6); spawnWaystarParticles(200, 100, 2, "#8B7355", 5); addWaystarLightning(waystarStone.x, waystarStone.y, 200, 100, "#8B7355", 0.8, 2); }
            waystarSmallBoss.size *= 0.97; waystarSmallBoss.alpha *= 0.98; waystarStone.size += 0.3;
            if (waystarFinalTimer > 90) { waystarSmallBoss.alpha = 0; waystarFinalPhase = "stone_grow"; waystarFinalTimer = 0; wsPlaySound(200, 'sawtooth', 1.5, 0.4); console.log("[WAYSTAR] Финал: поглощение"); }
        }
        else if (waystarFinalPhase === "stone_grow") {
            waystarStone.size += 1.2; waystarStone.scale = waystarStone.size / 40; waystarStone.rotation += 0.05;
            if (waystarFinalTimer % 3 === 0) spawnWaystarParticles(waystarStone.x + (Math.random()-0.5)*100, waystarStone.y + (Math.random()-0.5)*100, 2, "#ff0000", 6);
            if (waystarFinalTimer % 10 === 0) { addWaystarShockwave(waystarStone.x, waystarStone.y, "#ff0000", 150, 20, 4); wsPlaySound(100, 'sawtooth', 0.5, 0.3); }
            if (waystarFinalTimer > 90) { waystarFinalPhase = "stone_talk"; waystarFinalTimer = 0; waystarStoneText = "ХА-ХА-ХА! СПАСИБО ЗА КОРМ!"; console.log("[WAYSTAR] Финал: рост"); }
        }
        else if (waystarFinalPhase === "stone_talk") {
            waystarStone.rotation += 0.03;
            if (waystarFinalTimer % 4 === 0) spawnWaystarParticles(waystarStone.x + (Math.random()-0.5)*120, waystarStone.y + (Math.random()-0.5)*120, 2, ["#ff0000", "#8B7355"][Math.floor(Math.random()*2)], 7);
            if (waystarFinalTimer === 90) waystarStoneText = "ТЕПЕРЬ Я СТАНУ СИЛЬНЕЕ!";
            if (waystarFinalTimer > 180) { waystarFinalPhase = "stone_dash"; waystarFinalTimer = 0; waystarStoneText = "ПРОЩАЙ, ГЛУПЕЦ!"; wsPlaySound(100, 'sawtooth', 1.0, 0.5); console.log("[WAYSTAR] Финал: разговор"); }
        }
        else if (waystarFinalPhase === "stone_dash") {
            var targetX2 = waystarPlayer.x; var targetY2 = waystarPlayer.y;
            var dx2 = targetX2 - waystarStone.x; var dy2 = targetY2 - waystarStone.y;
            var dist2 = Math.sqrt(dx2*dx2 + dy2*dy2) || 1;
            waystarStone.x += (dx2 / dist2) * 40; waystarStone.y += (dy2 / dist2) * 40;
            waystarShake = 25;
            if (dist2 < 30 || waystarFinalTimer > 40) { waystarFinalPhase = "blackout"; waystarFinalTimer = 0; waystarScreenFlash = 60; waystarScreenFlashColor = "#ffffff"; wsPlaySound(30, 'sawtooth', 2.5, 0.6); console.log("[WAYSTAR] Финал: рывок"); }
        }
        else if (waystarFinalPhase === "blackout") {
            if (waystarBlackoutAlpha < 1) waystarBlackoutAlpha = Math.min(1, waystarBlackoutAlpha + 0.03);
            if (waystarBlackoutAlpha >= 1 && waystarFinalTimer > 60) { waystarFinalPhase = "wakeup"; waystarFinalTimer = 0; if (!waystarWakeText) waystarWakeText = "Вы очнулись, но чувствуете себя плохо.\nПродолжайте путь..."; console.log("[WAYSTAR] Финал: пробуждение"); }
        }
        else if (waystarFinalPhase === "wakeup") {
            waystarBlackoutAlpha = 1;
            waystarWakeAlpha = Math.min(1, waystarWakeAlpha + 0.02);
            if (waystarFinalTimer > 180) { waystarFinalPhase = "done"; waystarFinalTimer = 0; }
        }
...
function waystarRenderLoop() {
    ...
    if (waystarFinalActive) {
        try {
            ...
            console.error("[WAYSTAR] ОШИБКА в финале (render loop):", e);
            console.error(e.stack);
            waystarFinalPhase = "done";
        }
        waystarAnimFrame = requestAnimationFrame(waystarRenderLoop);
        ...
        if (waystarState === "phase1") {
            if (waystarBossHp <= waystarBossMaxHp * 0.5 && waystarBossHp > 0) { waystarTriggerSplit(); }
                ...
                for (var i = waystarPlayerBullets.length - 1; i >= 0; i--) {
                    ...
                    if (Math.sqrt(dx * dx + dy * dy) < waystarBoss.size + b.size) {
                        ...
                        spawnWaystarParticles(b.x, b.y, 8, "#ffd700", 5); addWaystarFlash(b.x, b.y, 15);
            ...
            for (var i = waystarPlayerBullets.length - 1; i >= 0; i--) {
                ...
                if (Math.sqrt(dx * dx + dy * dy) < waystarSmallBoss.size + b.size) {
                    waystarBossHp -= b.damage; waystarBossFlash = 6;
                    spawnWaystarParticles(b.x, b.y, 10, "#ff00ff", 5); addWaystarFlash(b.x, b.y, 20);
            ...
            if (waystarEscalationTimer >= 90) {
                ...
                if (newLevel > waystarEscalationLevel) {
                    ...
                    waystarScreenFlash = 12; waystarScreenFlashColor = "#ff00ff"; waystarShake = 20;
    ...
    for(var i=0; i<waystarNebulas.length; i++) { var neb = waystarNebulas[i]; ctx.save(); ctx.translate(neb.x, neb.y); ctx.rotate(neb.angle); var ng = ctx.createRadialGradient(0,0,0,0,0,neb.radius); ng.addColorStop(0, neb.color); ng.addColorStop(1, "transparent"); ctx.fillStyle = ng; ctx.globalAlpha = waystarState === "phase3" ? (waystarRageMode ? 0.5 : 0.35) : 0.25; ctx.beginPath(); ctx.arc(0,0,neb.radius,0,Math.PI*2); ctx.fill(); ctx.restore(); }
    ctx.globalAlpha = 1;
    ...
    if (waystarState === "phase3") {
        for (var i = 0; i < waystarPhase3Embers.length; i++) { var e = waystarPhase3Embers[i]; ctx.save(); ctx.globalAlpha = Math.max(0, e.life / 100); ctx.fillStyle = e.color; ctx.shadowColor = e.color; ctx.shadowBlur = 10; ctx.beginPath(); ctx.arc(e.x, e.y, e.size, 0, Math.PI * 2); ctx.fill(); ctx.restore(); }
        for (var i = 0; i < waystarPhase3Rings.length; i++) { var r = waystarPhase3Rings[i]; ctx.save(); ctx.globalAlpha = Math.max(0, r.life / r.maxLife) * 0.6; ctx.translate(r.x, r.y); ctx.rotate(r.angle); ctx.strokeStyle = r.color; ctx.lineWidth = 2.5; ctx.shadowColor = r.color; ctx.shadowBlur = 15; ctx.beginPath(); ctx.arc(0, 0, r.radius, 0, Math.PI * 1.7); ctx.stroke(); ctx.restore(); }
    }
    ...
    if (waystarState === "phase1" || waystarDialogActive || waystarState === "split1_anim" || waystarState === "split2_anim" || waystarState === "pre_split2_dialog" || waystarState === "pre_phase2_dialog" || waystarState === "pre_phase3_dialog") {
        ...
        if (waystarState === "phase1") { drawWaystarPhase1Attacks(); drawWaystarPlayerBullets(); }
...
function renderWaystarFinalScene() {
    try {
        ...
        if (waystarShake > 0.5) ctx.translate((Math.random() - 0.5) * waystarShake, (Math.random() - 0.5) * waystarShake);
        var bg = ctx.createLinearGradient(0, 0, 0, 500);
        bg.addColorStop(0, "#0a0015"); bg.addColorStop(0.5, "#1a0025"); bg.addColorStop(1, "#000000");
        ctx.fillStyle = bg; ctx.fillRect(0, 0, 400, 500);
        ...
        if (waystarSmallBoss.alpha > 0 && waystarFinalPhase !== "stone_absorb" && waystarFinalPhase !== "stone_grow" && waystarFinalPhase !== "stone_talk" && waystarFinalPhase !== "stone_dash") { drawWaystarSmallBoss(); }
        else if (waystarSmallBoss.alpha > 0 && waystarFinalPhase === "stone_absorb") { drawWaystarSmallBoss(); }
        if (waystarFinalPhase !== "blackout" && waystarFinalPhase !== "wakeup" && waystarFinalPhase !== "done") { drawWaystarPlayer(); }
        if (waystarStone.alpha > 0) { drawWaystarStone(); }
        if (waystarFinalPhase === "choice") { drawWaystarFinalChoice(); }
        if (waystarStoneText && waystarStone.alpha > 0 && waystarFinalPhase === "stone_talk") { ctx.save(); ctx.font = "bold 18px monospace"; ctx.textAlign = "center"; ctx.fillStyle = "#ff2222"; ctx.shadowColor = "#ff0000"; ctx.shadowBlur = 20; var pulse = 0.85 + Math.abs(Math.sin(waystarFinalTimer / 15)) * 0.15; ctx.globalAlpha = pulse; ctx.fillText(waystarStoneText, 200, 430); ctx.restore(); }
        ...
        if (waystarWakeText && waystarWakeAlpha > 0) { ctx.save(); ctx.globalAlpha = waystarWakeAlpha; ctx.font = "bold 20px monospace"; ctx.textAlign = "center"; ctx.fillStyle = "#ffffff"; ctx.shadowColor = "#aaaaaa"; ctx.shadowBlur = 10; var lines = waystarWakeText.split("\n"); for (var i = 0; i < lines.length; i++) { ctx.fillText(lines[i], 200, 220 + i * 30); } ctx.restore(); }
        if (waystarVignette > 0) { var vg = ctx.createRadialGradient(200, 250, 100, 200, 250, 400); vg.addColorStop(0, "rgba(0,0,0,0)"); vg.addColorStop(1, "rgba(255,0,0," + (waystarVignette / 30) + ")"); ctx.fillStyle = vg; ctx.fillRect(0, 0, 400, 500); }
        if (waystarScreenFlash > 0) { ctx.globalAlpha = waystarScreenFlash / 40; ctx.fillStyle = waystarScreenFlashColor; ctx.fillRect(0, 0, 400, 500); ctx.globalAlpha = 1; }
        ctx.restore();
    } catch(e) {
        console.error("[WAYSTAR] ОШИБКА в renderWaystarFinalScene:", e);
        console.error(e.stack);
    }
}

function drawWaystarStone() {
    var s = waystarStone; if (s.alpha <= 0) return;
    ctx.save(); ctx.globalAlpha = s.alpha; ctx.translate(s.x, s.y); ctx.rotate(s.rotation);
    var pulse = 1 + Math.sin(performance.now() / 200) * 0.05; var finalSize = s.size * pulse;
    var auraSize = finalSize * 3;
    var aura = ctx.createRadialGradient(0, 0, 0, 0, 0, auraSize);
    aura.addColorStop(0, "rgba(255, 0, 0, 0.6)"); aura.addColorStop(0.5, "rgba(139, 115, 85, 0.4)"); aura.addColorStop(1, "transparent");
    ctx.fillStyle = aura; ctx.beginPath(); ctx.arc(0, 0, auraSize, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = "#8B7355"; ctx.strokeStyle = "#3a2818"; ctx.lineWidth = 4; ctx.shadowColor = "#000000"; ctx.shadowBlur = 20;
    ctx.beginPath();
    var sides = 8;
    for (var i = 0; i < sides; i++) { var ang = (i / sides) * Math.PI * 2; var r = finalSize * (0.9 + Math.sin(i * 1.7) * 0.15); var px = Math.cos(ang) * r; var py = Math.sin(ang) * r; if (i === 0) ctx.moveTo(px, py); else ctx.lineTo(px, py); }
    ctx.closePath(); ctx.fill(); ctx.stroke(); ctx.shadowBlur = 0;
    ctx.strokeStyle = "#2a1810"; ctx.lineWidth = 3;
    for (var i = 0; i < 6; i++) { var a1 = (i / 6) * Math.PI * 2; var a2 = a1 + 0.8; ctx.beginPath(); ctx.moveTo(Math.cos(a1) * finalSize * 0.2, Math.sin(a1) * finalSize * 0.2); ctx.lineTo(Math.cos(a2) * finalSize * 0.7, Math.sin(a2) * finalSize * 0.7); ctx.stroke(); }
    var eyeColor = "#ff0000"; ctx.fillStyle = eyeColor; ctx.shadowColor = eyeColor; ctx.shadowBlur = 25;
    var eyeOX = finalSize * 0.3; var eyeOY = -finalSize * 0.15; var eyeS = finalSize * 0.15;
    ctx.beginPath(); ctx.arc(-eyeOX, eyeOY, eyeS, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.arc(eyeOX, eyeOY, eyeS, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = "#000000"; ctx.shadowBlur = 0;
    ctx.beginPath(); ctx.arc(-eyeOX, eyeOY, eyeS * 0.4, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.arc(eyeOX, eyeOY, eyeS * 0.4, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = "#000000"; ctx.lineWidth = 4;
    ctx.beginPath(); ctx.arc(0, finalSize * 0.25, finalSize * 0.4, 0.2, Math.PI - 0.2); ctx.stroke();
    ctx.restore();
}
...
function drawWaystarSmallBoss() {
    ...
    if (waystarRageMode) { glow.addColorStop(0, "rgba(255, 0, 0, 0.95)"); glow.addColorStop(0.3, "rgba(255, 100, 100, 0.6)"); glow.addColorStop(0.6, "rgba(255, 0, 0, 0.3)"); glow.addColorStop(1, "transparent"); }
    ...
    var rayCount = waystarRageMode ? 12 : 8; rayCount += waystarEscalationLevel;
...
function drawWaystarPhase1Attacks() {
    for (var i = 0; i < waystarAttacks.length; i++) {
        ...
        if (a.type === "meteor" || a.type === "spiral" || a.type === "star_rain" || a.type === "shotgun_bullet") {
            ...
            ctx.save();
            if (a.state === "warning") { var pulseAlpha = 0.3 + Math.sin(performance.now() / 60) * 0.25; var chargeR = 15 + Math.abs(Math.sin(performance.now() / 80)) * 15; var chargeGlow = ctx.createRadialGradient(a.x, 490, 0, a.x, 490, chargeR * 2); chargeGlow.addColorStop(0, "rgba(255, 255, 255, 0.9)"); chargeGlow.addColorStop(0.4, "rgba(255, 100, 100, 0.6)"); chargeGlow.addColorStop(1, "transparent"); ctx.fillStyle = chargeGlow; ctx.beginPath(); ctx.arc(a.x, 490, chargeR * 2, 0, Math.PI * 2); ctx.fill(); ctx.globalAlpha = pulseAlpha; ctx.fillStyle = "#ff3333"; ctx.fillRect(a.x - a.width / 2, 0, a.width, 500); ctx.globalAlpha = 0.9; ctx.strokeStyle = "#ffffff"; ctx.lineWidth = 2; ctx.setLineDash([8, 6]); ctx.beginPath(); ctx.moveTo(a.x - a.width / 2, 0); ctx.lineTo(a.x - a.width / 2, 500); ctx.moveTo(a.x + a.width / 2, 0); ctx.lineTo(a.x + a.width / 2, 500); ctx.stroke(); ctx.setLineDash([]); ctx.globalAlpha = 1; }
            else if (a.state === "active") { var fadeIn = Math.min(1, (a.maxActive - a.activeTimer) / 5 + 0.3); var fadeOut = Math.min(1, a.activeTimer / 12); var activeAlpha = Math.min(fadeIn, fadeOut); ctx.globalAlpha = activeAlpha; var outerGlow = ctx.createLinearGradient(a.x - a.width * 1.5, 0, a.x + a.width * 1.5, 0); outerGlow.addColorStop(0, "rgba(255, 50, 50, 0)"); outerGlow.addColorStop(0.3, "rgba(255, 100, 50, 0.5)"); outerGlow.addColorStop(0.5, "rgba(255, 255, 255, 0.9)"); outerGlow.addColorStop(0.7, "rgba(255, 100, 50, 0.5)"); outerGlow.addColorStop(1, "rgba(255, 50, 50, 0)"); ctx.fillStyle = outerGlow; ctx.fillRect(a.x - a.width * 1.5, 0, a.width * 3, 500); ctx.fillStyle = "#ff3333"; ctx.globalAlpha = activeAlpha * 0.95; ctx.fillRect(a.x - a.width / 2, 0, a.width, 500); var coreWidth = a.width * (0.35 + Math.sin(performance.now() / 40) * 0.08); ctx.fillStyle = "#ffffff"; ctx.globalAlpha = activeAlpha; ctx.fillRect(a.x - coreWidth / 2, 0, coreWidth, 500); ctx.fillStyle = "#00d4ff"; ctx.globalAlpha = activeAlpha * 0.6; ctx.fillRect(a.x - a.width / 2 + 4, 0, 6, 500); ctx.fillRect(a.x + a.width / 2 - 10, 0, 6, 500); ctx.strokeStyle = "#ffffff"; ctx.lineWidth = 2; ctx.globalAlpha = activeAlpha; ctx.beginPath(); ctx.moveTo(a.x - a.width / 2, 0); ctx.lineTo(a.x - a.width / 2, 500); ctx.moveTo(a.x + a.width / 2, 0); ctx.lineTo(a.x + a.width / 2, 500); ctx.stroke(); if (a.activeTimer % 3 === 0) addWaystarLightning(a.x + (Math.random() - 0.5) * a.width, 0, a.x + (Math.random() - 0.5) * a.width, 500, "#ffffff", 0.5, 1.5); }
            ctx.restore();
        } else if (a.type === "giant_laser") {
            ctx.save(); ctx.translate(a.x, a.y); ctx.rotate(a.angle);
            if (a.state === "warning") { var pulseAlpha2 = 0.4 + Math.sin(performance.now() / 60) * 0.25; var chargeR2 = 20 + Math.abs(Math.sin(performance.now() / 70)) * 25; var chargeGlow2 = ctx.createRadialGradient(0, 0, 0, 0, 0, chargeR2 * 2.5); chargeGlow2.addColorStop(0, "rgba(255, 255, 255, 0.95)"); chargeGlow2.addColorStop(0.4, "rgba(255, 100, 255, 0.7)"); chargeGlow2.addColorStop(1, "transparent"); ctx.fillStyle = chargeGlow2; ctx.beginPath(); ctx.arc(0, 0, chargeR2 * 2.5, 0, Math.PI * 2); ctx.fill(); ctx.globalAlpha = pulseAlpha2; ctx.fillStyle = "#ff00ff"; ctx.fillRect(0, -a.width / 2, 800, a.width); ctx.globalAlpha = 0.95; ctx.strokeStyle = "#ffffff"; ctx.lineWidth = 2; ctx.setLineDash([8, 6]); ctx.beginPath(); ctx.moveTo(0, -a.width / 2); ctx.lineTo(800, -a.width / 2); ctx.moveTo(0, a.width / 2); ctx.lineTo(800, a.width / 2); ctx.stroke(); ctx.setLineDash([]); ctx.globalAlpha = 1; ctx.font = "bold 22px monospace"; ctx.textAlign = "center"; ctx.fillStyle = "#ffffff"; ctx.fillText("⚠", 200, 8); }
            else if (a.state === "active") { var fadeIn2 = Math.min(1, (a.maxActive - a.activeTimer) / 5 + 0.3); var fadeOut2 = Math.min(1, a.activeTimer / 12); var activeAlpha2 = Math.min(fadeIn2, fadeOut2); ctx.globalAlpha = activeAlpha2; var outerGlow2 = ctx.createLinearGradient(0, -a.width * 1.5, 0, a.width * 1.5); outerGlow2.addColorStop(0, "rgba(255, 0, 255, 0)"); outerGlow2.addColorStop(0.3, "rgba(255, 100, 255, 0.5)"); outerGlow2.addColorStop(0.5, "rgba(255, 255, 255, 0.95)"); outerGlow2.addColorStop(0.7, "rgba(255, 100, 255, 0.5)"); outerGlow2.addColorStop(1, "rgba(255, 0, 255, 0)"); ctx.fillStyle = outerGlow2; ctx.fillRect(0, -a.width * 1.5, 800, a.width * 3); ctx.fillStyle = "#ff00ff"; ctx.globalAlpha = activeAlpha2 * 0.95; ctx.fillRect(0, -a.width / 2, 800, a.width); var coreWidth2 = a.width * (0.35 + Math.sin(performance.now() / 40) * 0.1); ctx.fillStyle = "#ffffff"; ctx.globalAlpha = activeAlpha2; ctx.fillRect(0, -coreWidth2 / 2, 800, coreWidth2); ctx.strokeStyle = "#ffffff"; ctx.lineWidth = 2; ctx.globalAlpha = activeAlpha2; ctx.beginPath(); ctx.moveTo(0, -a.width / 2); ctx.lineTo(800, -a.width / 2); ctx.moveTo(0, a.width / 2); ctx.lineTo(800, a.width / 2); ctx.stroke(); if (a.activeTimer % 4 === 0) { var dist3 = Math.random() * 400; addWaystarSlash(a.x + Math.cos(a.angle) * dist3, a.y + Math.sin(a.angle) * dist3, a.angle + Math.PI/2, 60, 4); } }
            ctx.restore();
        } else if (a.type === "constellation") {
            ctx.save();
            var isWarn = a.warning > 0; var now = performance.now();
            if (isWarn) { var warnAlpha = 0.3 + Math.sin(now / 100) * 0.2; ctx.globalAlpha = warnAlpha; ctx.strokeStyle = "#00d4ff"; ctx.lineWidth = 2; ctx.setLineDash([8, 6]); ctx.beginPath(); for (var p = 0; p < a.points.length; p++) { if (p === 0) ctx.moveTo(a.points[p].x, a.points[p].y); else ctx.lineTo(a.points[p].x, a.points[p].y); } ctx.stroke(); ctx.setLineDash([]); for (var p = 0; p < a.points.length; p++) { var pt = a.points[p]; pt.pulse += 0.15; var pulseSize = 4 + Math.sin(pt.pulse) * 2; ctx.fillStyle = "#00d4ff"; ctx.shadowColor = "#00d4ff"; ctx.shadowBlur = 15; ctx.beginPath(); ctx.arc(pt.x, pt.y, pulseSize, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = "#ffffff"; ctx.shadowBlur = 0; ctx.beginPath(); ctx.arc(pt.x, pt.y, pulseSize * 0.4, 0, Math.PI * 2); ctx.fill(); } }
            else { var lifeRatio = a.timer / 180; var activeAlpha = Math.min(1, lifeRatio * 2); ctx.strokeStyle = "#00ffff"; ctx.shadowColor = "#00ffff"; ctx.shadowBlur = 25; for (var p1 = 0; p1 < a.points.length; p1++) { for (var p2 = p1 + 1; p2 < a.points.length; p2++) { var startPt = a.points[p1]; var endPt = a.points[p2]; var dist = Math.sqrt(Math.pow(endPt.x - startPt.x, 2) + Math.pow(endPt.y - startPt.y, 2)); if (dist > 220) continue; ctx.globalAlpha = activeAlpha * 0.4; ctx.lineWidth = 10; ctx.beginPath(); ctx.moveTo(startPt.x, startPt.y); ctx.lineTo(endPt.x, endPt.y); ctx.stroke(); ctx.globalAlpha = activeAlpha * 0.95; ctx.strokeStyle = "#00ffff"; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(startPt.x, startPt.y); ctx.lineTo(endPt.x, endPt.y); ctx.stroke(); ctx.globalAlpha = activeAlpha; ctx.strokeStyle = "#ffffff"; ctx.lineWidth = 1.5; ctx.beginPath(); ctx.moveTo(startPt.x, startPt.y); ctx.lineTo(endPt.x, endPt.y); ctx.stroke(); if (Math.random() < 0.15) { var t = Math.random(); var sx = startPt.x + (endPt.x - startPt.x) * t; var sy = startPt.y + (endPt.y - startPt.y) * t; addWaystarLightning(sx, sy, sx + (Math.random() - 0.5) * 30, sy + (Math.random() - 0.5) * 30, "#ffffff", 0.7, 2); } } } for (var p = 0; p < a.points.length; p++) { var pt = a.points[p]; pt.pulse += 0.2; var pulseSize = 6 + Math.sin(pt.pulse) * 2.5; var sg = ctx.createRadialGradient(pt.x, pt.y, 0, pt.x, pt.y, pulseSize * 3); sg.addColorStop(0, "rgba(0, 255, 255, 0.9)"); sg.addColorStop(0.4, "rgba(0, 200, 255, 0.5)"); sg.addColorStop(1, "transparent"); ctx.globalAlpha = activeAlpha; ctx.fillStyle = sg; ctx.beginPath(); ctx.arc(pt.x, pt.y, pulseSize * 3, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = "#ffffff"; ctx.shadowColor = "#00ffff"; ctx.shadowBlur = 20; ctx.beginPath(); ctx.arc(pt.x, pt.y, pulseSize, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0; } if (a.hitFlash > 0) { a.hitFlash--; var flashAlpha = a.hitFlash / 20; ctx.globalAlpha = flashAlpha; var fg = ctx.createRadialGradient(waystarPlayer.x, waystarPlayer.y, 0, waystarPlayer.x, waystarPlayer.y, 100 * (1 - flashAlpha)); fg.addColorStop(0, "#ffffff"); fg.addColorStop(0.4, "#00ffff"); fg.addColorStop(1, "transparent"); ctx.fillStyle = fg; ctx.beginPath(); ctx.arc(waystarPlayer.x, waystarPlayer.y, 100 * (1 - flashAlpha), 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = "#ffffff"; ctx.lineWidth = 4 * flashAlpha; ctx.shadowColor = "#00ffff"; ctx.shadowBlur = 30; ctx.beginPath(); ctx.arc(waystarPlayer.x, waystarPlayer.y, 60 * (1 - flashAlpha), 0, Math.PI * 2); ctx.stroke(); } }
            ctx.restore();
        }
    }
}
...
function drawWaystarBombs() {
    for (var i = 0; i < waystarBombs.length; i++) {
        ...
        if (!b.exploded) {
            ...
            ctx.fillStyle = coreGrad; ctx.shadowColor = sunColor; ctx.shadowBlur = b.sunPhase === "flash" ? 40 : 25;
            ctx.beginPath(); ctx.arc(b.x, b.y, size, 0, Math.PI * 2); ctx.fill();
            if (b.sunPhase === "big" || b.sunPhase === "shrink") { ctx.strokeStyle = sunColor; ctx.lineWidth = 2; ctx.shadowBlur = 15; var rayCount = b.sunPhase === "big" ? 8 : 6; for (var r = 0; r < rayCount; r++) { var ang = (r / rayCount) * Math.PI * 2 + performance.now() / 500; var innerR = size * 1.1; var outerR = size * (1.3 + Math.sin(performance.now() / 100 + r) * 0.1); ctx.beginPath(); ctx.moveTo(b.x + Math.cos(ang) * innerR, b.y + Math.sin(ang) * innerR); ctx.lineTo(b.x + Math.cos(ang) * outerR, b.y + Math.sin(ang) * outerR); ctx.stroke(); } }
            if (b.sunPhase === "blue") { ctx.strokeStyle = "#00aaff"; ctx.lineWidth = 2; ctx.shadowBlur = 20; for (var k = 0; k < 3; k++) { var offset = (k / 3) * Math.PI * 2 + performance.now() / 300; ctx.beginPath(); for (var step = 0; step <= 20; step++) { var t = step / 20; var spiralAng = offset + t * Math.PI * 4; var spiralR = size * 0.8 * (1 - t * 0.5); var sx = b.x + Math.cos(spiralAng) * spiralR; var sy = b.y + Math.sin(spiralAng) * spiralR; if (step === 0) ctx.moveTo(sx, sy); else ctx.lineTo(sx, sy); } ctx.stroke(); } }
            var warnPulse = Math.max(0, 0.3 + Math.sin(b.timer / 8) * 0.3); ctx.strokeStyle = b.sunPhase === "blue" ? "#00aaff" : "#ff3333"; ctx.lineWidth = 2; ctx.globalAlpha = warnPulse; ctx.setLineDash([6, 4]); ctx.beginPath(); ctx.arc(b.x, b.y, b.radius, 0, Math.PI * 2); ctx.stroke(); ctx.setLineDash([]);
            ctx.restore();
        } else {
            var explProgress = 1 - b.explosionTimer / 25; var explAlpha = 1 - explProgress;
            ctx.globalAlpha = explAlpha; var ringR = b.radius * explProgress * 1.5;
            ...
            ctx.globalAlpha = explAlpha * 0.9; var coreR = b.radius * explProgress * 0.6;
...
function drawWaystarDash() { if (!waystarDash) return; ctx.save(); var d = waystarDash; for (var i = 0; i < 8; i++) { var t = i / 8; var x = d.startX + (d.targetX - d.startX) * t; var y = d.startY + (d.targetY - d.startY) * t; ctx.globalAlpha = (1 - t) * 0.4; ctx.fillStyle = "#ff00ff"; ctx.beginPath(); ctx.arc(x, y, 20 * (1 - t), 0, Math.PI * 2); ctx.fill(); } ctx.restore(); }
...
console.log("[WAYSTAR] v10.5 загружено! Фикс: выбор сразу переключает фазу");
